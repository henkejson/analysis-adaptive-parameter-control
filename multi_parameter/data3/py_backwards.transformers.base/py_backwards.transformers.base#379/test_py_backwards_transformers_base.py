# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import typed_ast._ast3 as module_0
import py_backwards.transformers.base as module_1
import typed_ast.ast3 as module_2


def test_case_0():
    l_shift_0 = module_0.LShift()
    base_node_transformer_0 = module_1.BaseNodeTransformer(l_shift_0)


def test_case_1():
    r_shift_0 = module_0.RShift()
    base_import_rewrite_0 = module_1.BaseImportRewrite(r_shift_0)
    list_0 = [base_import_rewrite_0, base_import_rewrite_0]
    import_from_0 = module_0.ImportFrom(*list_0)
    base_import_rewrite_0.visit_ImportFrom(import_from_0)


def test_case_2():
    r_shift_0 = module_0.RShift()
    base_import_rewrite_0 = module_1.BaseImportRewrite(r_shift_0)
    base_import_rewrite_1 = module_2.iter_child_nodes(r_shift_0)
    list_0 = [base_import_rewrite_1, base_import_rewrite_1]
    import_from_0 = module_0.ImportFrom(*list_0)
    var_0 = base_import_rewrite_0.visit_ImportFrom(import_from_0)


def test_case_3():
    r_shift_0 = module_0.RShift()
    base_import_rewrite_0 = module_1.BaseImportRewrite(r_shift_0)
    var_0 = module_2.dump(r_shift_0)
    list_0 = [var_0, var_0]
    import_from_0 = module_0.ImportFrom(*list_0)
    base_import_rewrite_0.visit_ImportFrom(import_from_0)


def test_case_4():
    r_shift_0 = module_0.RShift()
    none_type_0 = None
    base_import_rewrite_0 = module_1.BaseImportRewrite(none_type_0)
    var_0 = module_2.iter_child_nodes(r_shift_0)
    list_0 = [none_type_0, var_0, var_0]
    import_from_0 = module_0.ImportFrom(*list_0)
    var_1 = base_import_rewrite_0.visit_ImportFrom(import_from_0)
    var_0.generic_visit(list_0)
