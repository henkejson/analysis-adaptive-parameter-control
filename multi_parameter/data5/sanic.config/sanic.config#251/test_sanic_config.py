# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import sanic.config as module_0
import sanic.utils as module_1
import inspect as module_2


def test_case_0():
    config_0 = module_0.Config()


def test_case_1():
    str_0 = "fD^RG?V\td)/"
    str_1 = 'K"t*Vr9+018g'
    str_2 = "usISVRl3!uAN)"
    dict_0 = {str_0: str_0, str_1: str_1, str_2: str_2}
    config_0 = module_0.Config(dict_0, converters=str_0)
    config_1 = module_0.Config()


def test_case_2():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)
    var_1 = module_0.Config()
    config_1 = module_0.Config(config_0, keep_alive=var_1)
    module_1.load_module_from_file_location(config_1, config_1, **config_1)


def test_case_3():
    str_0 = "Decorator to register a function to be called later.\n\n    Args:\n        func (Callable): Function to be called later.\n        as_decorator (bool): Whether the function should be called\n            immediately or not.\n    "
    str_1 = '$nVmb}"M'
    config_0 = module_0.Config(env_prefix=str_1, keep_alive=str_0)
    config_0.__getattr__(str_0)


def test_case_4():
    config_0 = module_0.Config()
    var_0 = module_2.getmembers(config_0)
    list_0 = [var_0]
    none_type_0 = config_0.update(*list_0)


def test_case_5():
    config_0 = module_0.Config()
    config_0.__getattr__(config_0)


def test_case_6():
    config_0 = module_0.Config()
    none_type_0 = None
    config_0.update_config(none_type_0)


def test_case_7():
    config_0 = module_0.Config()
    bytes_0 = b"\x97\xfa{T\x86\xd7\xf3\x83`\xd9n"
    config_0.update_config(bytes_0)


def test_case_8():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)


def test_case_9():
    config_0 = module_0.Config()
    var_0 = config_0.load_environment_vars()
    none_type_0 = config_0.register_type(config_0)
    var_1 = config_0.update_config(config_0)


def test_case_10():
    config_0 = module_0.Config()
    var_0 = module_2.getmembers(config_0)
    list_0 = [var_0]
    str_0 = "XI\tR]z5!{qw;.hg/|"
    none_type_0 = config_0.__setitem__(str_0, config_0)
    none_type_1 = config_0.update(*list_0)
    var_1 = config_0.update_config(config_0)
    config_0.update_config(var_1)


def test_case_11():
    config_0 = module_0.Config()
    str_0 = "!sD'+qTG+^&"
    none_type_0 = config_0.__setattr__(str_0, config_0)
    config_1 = module_0.Config(config_0, converters=none_type_0)
    var_0 = module_2.getmembers(config_0)
    list_0 = [var_0]
    none_type_1 = config_0.update(*list_0)
    none_type_2 = config_0.update(*list_0)
    config_2 = module_0.Config(env_prefix=none_type_1)
    var_1 = config_0.update_config(config_0)
    var_2 = var_1.__bool__()
    config_0.update_config(var_1)


def test_case_12():
    str_0 = "{[,|g(#{"
    module_0.Config(converters=str_0)


def test_case_13():
    config_0 = module_0.Config()
    var_0 = module_2.getmembers(config_0)
    list_0 = [var_0]
    none_type_0 = config_0.update(*list_0)
    str_0 = "XI\tR]z5!{qw;.hg/|"
    none_type_1 = config_0.__setitem__(str_0, config_0)
    none_type_2 = config_0.update(*list_0)
    var_1 = config_0.update_config(config_0)
    config_0.update_config(var_1)


def test_case_14():
    config_0 = module_0.Config()
    bytes_0 = b"YK"
    none_type_0 = config_0.register_type(bytes_0)
    var_0 = module_2.getmembers(config_0)
    list_0 = [var_0]
    none_type_1 = config_0.update(*list_0)
    none_type_2 = config_0.update(*list_0)
    var_1 = config_0.update_config(config_0)
    var_2 = module_2.getmembers(config_0)
    config_0.__getattr__(var_2)
