# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import codetiming._timer as module_0
import contextlib as module_1


def test_case_0():
    float_arg_0 = module_0.FloatArg()


def test_case_1():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    float_0 = timer_0.stop()


def test_case_2():
    timer_0 = module_0.Timer()
    timer_0.stop()


def test_case_3():
    timer_0 = module_0.Timer()
    timer_1 = module_0.Timer(initial_text=timer_0)
    timer_2 = timer_1.__enter__()
    timer_0.__exit__()


def test_case_4():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    timer_0.start()


def test_case_5():
    context_decorator_0 = module_1.ContextDecorator()
    none_type_0 = None
    timer_0 = module_0.Timer(text=context_decorator_0, logger=none_type_0)
    float_arg_0 = module_0.FloatArg()
    var_0 = timer_0.__eq__(timer_0)
    timer_1 = timer_0.__enter__()


def test_case_6():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    var_0 = timer_0.__eq__(timer_1)
    bool_0 = True
    timer_2 = module_0.Timer(timer_1, initial_text=bool_0)
    timer_3 = timer_2.__enter__()
    none_type_0 = timer_1.__exit__()
    timer_3.stop()


def test_case_7():
    list_0 = []
    timer_0 = module_0.Timer(logger=list_0)
    none_type_0 = timer_0.start()
    none_type_1 = timer_0.__exit__()
    var_0 = timer_0.__repr__()
    var_1 = var_0.__len__()
    none_type_2 = timer_0.start()
    var_2 = var_0.__eq__(none_type_1)
    var_3 = timer_0.__eq__(timer_0)
    str_0 = "Timer started"
    timer_1 = module_0.Timer(str_0)
    float_arg_0 = module_0.FloatArg()
    timer_2 = module_0.Timer(text=var_3, initial_text=timer_0)
    var_4 = var_3.__repr__()
    var_3.__iter__()


def test_case_8():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    timer_2 = module_0.Timer(timer_0, timer_0)
    timer_3 = timer_2.__enter__()
    str_0 = "seconds"
    timer_3.__exit__(*str_0)


def test_case_9():
    str_0 = "HwXQ&&Zx~~)"
    timer_0 = module_0.Timer(initial_text=str_0)
    none_type_0 = None
    none_type_1 = timer_0.start()
    var_0 = timer_0.__call__(none_type_0)
    module_0.TimerError(**var_0)
