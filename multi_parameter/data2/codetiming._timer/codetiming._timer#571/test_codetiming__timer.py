# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import codetiming._timer as module_0
import builtins as module_1
import dataclasses as module_2


def test_case_0():
    timer_0 = module_0.Timer()


def test_case_1():
    float_arg_0 = module_0.FloatArg()
    timer_0 = module_0.Timer()
    list_0 = [timer_0, float_arg_0, float_arg_0]
    timer_1 = timer_0.__enter__()
    timer_error_0 = module_0.TimerError(*list_0)
    none_type_0 = timer_0.__exit__()
    timer_2 = timer_0.__enter__()
    timer_1.__enter__()


def test_case_2():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()


def test_case_3():
    timer_0 = module_0.Timer()
    float_arg_0 = module_0.FloatArg()
    timer_0.stop()


def test_case_4():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    float_0 = timer_1.stop()


def test_case_5():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    none_type_0 = timer_0.__exit__()
    timer_2 = timer_0.__enter__()
    float_0 = timer_2.stop()
    exception_0 = module_1.Exception()


def test_case_6():
    timer_error_0 = module_0.TimerError()
    none_type_0 = None
    timer_0 = module_0.Timer(text=none_type_0, logger=none_type_0)
    var_0 = timer_0.__enter__()
    module_0.TimerError(*var_0, **var_0)


def test_case_7():
    float_arg_0 = module_0.FloatArg()
    list_0 = [float_arg_0, float_arg_0, float_arg_0]
    var_0 = module_2.dataclass(unsafe_hash=list_0)
    timer_0 = module_0.Timer(var_0, float_arg_0, var_0)
    timer_1 = timer_0.__enter__()
    timer_error_0 = module_0.TimerError()
    var_0.__enter__()


def test_case_8():
    bool_0 = True
    timer_0 = module_0.Timer(initial_text=bool_0)
    float_arg_0 = module_0.FloatArg()
    none_type_0 = timer_0.start()
    none_type_1 = None
    float_arg_0.__contains__(none_type_1)


def test_case_9():
    float_arg_0 = module_0.FloatArg()
    str_0 = "A)O76H<(Mhd0,%T"
    timer_0 = module_0.Timer(str_0, initial_text=str_0)
    var_0 = timer_0.__repr__()
    list_0 = [var_0, float_arg_0, float_arg_0]
    timer_1 = timer_0.__enter__()
    timer_error_0 = module_0.TimerError(*list_0)
    none_type_0 = timer_1.__exit__()
    timer_2 = timer_1.__enter__()
    var_1 = timer_2.__eq__(var_0)
    none_type_1 = None
    var_2 = var_1.__eq__(none_type_1)
    var_2.__enter__()


def test_case_10():
    timer_error_0 = module_0.TimerError()
    none_type_0 = None
    timer_0 = module_0.Timer(none_type_0)
    timer_1 = module_0.Timer(none_type_0, logger=none_type_0)
    none_type_1 = timer_1.start()
    var_0 = timer_1.__repr__()
    var_1 = timer_1.__call__(timer_0)
    timer_2 = timer_0.__enter__()
    var_2 = timer_2.__repr__()
    float_0 = timer_1.stop()
    module_0.TimerError(**timer_1)
