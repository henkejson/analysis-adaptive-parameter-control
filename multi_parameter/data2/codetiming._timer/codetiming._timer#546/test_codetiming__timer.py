# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import codetiming._timer as module_0
import dataclasses as module_1


def test_case_0():
    timer_error_0 = module_0.TimerError()


def test_case_1():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    float_0 = timer_0.stop()
    none_type_0 = timer_0.start()
    timer_1.start()


def test_case_2():
    timer_0 = module_0.Timer()
    none_type_0 = timer_0.start()


def test_case_3():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    none_type_0 = timer_0.__exit__()


def test_case_4():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    float_0 = timer_0.stop()
    timer_0.stop()


def test_case_5():
    bool_0 = False
    timer_0 = module_0.Timer(logger=bool_0)
    none_type_0 = timer_0.start()
    none_type_1 = None
    set_0 = {none_type_1}
    list_0 = [none_type_1, none_type_0, none_type_1, set_0]
    none_type_2 = timer_0.__exit__(*list_0)
    none_type_3 = timer_0.start()
    timer_0.__enter__()


def test_case_6():
    timer_0 = module_0.Timer()
    var_0 = module_1.dataclass(match_args=timer_0, kw_only=timer_0)
    str_0 = "i;#1[J0#orxon.z#C"
    timer_1 = module_0.Timer(text=str_0, initial_text=timer_0)
    timer_2 = timer_1.__enter__()
    timer_2.__exit__(*var_0)


def test_case_7():
    str_0 = "Up0A2UT4\n>"
    timer_0 = module_0.Timer(str_0)
    timer_1 = timer_0.__enter__()
    none_type_0 = timer_0.__exit__()


def test_case_8():
    str_0 = "j<UyD:+#0x.IiQ{"
    timer_0 = module_0.Timer(initial_text=str_0)
    timer_0.__enter__()


def test_case_9():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    str_0 = "i;#1[J0#orxon.z#C"
    timer_2 = module_0.Timer(str_0, str_0, timer_0, timer_1)
    timer_3 = timer_2.__enter__()
    none_type_0 = timer_1.__exit__()


def test_case_10():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    float_0 = timer_0.stop()
    float_arg_0 = module_0.FloatArg()
    timer_2 = timer_0.__enter__()
    float_1 = timer_2.stop()
    timer_3 = module_0.Timer(text=timer_2)
    timer_4 = timer_3.__enter__()
    none_type_0 = timer_4.__exit__()
