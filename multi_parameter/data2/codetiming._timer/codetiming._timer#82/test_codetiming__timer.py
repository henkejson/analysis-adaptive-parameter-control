# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import codetiming._timer as module_0
import dataclasses as module_1
import contextlib as module_2


def test_case_0():
    float_arg_0 = module_0.FloatArg()


def test_case_1():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    none_type_0 = timer_0.__exit__()


def test_case_2():
    none_type_0 = None
    timer_0 = module_0.Timer(none_type_0)
    timer_0.stop()


def test_case_3():
    timer_0 = module_0.Timer()
    timer_0.__exit__()


def test_case_4():
    none_type_0 = None
    var_0 = module_1.field(init=none_type_0, repr=none_type_0, compare=none_type_0)
    timer_0 = module_0.Timer(none_type_0, initial_text=var_0)
    timer_1 = timer_0.__enter__()


def test_case_5():
    none_type_0 = None
    var_0 = module_1.field(init=none_type_0, repr=none_type_0, compare=none_type_0)
    timer_0 = module_0.Timer(none_type_0, initial_text=var_0)
    timer_1 = timer_0.__enter__()
    timer_1.__enter__()


def test_case_6():
    timer_0 = module_0.Timer()
    int_0 = 0
    list_0 = []
    timer_error_0 = module_0.TimerError(*list_0)
    timer_1 = module_0.Timer(logger=int_0)
    none_type_0 = timer_1.start()
    var_0 = timer_0.__repr__()


def test_case_7():
    str_0 = "SWb|l"
    timer_0 = module_0.Timer(initial_text=str_0)
    none_type_0 = timer_0.start()
    bytes_0 = b"O\xf7`*J4\xb0\xe1~V"
    timer_1 = module_0.Timer(initial_text=bytes_0, logger=bytes_0)
    timer_1.__enter__()


def test_case_8():
    none_type_0 = None
    timer_0 = module_0.Timer(text=none_type_0, logger=none_type_0)
    timer_1 = timer_0.__enter__()
    var_0 = timer_0.__call__(none_type_0)
    float_0 = timer_0.stop()
    var_1 = var_0.__eq__(var_0)
    module_2.ContextDecorator(**var_0)


def test_case_9():
    none_type_0 = None
    bytes_0 = b"\xba\xb6"
    var_0 = module_1.field(compare=none_type_0, kw_only=bytes_0)
    timer_0 = module_0.Timer(var_0, initial_text=var_0)
    timer_1 = timer_0.__enter__()
    var_0.__ior__(var_0)


def test_case_10():
    none_type_0 = None
    var_0 = module_1.field(init=none_type_0, repr=none_type_0, compare=none_type_0)
    var_1 = module_1.field(compare=var_0)
    timer_0 = module_0.Timer(var_1)
    timer_1 = timer_0.__enter__()
    float_0 = timer_0.stop()
    var_2 = timer_1.__call__(var_0)
    float_0.stop()


def test_case_11():
    none_type_0 = None
    var_0 = module_1.field(init=none_type_0, repr=none_type_0, compare=none_type_0)
    timer_0 = module_0.Timer(none_type_0, initial_text=var_0)
    timer_1 = module_0.Timer(text=timer_0, initial_text=timer_0)
    timer_2 = timer_1.__enter__()
    var_1 = timer_2.__call__(none_type_0)
    float_0 = timer_2.stop()
    var_1.keys()
