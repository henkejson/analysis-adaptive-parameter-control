# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import codetiming._timer as module_0


def test_case_0():
    timer_error_0 = module_0.TimerError()


def test_case_1():
    timer_0 = module_0.Timer()
    none_type_0 = timer_0.start()
    timer_0.__enter__()


def test_case_2():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()


def test_case_3():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    none_type_0 = timer_0.__exit__()
    str_0 = "]I\t`{"
    timer_2 = module_0.Timer(text=str_0)
    timer_2.stop()


def test_case_4():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    none_type_0 = timer_0.__exit__()


def test_case_5():
    str_0 = "]I\t`{"
    none_type_0 = None
    timer_0 = module_0.Timer(str_0, initial_text=str_0, logger=none_type_0)
    timer_1 = module_0.Timer()
    none_type_1 = timer_0.start()
    float_0 = timer_0.stop()


def test_case_6():
    str_0 = "]I\t`{"
    none_type_0 = None
    timer_0 = module_0.Timer(str_0, initial_text=str_0, logger=none_type_0)
    timer_1 = module_0.Timer(initial_text=str_0)
    timer_1.start()


def test_case_7():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    var_0 = timer_1.__call__(timer_0)
    var_1 = timer_1.__call__(timer_1)
    timer_2 = module_0.Timer(initial_text=var_0, logger=var_1)
    timer_2.__enter__()


def test_case_8():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    float_0 = timer_0.stop()
    none_type_0 = None
    var_0 = timer_0.__call__(none_type_0)
    none_type_1 = None
    timer_2 = module_0.Timer(text=var_0, initial_text=timer_1)
    timer_3 = module_0.Timer(none_type_0, none_type_1, var_0, none_type_0)
    none_type_2 = timer_2.start()
    timer_2.__exit__()


def test_case_9():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    float_0 = timer_1.stop()
    var_0 = timer_0.__call__(timer_1)
    str_0 = "]I\t`{"
    none_type_0 = None
    timer_2 = module_0.Timer(str_0, initial_text=none_type_0)
    timer_3 = module_0.Timer(str_0, var_0, var_0)
    none_type_1 = timer_3.start()
    var_1 = timer_0.__call__(timer_1)
    var_1.__enter__()
