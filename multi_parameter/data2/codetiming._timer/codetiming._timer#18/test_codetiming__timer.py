# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import codetiming._timer as module_0
import dataclasses as module_1


def test_case_0():
    timer_0 = module_0.Timer()


def test_case_1():
    str_0 = ">^"
    timer_0 = module_0.Timer(str_0, str_0, str_0)
    none_type_0 = timer_0.start()
    timer_0.__enter__()


def test_case_2():
    bool_0 = True
    timer_0 = module_0.Timer(initial_text=bool_0)
    none_type_0 = timer_0.start()


def test_case_3():
    str_0 = ",4/O2#qx#^^["
    none_type_0 = None
    timer_0 = module_0.Timer(text=str_0, logger=none_type_0)
    timer_1 = timer_0.__enter__()
    var_0 = timer_1.__repr__()
    var_1 = var_0.__len__()
    var_0.__delitem__(var_0)


def test_case_4():
    str_0 = "U"
    timer_0 = module_0.Timer(str_0)
    none_type_0 = timer_0.start()
    float_0 = timer_0.stop()


def test_case_5():
    str_0 = "U"
    timer_0 = module_0.Timer(str_0)
    none_type_0 = timer_0.__eq__(str_0)
    timer_0.stop()


def test_case_6():
    str_0 = "U"
    timer_0 = module_0.Timer(text=str_0, initial_text=str_0)
    none_type_0 = timer_0.start()
    timer_1 = module_0.Timer(text=str_0)
    float_0 = timer_0.stop()
    none_type_1 = timer_0.start()
    timer_1.stop()


def test_case_7():
    bool_0 = False
    str_0 = "U"
    timer_0 = module_0.Timer(bool_0, str_0)
    none_type_0 = timer_0.start()
    var_0 = module_1.field(default_factory=none_type_0)
    float_0 = timer_0.stop()
    timer_0.__exit__()


def test_case_8():
    timer_error_0 = module_0.TimerError()
    str_0 = "U"
    timer_0 = module_0.Timer(text=timer_error_0, initial_text=str_0)
    none_type_0 = timer_0.start()
    timer_0.stop()


def test_case_9():
    str_0 = "U"
    timer_0 = module_0.Timer(str_0)
    none_type_0 = timer_0.start()
    timer_1 = module_0.Timer(text=timer_0)
    none_type_1 = timer_1.start()
    float_0 = timer_0.stop()
    none_type_2 = timer_0.start()
    dict_0 = {}
    float_arg_0 = module_0.FloatArg(**dict_0)
    float_1 = timer_1.stop()


def test_case_10():
    timer_error_0 = module_0.TimerError()
    str_0 = "U"
    none_type_0 = None
    timer_0 = module_0.Timer(text=str_0, logger=none_type_0)
    none_type_1 = timer_0.start()
    var_0 = module_1.field(init=timer_0, hash=none_type_0)
    float_0 = timer_0.stop()
    none_type_2 = timer_0.start()
    dict_0 = {}
    module_0.FloatArg(*var_0, **dict_0)


def test_case_11():
    bool_0 = True
    str_0 = "U"
    timer_0 = module_0.Timer(str_0, initial_text=bool_0)
    none_type_0 = timer_0.start()
    none_type_1 = None
    timer_1 = module_0.Timer(text=none_type_1, initial_text=bool_0)
    none_type_2 = timer_1.start()
    var_0 = module_1.field(default=none_type_2, init=str_0, compare=bool_0)
    timer_1.stop()
