# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import codetiming._timer as module_0
import dataclasses as module_1


def test_case_0():
    timer_0 = module_0.Timer()


def test_case_1():
    str_0 = "d/!"
    timer_0 = module_0.Timer(text=str_0)
    var_0 = timer_0.__call__(timer_0)
    timer_1 = timer_0.__enter__()
    timer_1.__enter__()


def test_case_2():
    timer_0 = module_0.Timer()
    var_0 = timer_0.__repr__()
    timer_1 = timer_0.__enter__()


def test_case_3():
    str_0 = "d/!"
    timer_0 = module_0.Timer(text=str_0)
    timer_0.stop()


def test_case_4():
    dict_0 = {}
    float_arg_0 = module_0.FloatArg(**dict_0)
    timer_0 = module_0.Timer(initial_text=float_arg_0, logger=dict_0)
    timer_1 = timer_0.__enter__()
    none_type_0 = timer_0.__exit__()


def test_case_5():
    str_0 = "d/!"
    timer_0 = module_0.Timer(text=str_0)
    timer_1 = module_0.Timer(initial_text=timer_0)
    timer_2 = timer_0.__enter__()
    none_type_0 = timer_0.__exit__()


def test_case_6():
    str_0 = "d/!"
    timer_0 = module_0.Timer(initial_text=str_0)
    timer_1 = timer_0.__enter__()
    var_0 = timer_1.__repr__()
    var_0.__exit__()


def test_case_7():
    str_0 = "d/!"
    timer_0 = module_0.Timer(text=str_0)
    var_0 = timer_0.__call__(timer_0)
    timer_1 = module_0.Timer(text=str_0, initial_text=var_0)
    timer_2 = timer_0.__enter__()
    timer_3 = timer_1.__enter__()
    timer_3.__enter__()


def test_case_8():
    none_type_0 = None
    var_0 = module_1.dataclass(
        init=none_type_0, eq=none_type_0, frozen=none_type_0, slots=none_type_0
    )
    timer_0 = var_0.__eq__(var_0)
    timer_1 = module_0.Timer(text=var_0)
    timer_2 = module_0.Timer(initial_text=timer_1)
    timer_3 = timer_1.__enter__()
    var_1 = timer_3.__repr__()
    timer_1.__exit__()


def test_case_9():
    str_0 = "d"
    none_type_0 = None
    var_0 = module_1.dataclass(
        init=str_0, eq=str_0, frozen=none_type_0, slots=none_type_0
    )
    timer_0 = module_0.Timer(logger=var_0)
    timer_1 = module_0.Timer(text=str_0)
    none_type_1 = timer_1.start()
    timer_2 = module_0.Timer(timer_1)
    timer_3 = timer_2.__enter__()
    var_1 = timer_2.__repr__()
    timer_2.__exit__()


def test_case_10():
    timer_0 = module_0.Timer()
    var_0 = timer_0.__eq__(timer_0)
    none_type_0 = None
    var_1 = timer_0.__call__(none_type_0)
    timer_1 = timer_0.__enter__()
    none_type_1 = timer_0.__exit__()
    timer_2 = module_0.Timer(initial_text=var_1, logger=timer_1)
    none_type_2 = timer_0.start()
    var_2 = var_0.__eq__(timer_0)
    timer_3 = module_0.Timer(text=none_type_2, logger=var_1)
    timer_4 = module_0.Timer(var_2, none_type_1, var_2)
    timer_5 = timer_4.__enter__()
    var_3 = var_1.__repr__()
    var_4 = var_0.__repr__()
    var_5 = var_3.__repr__()
