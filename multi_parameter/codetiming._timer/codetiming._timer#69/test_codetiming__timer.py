# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import codetiming._timer as module_0


def test_case_0():
    timer_error_0 = module_0.TimerError()


def test_case_1():
    timer_error_0 = module_0.TimerError()
    str_0 = "\tST38tm8@\tj\x0c"
    timer_0 = module_0.Timer(str_0, initial_text=str_0)
    none_type_0 = timer_0.start()
    timer_0.__enter__()


def test_case_2():
    bool_0 = False
    timer_0 = module_0.Timer(bool_0, initial_text=bool_0)
    timer_1 = timer_0.__enter__()
    float_0 = timer_0.stop()


def test_case_3():
    bool_0 = False
    timer_0 = module_0.Timer(bool_0, initial_text=bool_0)
    timer_1 = timer_0.__enter__()
    float_0 = timer_0.stop()
    timer_0.stop()


def test_case_4():
    float_arg_0 = module_0.FloatArg()
    list_0 = [float_arg_0]
    dict_0 = {}
    timer_error_0 = module_0.TimerError(*list_0, **dict_0)
    timer_0 = module_0.Timer(timer_error_0)
    timer_0.__exit__(*list_0)


def test_case_5():
    none_type_0 = None
    bool_0 = True
    timer_0 = module_0.Timer(none_type_0, initial_text=bool_0)
    var_0 = timer_0.__repr__()
    timer_1 = timer_0.__enter__()
    float_0 = timer_0.stop()


def test_case_6():
    bool_0 = True
    timer_0 = module_0.Timer(bool_0, initial_text=bool_0)
    none_type_0 = timer_0.start()
    float_0 = timer_0.stop()
    var_0 = timer_0.__call__(none_type_0)
    none_type_1 = None
    var_1 = timer_0.__repr__()
    timer_1 = module_0.Timer(text=none_type_1)
    var_0.__call__(var_1)


def test_case_7():
    none_type_0 = None
    timer_0 = module_0.Timer(text=none_type_0, logger=none_type_0)
    none_type_1 = timer_0.start()
    float_0 = timer_0.stop()
    var_0 = timer_0.__call__(none_type_1)
    none_type_2 = None
    var_1 = timer_0.__repr__()
    timer_1 = module_0.Timer(var_0, logger=none_type_2)
    var_2 = timer_0.__call__(none_type_2)
    module_0.FloatArg(*var_2)


def test_case_8():
    none_type_0 = None
    bool_0 = True
    timer_0 = module_0.Timer(none_type_0, initial_text=bool_0)
    none_type_1 = timer_0.start()
    float_0 = timer_0.stop()
    var_0 = timer_0.__call__(none_type_0)
    timer_1 = module_0.Timer(text=var_0, initial_text=bool_0)
    var_1 = var_0.__repr__()
    timer_2 = timer_1.__enter__()
    timer_2.stop()
