# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import codetiming._timer as module_0
import dataclasses as module_1


def test_case_0():
    timer_error_0 = module_0.TimerError()


def test_case_1():
    timer_error_0 = module_0.TimerError()
    timer_0 = module_0.Timer(timer_error_0, initial_text=timer_error_0)
    timer_1 = timer_0.__enter__()
    timer_1.__enter__()


def test_case_2():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    float_0 = timer_1.stop()


def test_case_3():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    float_0 = timer_1.stop()
    timer_1.stop()


def test_case_4():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    none_type_0 = timer_1.__exit__()
    timer_1.stop()


def test_case_5():
    set_0 = set()
    timer_0 = module_0.Timer(initial_text=set_0, logger=set_0)
    float_0 = timer_0.__repr__()
    var_0 = timer_0.__repr__()
    none_type_0 = timer_0.start()
    timer_0.start()


def test_case_6():
    float_arg_0 = module_0.FloatArg()
    none_type_0 = None
    timer_0 = module_0.Timer(logger=none_type_0)
    timer_error_0 = module_0.TimerError()
    timer_1 = module_0.Timer(initial_text=float_arg_0, logger=none_type_0)
    timer_2 = module_0.Timer(none_type_0, none_type_0, logger=none_type_0)
    timer_3 = timer_1.__enter__()
    none_type_1 = timer_0.start()
    float_0 = timer_3.stop()
    var_0 = timer_0.__eq__(timer_2)
    float_arg_1 = module_0.FloatArg()
    timer_2.stop()


def test_case_7():
    str_0 = "7r?I.'z=X++,_@&ZK9cU"
    var_0 = module_1.field(hash=str_0, kw_only=str_0)
    timer_0 = module_0.Timer(initial_text=var_0)
    none_type_0 = timer_0.start()


def test_case_8():
    timer_error_0 = module_0.TimerError()
    timer_0 = module_0.Timer(timer_error_0, initial_text=timer_error_0)
    timer_1 = timer_0.__enter__()
    float_0 = timer_0.stop()
    timer_2 = timer_0.__enter__()


def test_case_9():
    timer_error_0 = module_0.TimerError()
    str_0 = "\r/S%k$\\"
    timer_0 = module_0.Timer(initial_text=str_0)
    timer_1 = timer_0.__enter__()
    var_0 = timer_0.__eq__(timer_0)
    var_0.__contains__(timer_0)


def test_case_10():
    timer_error_0 = module_0.TimerError()
    timer_0 = module_0.Timer(timer_error_0, initial_text=timer_error_0)
    timer_1 = module_0.Timer(text=timer_0)
    timer_2 = timer_1.__enter__()
    float_0 = timer_2.stop()
    none_type_0 = timer_2.start()
    none_type_1 = timer_1.__exit__()
    var_0 = timer_2.__repr__()
    var_0.get(timer_1)
