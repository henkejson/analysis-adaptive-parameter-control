# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import codetiming._timer as module_0


def test_case_0():
    timer_error_0 = module_0.TimerError()


def test_case_1():
    str_0 = "jv[<y-<`xT!;c<wL`r]"
    timer_0 = module_0.Timer(initial_text=str_0)
    none_type_0 = timer_0.start()
    timer_1 = module_0.Timer()
    float_0 = timer_0.stop()
    var_0 = timer_0.__repr__()
    timer_2 = timer_0.__enter__()
    timer_0.__enter__()


def test_case_2():
    none_type_0 = None
    timer_0 = module_0.Timer(none_type_0)
    timer_1 = timer_0.__enter__()
    float_0 = timer_0.stop()


def test_case_3():
    float_arg_0 = module_0.FloatArg()
    dict_0 = {}
    timer_0 = module_0.Timer(text=dict_0, logger=dict_0)
    list_0 = []
    timer_0.__exit__(*list_0)


def test_case_4():
    none_type_0 = None
    float_arg_0 = module_0.FloatArg()
    timer_0 = module_0.Timer(none_type_0)
    timer_1 = module_0.Timer(initial_text=float_arg_0)
    timer_2 = timer_1.__enter__()
    timer_0.stop()


def test_case_5():
    dict_0 = {}
    timer_0 = module_0.Timer(text=dict_0, logger=dict_0)
    timer_1 = timer_0.__enter__()
    list_0 = []
    none_type_0 = None
    list_0.__getitem__(none_type_0)


def test_case_6():
    complex_0 = 4975.95 + 1535.7j
    float_arg_0 = module_0.FloatArg()
    timer_0 = module_0.Timer(complex_0)
    timer_1 = timer_0.__enter__()
    timer_2 = module_0.Timer(initial_text=float_arg_0)
    float_0 = timer_0.stop()
    float_arg_1 = module_0.FloatArg()
    var_0 = timer_0.__repr__()
    timer_3 = module_0.Timer(logger=var_0)


def test_case_7():
    complex_0 = 4975.95 + 1535.7j
    float_arg_0 = module_0.FloatArg()
    timer_0 = module_0.Timer(complex_0, initial_text=float_arg_0, logger=float_arg_0)
    timer_0.__enter__()


def test_case_8():
    none_type_0 = None
    float_arg_0 = module_0.FloatArg()
    timer_0 = module_0.Timer(initial_text=none_type_0, logger=none_type_0)
    timer_1 = timer_0.__enter__()
    timer_2 = module_0.Timer(none_type_0)
    float_0 = timer_1.stop()
    timer_3 = timer_1.__enter__()
    var_0 = timer_3.__repr__()
    float_1 = timer_1.stop()


def test_case_9():
    float_arg_0 = module_0.FloatArg()
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    timer_2 = module_0.Timer(text=timer_0)
    float_0 = timer_0.stop()
    timer_3 = timer_2.__enter__()
    float_1 = timer_3.stop()
    timer_3.__exit__()
