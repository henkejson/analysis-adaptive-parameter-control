# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import flutils.namedtupleutils as module_0
import collections as module_1


def test_case_0():
    bool_0 = False
    module_0.to_namedtuple(bool_0)


def test_case_1():
    bool_0 = False
    list_0 = [bool_0, bool_0]
    var_0 = module_0.to_namedtuple(list_0)


def test_case_2():
    str_0 = "FIFO"
    str_1 = "Xg aP1Jk!e m"
    dict_0 = {str_0: str_0, str_0: str_0, str_1: str_0}
    var_0 = module_0.to_namedtuple(dict_0)
    var_1 = module_0.to_namedtuple(var_0)


def test_case_3():
    bytes_0 = b""
    module_0.to_namedtuple(bytes_0)


def test_case_4():
    list_0 = []
    var_0 = module_0.to_namedtuple(list_0)


def test_case_5():
    str_0 = 'N("TJ>kCF9Hc\\2'
    dict_0 = {str_0: str_0, str_0: str_0}
    ordered_dict_0 = module_1.OrderedDict(**dict_0)
    var_0 = module_0.to_namedtuple(ordered_dict_0)
    none_type_0 = None
    module_0.to_namedtuple(none_type_0)


def test_case_6():
    bytes_0 = b"L\xcb7h-\xf0\xb1\x87\xbc9L\xcf~\xdb\x9c"
    bool_0 = True
    list_0 = []
    tuple_0 = (bytes_0, bool_0, list_0, bytes_0)
    var_0 = module_0.to_namedtuple(tuple_0)


def test_case_7():
    float_0 = 1018.07
    list_0 = [float_0, float_0, float_0, float_0]
    bool_0 = True
    bytes_0 = b"\r\x90$\xa6\xad\xdd7p\xa0\xcf\x07\x13\x7fp4"
    dict_0 = {bytes_0: bool_0, float_0: float_0}
    tuple_0 = (bool_0, bytes_0, dict_0, float_0)
    tuple_1 = (float_0, list_0, float_0, tuple_0)
    module_0.to_namedtuple(tuple_1)


def test_case_8():
    dict_0 = {}
    var_0 = module_0.to_namedtuple(dict_0)
    var_1 = module_0.to_namedtuple(var_0)
    ordered_dict_0 = module_1.OrderedDict(**dict_0)
    var_2 = module_0.to_namedtuple(var_1)
    var_3 = module_0.to_namedtuple(ordered_dict_0)
    var_4 = module_0.to_namedtuple(var_1)
    var_5 = module_0.to_namedtuple(dict_0)
    var_6 = module_0.to_namedtuple(var_0)
    var_7 = module_0.to_namedtuple(var_3)
    var_8 = module_0.to_namedtuple(ordered_dict_0)
    var_9 = module_0.to_namedtuple(var_8)
    var_10 = module_0.to_namedtuple(var_5)
    var_11 = module_0.to_namedtuple(ordered_dict_0)
    var_12 = module_0.to_namedtuple(var_8)


def test_case_9():
    tuple_0 = ()
    var_0 = module_0.to_namedtuple(tuple_0)


def test_case_10():
    tuple_0 = ()
    var_0 = module_0.to_namedtuple(tuple_0)
    var_1 = module_0.to_namedtuple(tuple_0)
    var_2 = module_0.to_namedtuple(tuple_0)
    var_3 = module_0.to_namedtuple(tuple_0)
    var_4 = module_0.to_namedtuple(tuple_0)
    var_5 = module_0.to_namedtuple(tuple_0)
    var_6 = module_0.to_namedtuple(var_3)
    var_7 = module_0.to_namedtuple(var_5)
    var_8 = module_0.to_namedtuple(var_2)
    var_9 = module_0.to_namedtuple(tuple_0)
    var_10 = module_0.to_namedtuple(var_3)
    var_11 = module_0.to_namedtuple(var_2)
    var_12 = module_0.to_namedtuple(var_1)
    var_13 = module_0.to_namedtuple(var_11)
    dict_0 = {var_12: var_13, var_13: var_6}
    var_14 = module_0.to_namedtuple(dict_0)
    var_15 = module_0.to_namedtuple(var_7)
    var_16 = module_0.to_namedtuple(var_1)
    var_17 = module_0.to_namedtuple(var_6)
    list_0 = [tuple_0, var_2, var_5]
    var_18 = module_0.to_namedtuple(var_14)
    var_19 = module_0.to_namedtuple(var_2)
    var_20 = module_0.to_namedtuple(list_0)
    var_21 = module_0.to_namedtuple(tuple_0)
    var_22 = module_0.to_namedtuple(var_21)
    var_23 = module_0.to_namedtuple(tuple_0)
    var_24 = module_0.to_namedtuple(tuple_0)
    var_25 = module_0.to_namedtuple(var_16)
    var_26 = module_0.to_namedtuple(list_0)


def test_case_11():
    str_0 = "FIFO"
    set_0 = {str_0, str_0, str_0}
    list_0 = []
    str_1 = "7Y6_B$hKVr>b"
    str_2 = "y\x0b\r"
    var_0 = module_0.to_namedtuple(list_0)
    dict_0 = {str_1: set_0, str_0: str_0, str_2: set_0}
    ordered_dict_0 = module_1.OrderedDict(*list_0, **dict_0)
    tuple_0 = (str_0, set_0, ordered_dict_0)
    var_1 = module_0.to_namedtuple(tuple_0)
    var_2 = module_0.to_namedtuple(ordered_dict_0)
    var_3 = module_0.to_namedtuple(var_1)
