# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import sanic.config as module_0
import inspect as module_1


def test_case_0():
    config_0 = module_0.Config()


def test_case_1():
    config_0 = module_0.Config()
    config_1 = module_0.Config(converters=config_0)


def test_case_2():
    str_0 = "INSPECTOR_PORT"
    dict_0 = {str_0: str_0}
    config_0 = module_0.Config(dict_0, str_0)


def test_case_3():
    str_0 = "_"
    bytes_0 = b"\xe0\xec\x7f)\x80\xc7vSg\xc0\xa2."
    config_0 = module_0.Config(env_prefix=str_0, keep_alive=bytes_0)
    config_1 = module_0.Config()
    str_1 = "B*F%2\\'[h_Ju\x0cB9XI"
    none_type_0 = config_1.__setitem__(str_1, bytes_0)


def test_case_4():
    str_0 = "mUP(%oc"
    none_type_0 = None
    config_0 = module_0.Config(none_type_0, none_type_0)
    config_0.__getattr__(str_0)


def test_case_5():
    config_0 = module_0.Config()
    config_0.__getattr__(config_0)


def test_case_6():
    config_0 = module_0.Config()
    bool_0 = False
    config_0.update_config(bool_0)


def test_case_7():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)


def test_case_8():
    str_0 = "_"
    bytes_0 = b"\xe0\xec\x7f)\x80\xc7vSg\xc0\xa2."
    config_0 = module_0.Config(env_prefix=str_0, keep_alive=bytes_0)


def test_case_9():
    str_0 = "_"
    bytes_0 = b"\xe0\xec\x7f)\x80\xc7vSg\xc0\xa2."
    config_0 = module_0.Config(env_prefix=str_0, keep_alive=bytes_0)
    config_1 = module_0.Config()
    config_0.update_config(str_0)


def test_case_10():
    config_0 = module_0.Config()
    var_0 = config_0.load_environment_vars()
    none_type_0 = config_0.register_type(var_0)
    config_0.register_type(var_0)


def test_case_11():
    config_0 = module_0.Config()
    str_0 = "LOCAL_CERT_CREATOR"
    config_0.__setitem__(str_0, str_0)


def test_case_12():
    none_type_0 = None
    config_0 = module_0.Config(keep_alive=none_type_0)
    config_1 = module_0.Config()
    bool_0 = False
    none_type_1 = config_1.register_type(bool_0)
    str_0 = "_FALLBACK_ERROR_FORMAT"
    none_type_2 = config_1.__setattr__(str_0, bool_0)
    var_0 = module_1.getmembers(config_1)
    var_1 = config_1.update_config(config_1)
    str_1 = "FORWARDED_FORoHEADER"
    none_type_3 = config_1.__setitem__(str_1, config_1)
    config_1.update_config(var_1)
