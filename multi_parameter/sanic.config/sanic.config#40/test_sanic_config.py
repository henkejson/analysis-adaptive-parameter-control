# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import sanic.config as module_0
import inspect as module_1
import builtins as module_2
import sanic.errorpages as module_3
import sanic.http.http1 as module_4


def test_case_0():
    config_0 = module_0.Config()


def test_case_1():
    none_type_0 = None
    var_0 = module_1.isclass(none_type_0)
    var_1 = var_0.__repr__()
    config_0 = module_0.Config(var_0, keep_alive=var_1, converters=none_type_0)
    var_1.__bool__()


def test_case_2():
    none_type_0 = None
    config_0 = module_0.Config(env_prefix=none_type_0)
    config_1 = module_0.Config(none_type_0)
    int_0 = 6457
    config_1.update_config(int_0)


def test_case_3():
    config_0 = module_0.Config()
    str_0 = "{S?BQGZTWyasU:6ueb"
    none_type_0 = config_0.register_type(str_0)
    config_0.__getattr__(str_0)


def test_case_4():
    config_0 = module_0.Config()
    str_0 = "{S?BQGZTWyasU:6ueb"
    none_type_0 = config_0.register_type(str_0)
    var_0 = module_2.object()
    var_0.respond(str_0)


def test_case_5():
    str_0 = "copied_from"
    config_0 = module_0.Config(env_prefix=str_0)
    module_3.check_error_format(str_0)


def test_case_6():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)


def test_case_7():
    config_0 = module_0.Config()
    str_0 = "KEEP_ALIVE"
    none_type_0 = config_0.register_type(str_0)
    none_type_1 = None
    none_type_2 = config_0.__setitem__(str_0, none_type_0)
    bytes_0 = b"I'm a teapot"
    none_type_3 = config_0.register_type(bytes_0)
    config_0.update_config(none_type_1)


def test_case_8():
    none_type_0 = None
    config_0 = module_0.Config(env_prefix=none_type_0)
    none_type_1 = config_0.update()
    var_0 = config_0.update_config(config_0)
    config_1 = module_0.Config(config_0)
    int_0 = 6457
    config_1.update_config(int_0)


def test_case_9():
    config_0 = module_0.Config()
    str_0 = "Gr#gT\r+c$\x0bKN1N{"
    none_type_0 = config_0.__setattr__(str_0, config_0)
    str_1 = "{S?BQGZTWyasU:6ueb"
    var_0 = config_0.update_config(config_0)
    config_0.update_config(str_1)


def test_case_10():
    config_0 = module_0.Config()
    none_type_0 = config_0.register_type(config_0)
    config_0.__getattr__(config_0)


def test_case_11():
    config_0 = module_0.Config()
    none_type_0 = None
    config_0.update_config(none_type_0)


def test_case_12():
    config_0 = module_0.Config()
    str_0 = "{S?BQGZTWyasU:6ueb"
    none_type_0 = config_0.register_type(str_0)
    config_0.register_type(str_0)


def test_case_13():
    float_0 = 439.72851
    module_0.Config(converters=float_0)


def test_case_14():
    none_type_0 = None
    config_0 = module_0.Config(env_prefix=none_type_0)
    none_type_1 = config_0.update()
    config_1 = module_0.Config(converters=config_0)
    module_4.Http(config_0)


def test_case_15():
    none_type_0 = None
    config_0 = module_0.Config(env_prefix=none_type_0)
    none_type_1 = config_0.update()
    none_type_2 = config_0.register_type(config_0)
    var_0 = config_0.update_config(config_0)
    config_1 = module_0.Config(none_type_0)
    str_0 = "LOCAL_CERT_CREATOR"
    config_0.__setitem__(str_0, config_1)
