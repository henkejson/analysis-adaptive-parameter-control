# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import sanic.config as module_0
import inspect as module_1


def test_case_0():
    config_0 = module_0.Config()


def test_case_1():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)
    config_1 = module_0.Config(env_prefix=var_0, keep_alive=var_0, converters=config_0)
    var_0.register_type(var_0)


def test_case_2():
    bool_0 = False
    config_0 = module_0.Config(keep_alive=bool_0)


def test_case_3():
    str_0 = "logging.Formatter"
    config_0 = module_0.Config(env_prefix=str_0)


def test_case_4():
    str_0 = "passthru"
    dict_0 = {str_0: str_0, str_0: str_0}
    config_0 = module_0.Config(dict_0)
    none_type_0 = config_0.update()


def test_case_5():
    config_0 = module_0.Config()
    none_type_0 = None
    config_0.__getattr__(none_type_0)


def test_case_6():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)


def test_case_7():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)
    config_0.update_config(var_0)


def test_case_8():
    str_0 = "logging.Formatter"
    config_0 = module_0.Config()
    var_0 = config_0.update_config(str_0)
    config_1 = module_0.Config(env_prefix=var_0)
    none_type_0 = config_0.register_type(config_1)


def test_case_9():
    str_0 = "FALLBACK_ERROR_FORMAT"
    config_0 = module_0.Config()
    config_0.__setitem__(str_0, str_0)


def test_case_10():
    str_0 = "logging.Formatter"
    config_0 = module_0.Config()
    var_0 = config_0.update_config(str_0)
    config_1 = module_0.Config(env_prefix=var_0)
    bool_0 = True
    tuple_0 = (bool_0, config_1, config_0, var_0)
    config_1.__getattr__(tuple_0)


def test_case_11():
    str_0 = "logging.Formatter"
    config_0 = module_0.Config()
    var_0 = config_0.update_config(str_0)
    config_1 = module_0.Config(env_prefix=var_0)
    none_type_0 = config_1.register_type(var_0)
    none_type_1 = config_0.update()
    var_1 = module_1.getmembers(config_0)
    config_1.register_type(var_0)


def test_case_12():
    str_0 = "FALLBACK_ERROR_FORMAT"
    config_0 = module_0.Config()
    none_type_0 = None
    none_type_1 = config_0.__setitem__(str_0, none_type_0)
    config_0.__setitem__(str_0, str_0)


def test_case_13():
    str_0 = "FALLBACK_ERROR_FORMAT"
    config_0 = module_0.Config()
    none_type_0 = None
    none_type_1 = config_0.__setitem__(str_0, none_type_0)
    config_1 = module_0.Config(env_prefix=none_type_0)
    var_0 = module_1.getmembers(config_0)
    config_0.update_config(none_type_0)


def test_case_14():
    config_0 = module_0.Config()
    none_type_0 = None
    str_0 = "LOCAL_CERT_CREATOR"
    config_0.__setitem__(str_0, none_type_0)
