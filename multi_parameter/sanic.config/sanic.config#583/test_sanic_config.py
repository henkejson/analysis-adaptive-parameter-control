# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import sanic.config as module_0
import inspect as module_1


def test_case_0():
    config_0 = module_0.Config()


def test_case_1():
    config_0 = module_0.Config()
    str_0 = "Z2\x0c&P9aATd"
    config_1 = module_0.Config(converters=config_0)
    none_type_0 = None
    none_type_1 = config_0.__setattr__(str_0, none_type_0)
    str_1 = " \" ltM}z<\\~'U"
    none_type_2 = None
    none_type_3 = config_0.__setattr__(str_1, none_type_2)
    var_0 = config_0.load_environment_vars()
    list_0 = [config_0, config_0, config_0, config_0]
    none_type_4 = config_0.register_type(list_0)
    config_0.__getattr__(none_type_1)


def test_case_2():
    str_0 = "P"
    config_0 = module_0.Config(env_prefix=str_0, keep_alive=str_0)


def test_case_3():
    str_0 = ""
    str_1 = "ALHv]Sv7<rU`s)9<m'["
    str_2 = "4fG*Y{[Ry9XJ!(a-y"
    dict_0 = {str_0: str_0, str_1: str_1, str_2: str_1}
    config_0 = module_0.Config(dict_0, keep_alive=dict_0)


def test_case_4():
    str_0 = "P"
    config_0 = module_0.Config(env_prefix=str_0, keep_alive=str_0)
    none_type_0 = config_0.register_type(config_0)
    config_0.__getattr__(none_type_0)


def test_case_5():
    none_type_0 = None
    config_0 = module_0.Config()
    config_0.update_config(none_type_0)


def test_case_6():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)


def test_case_7():
    config_0 = module_0.Config()
    none_type_0 = module_1.isclass(config_0)
    none_type_1 = config_0.register_type(config_0)


def test_case_8():
    config_0 = module_0.Config()
    config_0.__getattr__(config_0)


def test_case_9():
    config_0 = module_0.Config()
    var_0 = module_1.isclass(config_0)
    none_type_0 = None
    config_1 = module_0.Config(var_0, none_type_0, var_0)


def test_case_10():
    config_0 = module_0.Config()
    none_type_0 = None
    str_0 = " \" ltM}z<\\~'U"
    none_type_1 = config_0.__setattr__(str_0, none_type_0)
    var_0 = config_0.update_config(config_0)
    var_1 = config_0.load_environment_vars()
    none_type_2 = config_0.register_type(var_1)
    config_0.register_type(var_1)


def test_case_11():
    str_0 = "P"
    config_0 = module_0.Config(env_prefix=str_0, keep_alive=str_0)
    config_0.update_config(str_0)


def test_case_12():
    config_0 = module_0.Config()
    str_0 = "Z2\x0c&P9aATd"
    none_type_0 = config_0.update(**config_0)
    bool_0 = True
    none_type_1 = config_0.register_type(bool_0)
    none_type_2 = None
    none_type_3 = config_0.__setattr__(str_0, none_type_2)
    str_1 = " \" ltM}z<\\~'U"
    none_type_4 = None
    none_type_5 = config_0.__setattr__(str_1, none_type_4)
    var_0 = config_0.update_config(config_0)
    var_1 = config_0.load_environment_vars()
    list_0 = [config_0, config_0, config_0, config_0]
    str_2 = "LOCAL_CERT_CREATOR"
    config_0.__setitem__(str_2, list_0)


def test_case_13():
    str_0 = "Stream ID is only a property of a HTTP/3 request"
    str_1 = "_FALLBACK_ERROR_FORMAT"
    str_2 = "+ec)\tF++wU0,[|"
    dict_0 = {str_0: str_0, str_1: str_2, str_0: str_0}
    module_0.Config(dict_0)


def test_case_14():
    str_0 = "P"
    config_0 = module_0.Config(env_prefix=str_0, keep_alive=str_0)
    none_type_0 = config_0.update()
    none_type_1 = config_0.register_type(config_0)
    config_0.load_environment_vars(str_0)


def test_case_15():
    str_0 = "_"
    config_0 = module_0.Config(env_prefix=str_0, keep_alive=str_0)
    var_0 = config_0.update_config(config_0)
    var_1 = config_0.load_environment_vars()
    none_type_0 = config_0.register_type(config_0)
    config_0.update_config(var_0)
