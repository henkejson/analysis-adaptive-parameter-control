# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import sanic.config as module_0


def test_case_0():
    config_0 = module_0.Config()


def test_case_1():
    str_0 = "!Bn,8E2[4m"
    bytes_0 = b"\x00\xa1L\xa6\x9d<\x83\xb4@\xc1_\xd6\x98"
    config_0 = module_0.Config(env_prefix=str_0, converters=bytes_0)
    none_type_0 = config_0.register_type(config_0)


def test_case_2():
    none_type_0 = None
    config_0 = module_0.Config(
        none_type_0, keep_alive=none_type_0, converters=none_type_0
    )
    config_1 = module_0.Config(keep_alive=config_0)
    var_0 = config_1.update_config(config_1)


def test_case_3():
    none_type_0 = None
    config_0 = module_0.Config(none_type_0, none_type_0)
    var_0 = config_0.load_environment_vars()


def test_case_4():
    bool_0 = True
    none_type_0 = None
    config_0 = module_0.Config()
    config_1 = module_0.Config(config_0)
    config_2 = module_0.Config(env_prefix=none_type_0, keep_alive=bool_0)
    none_type_1 = config_1.register_type(bool_0)
    var_0 = config_2.update_config(config_0)
    config_2.update_config(bool_0)


def test_case_5():
    none_type_0 = None
    config_0 = module_0.Config(
        none_type_0, keep_alive=none_type_0, converters=none_type_0
    )
    none_type_1 = config_0.register_type(none_type_0)
    config_0.__getattr__(none_type_1)


def test_case_6():
    config_0 = module_0.Config()
    none_type_0 = config_0.update(**config_0)


def test_case_7():
    bytes_0 = b"Insufficient S\x84orage"
    none_type_0 = None
    config_0 = module_0.Config()
    none_type_1 = config_0.register_type(none_type_0)
    var_0 = config_0.update_config(config_0)
    config_0.update_config(bytes_0)


def test_case_8():
    bytes_0 = b"Insufficient Storage"
    config_0 = module_0.Config(keep_alive=bytes_0)
    str_0 = "!Bn,8E2[4m"
    module_0.Config(env_prefix=str_0, converters=bytes_0)


def test_case_9():
    none_type_0 = None
    none_type_1 = None
    config_0 = module_0.Config(
        none_type_1, keep_alive=none_type_1, converters=none_type_1
    )
    none_type_2 = config_0.register_type(none_type_0)


def test_case_10():
    none_type_0 = None
    config_0 = module_0.Config(
        none_type_0, keep_alive=none_type_0, converters=none_type_0
    )
    config_0.update_config(none_type_0)


def test_case_11():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)


def test_case_12():
    str_0 = "&P%Jh8*6(hUV1I:lx"
    config_0 = module_0.Config()
    none_type_0 = config_0.__setitem__(str_0, str_0)
    none_type_1 = None
    config_1 = module_0.Config(
        none_type_1, keep_alive=none_type_1, converters=none_type_1
    )
    config_1.update_config(none_type_1)


def test_case_13():
    none_type_0 = None
    config_0 = module_0.Config(
        none_type_0, keep_alive=none_type_0, converters=none_type_0
    )
    config_0.__getattr__(config_0)


def test_case_14():
    none_type_0 = None
    config_0 = module_0.Config(keep_alive=none_type_0, converters=none_type_0)
    config_1 = config_0.update_config(config_0)
    int_0 = 1224
    none_type_1 = config_0.register_type(int_0)
    str_0 = "3|*Pz\x0cQb_d}&y%#"
    str_1 = "_FALLBACK_ERROR_FORMAT"
    dict_0 = {str_0: int_0, str_1: none_type_1}
    module_0.Config(dict_0)


def test_case_15():
    none_type_0 = None
    none_type_1 = None
    config_0 = module_0.Config(
        none_type_0, keep_alive=none_type_0, converters=none_type_1
    )
    none_type_2 = config_0.register_type(none_type_0)
    str_0 = "LOCAL_CERT_CREATOR"
    config_0.__setattr__(str_0, config_0)
