# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pymonet.immutable_list as module_0


def test_case_0():
    int_0 = -116
    immutable_list_0 = module_0.ImmutableList(is_empty=int_0)
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    bool_0 = immutable_list_1.__eq__(immutable_list_0)
    immutable_list_1.find(bool_0)


def test_case_1():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)


def test_case_2():
    none_type_0 = None
    complex_0 = -5731.402758 - 1976.36j
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.unshift(complex_0)
    immutable_list_1.__add__(none_type_0)


def test_case_3():
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList(tail=none_type_0)
    str_0 = immutable_list_0.__str__()
    var_0 = immutable_list_0.__len__()


def test_case_4():
    int_0 = -1764
    immutable_list_0 = module_0.ImmutableList(is_empty=int_0)
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    var_0 = immutable_list_1.__len__()
    immutable_list_1.find(var_0)


def test_case_5():
    int_0 = -1764
    immutable_list_0 = module_0.ImmutableList(is_empty=int_0)
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    var_0 = immutable_list_1.to_list()
    var_1 = immutable_list_0.reduce(int_0, int_0)
    immutable_list_1.find(var_1)


def test_case_6():
    immutable_list_0 = module_0.ImmutableList()
    none_type_0 = None
    immutable_list_1 = module_0.ImmutableList(none_type_0)
    immutable_list_1.map(immutable_list_0)


def test_case_7():
    bool_0 = True
    list_0 = []
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList(none_type_0)
    immutable_list_1 = immutable_list_0.unshift(list_0)
    immutable_list_1.map(bool_0)


def test_case_8():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_0.filter(immutable_list_0)


def test_case_9():
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList(tail=none_type_0, is_empty=none_type_0)
    immutable_list_1 = immutable_list_0.__add__(immutable_list_0)
    var_0 = immutable_list_0.to_list()
    var_1 = immutable_list_1.find(var_0)
    immutable_list_1.filter(var_1)


def test_case_10():
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList(tail=none_type_0)
    str_0 = immutable_list_0.find(immutable_list_0)
    immutable_list_0.filter(str_0)


def test_case_11():
    int_0 = -1777
    immutable_list_0 = module_0.ImmutableList(is_empty=int_0)
    immutable_list_1 = immutable_list_0.unshift(int_0)
    immutable_list_1.find(immutable_list_1)


def test_case_12():
    int_0 = -1764
    immutable_list_0 = module_0.ImmutableList(is_empty=int_0)
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    var_0 = immutable_list_0.reduce(int_0, int_0)
    immutable_list_1.find(var_0)


def test_case_13():
    bool_0 = False
    bool_1 = False
    immutable_list_0 = module_0.ImmutableList(bool_0, is_empty=bool_0)
    immutable_list_1 = module_0.ImmutableList()
    none_type_0 = None
    immutable_list_2 = module_0.ImmutableList(tail=none_type_0, is_empty=bool_1)
    bool_2 = immutable_list_1.__eq__(bool_0)
    immutable_list_0.reduce(none_type_0, bool_2)


def test_case_14():
    immutable_list_0 = module_0.ImmutableList()


def test_case_15():
    bool_0 = False
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList(bool_0, is_empty=none_type_0)
    var_0 = immutable_list_0.append(immutable_list_0)
    immutable_list_0.find(var_0)


def test_case_16():
    int_0 = -1764
    immutable_list_0 = module_0.ImmutableList(is_empty=int_0)
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    immutable_list_2 = immutable_list_1.__add__(immutable_list_0)
    bool_0 = immutable_list_1.__eq__(immutable_list_1)
    immutable_list_1.find(immutable_list_2)


def test_case_17():
    int_0 = -1764
    immutable_list_0 = module_0.ImmutableList(is_empty=int_0)
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    var_0 = immutable_list_1.__add__(immutable_list_0)
    immutable_list_1.find(var_0)


def test_case_18():
    int_0 = -1806
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList(int_0, is_empty=none_type_0)
    var_0 = immutable_list_0.__len__()
    immutable_list_0.find(var_0)


def test_case_19():
    bool_0 = False
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList(bool_0, is_empty=none_type_0)
    immutable_list_0.find(immutable_list_0)


def test_case_20():
    none_type_0 = None
    bool_0 = False
    immutable_list_0 = module_0.ImmutableList(none_type_0)
    immutable_list_1 = immutable_list_0.__add__(immutable_list_0)
    bool_1 = immutable_list_1.__eq__(immutable_list_0)
    var_0 = immutable_list_0.unshift(bool_0)
    var_0.filter(var_0)


def test_case_21():
    bool_0 = False
    bool_1 = False
    immutable_list_0 = module_0.ImmutableList(bool_0, is_empty=bool_0)
    immutable_list_1 = module_0.ImmutableList(tail=bool_1, is_empty=bool_0)
    immutable_list_2 = immutable_list_0.__add__(immutable_list_1)
    bool_2 = immutable_list_0.__eq__(immutable_list_1)
    none_type_0 = None
    immutable_list_2.reduce(bool_2, none_type_0)
