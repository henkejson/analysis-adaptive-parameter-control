# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pymonet.immutable_list as module_0


def test_case_0():
    immutable_list_0 = module_0.ImmutableList()
    bool_0 = immutable_list_0.__eq__(immutable_list_0)


def test_case_1():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    bool_0 = immutable_list_0.__eq__(immutable_list_1)
    immutable_list_1.find(immutable_list_1)


def test_case_2():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    immutable_list_1.find(immutable_list_1)


def test_case_3():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = module_0.ImmutableList(immutable_list_0, immutable_list_0)
    immutable_list_2 = immutable_list_0.__add__(immutable_list_1)
    str_0 = immutable_list_0.__str__()
    none_type_0 = None
    immutable_list_3 = module_0.ImmutableList(none_type_0, none_type_0)
    immutable_list_4 = module_0.ImmutableList(is_empty=immutable_list_0)
    var_0 = immutable_list_0.reduce(immutable_list_4, immutable_list_3)
    var_1 = immutable_list_0.find(var_0)
    var_2 = immutable_list_4.to_list()
    bool_0 = var_2.__eq__(var_2)
    str_1 = var_2.__str__()
    var_3 = immutable_list_2.__len__()
    immutable_list_5 = var_3.__add__(var_1)
    immutable_list_3.__add__(var_3)


def test_case_4():
    str_0 = "TYF:w\x0bl8[uK(V\t"
    immutable_list_0 = module_0.ImmutableList(is_empty=str_0)
    var_0 = immutable_list_0.__len__()


def test_case_5():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    bool_0 = immutable_list_1.__len__()
    immutable_list_1.find(immutable_list_1)


def test_case_6():
    immutable_list_0 = module_0.ImmutableList()
    var_0 = immutable_list_0.to_list()


def test_case_7():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    str_0 = immutable_list_1.__str__()
    immutable_list_1.find(immutable_list_1)


def test_case_8():
    immutable_list_0 = module_0.ImmutableList()
    none_type_0 = None
    bool_0 = immutable_list_0.__eq__(immutable_list_0)
    immutable_list_1 = module_0.ImmutableList(none_type_0)
    immutable_list_2 = immutable_list_1.__add__(immutable_list_0)
    var_0 = immutable_list_2.to_list()
    var_1 = immutable_list_2.reduce(none_type_0, none_type_0)
    immutable_list_3 = immutable_list_0.unshift(none_type_0)
    var_2 = immutable_list_3.find(var_1)
    immutable_list_0.map(var_2)


def test_case_9():
    bool_0 = True
    immutable_list_0 = module_0.ImmutableList(is_empty=bool_0)
    var_0 = immutable_list_0.reduce(immutable_list_0, bool_0)
    immutable_list_1 = module_0.ImmutableList()
    var_1 = immutable_list_1.find(var_0)
    immutable_list_2 = module_0.ImmutableList(tail=var_0, is_empty=var_0)
    var_2 = immutable_list_2.__len__()
    var_3 = immutable_list_2.find(var_2)
    var_4 = immutable_list_2.find(var_3)
    immutable_list_2.map(var_2)


def test_case_10():
    bool_0 = False
    immutable_list_0 = module_0.ImmutableList(is_empty=bool_0)
    immutable_list_0.filter(bool_0)


def test_case_11():
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList(none_type_0, is_empty=none_type_0)
    immutable_list_1 = immutable_list_0.__add__(immutable_list_0)
    bool_0 = immutable_list_1.__eq__(immutable_list_1)
    var_0 = immutable_list_0.reduce(bool_0, immutable_list_0)
    var_1 = var_0.unshift(var_0)
    var_2 = immutable_list_0.find(var_1)
    var_3 = immutable_list_0.reduce(immutable_list_0, var_1)
    var_4 = var_3.__len__()
    var_3.filter(var_0)


def test_case_12():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    var_0 = immutable_list_0.find(immutable_list_1)
    immutable_list_1.find(immutable_list_1)


def test_case_13():
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList(none_type_0, is_empty=none_type_0)
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    immutable_list_2 = immutable_list_0.reduce(none_type_0, immutable_list_1)
    immutable_list_1.find(none_type_0)


def test_case_14():
    list_0 = []
    immutable_list_0 = module_0.ImmutableList(list_0)
    immutable_list_0.reduce(immutable_list_0, list_0)


def test_case_15():
    immutable_list_0 = module_0.ImmutableList()


def test_case_16():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    var_0 = immutable_list_0.append(immutable_list_1)
    immutable_list_1.find(immutable_list_1)


def test_case_17():
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList(none_type_0, is_empty=none_type_0)
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    immutable_list_2 = immutable_list_1.append(immutable_list_1)
    immutable_list_1.find(none_type_0)


def test_case_18():
    int_0 = 1469
    immutable_list_0 = module_0.ImmutableList(int_0, is_empty=int_0)
    immutable_list_1 = immutable_list_0.__len__()
    immutable_list_0.find(immutable_list_0)


def test_case_19():
    int_0 = 1469
    immutable_list_0 = module_0.ImmutableList(int_0, is_empty=int_0)
    immutable_list_0.find(immutable_list_0)


def test_case_20():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = module_0.ImmutableList(immutable_list_0, immutable_list_0)
    immutable_list_2 = immutable_list_0.__add__(immutable_list_1)
    str_0 = immutable_list_0.__str__()
    none_type_0 = None
    immutable_list_3 = module_0.ImmutableList(none_type_0, none_type_0)
    var_0 = immutable_list_0.reduce(immutable_list_1, immutable_list_3)
    var_1 = immutable_list_0.find(var_0)
    var_2 = var_0.to_list()
    immutable_list_1.reduce(var_1, immutable_list_3)


def test_case_21():
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.unshift(none_type_0)
    immutable_list_2 = module_0.ImmutableList(is_empty=none_type_0)
    bool_0 = immutable_list_2.__eq__(immutable_list_1)
    immutable_list_3 = immutable_list_2.unshift(immutable_list_2)
    immutable_list_3.find(none_type_0)
