# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pymonet.immutable_list as module_0


def test_case_0():
    bool_0 = False
    immutable_list_0 = module_0.ImmutableList(bool_0, bool_0)
    bool_1 = immutable_list_0.__eq__(immutable_list_0)
    immutable_list_0.find(bool_0)


def test_case_1():
    bool_0 = False
    immutable_list_0 = module_0.ImmutableList(bool_0, bool_0)
    bool_1 = immutable_list_0.__eq__(bool_0)
    immutable_list_0.find(bool_0)


def test_case_2():
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList(none_type_0)
    immutable_list_1 = immutable_list_0.append(immutable_list_0)
    var_0 = immutable_list_0.find(immutable_list_0)
    immutable_list_0.filter(var_0)


def test_case_3():
    float_0 = -1733.47
    bytes_0 = b""
    dict_0 = {bytes_0: bytes_0}
    tuple_0 = (bytes_0, dict_0)
    immutable_list_0 = module_0.ImmutableList(is_empty=tuple_0)
    var_0 = immutable_list_0.__len__()
    immutable_list_0.__add__(float_0)


def test_case_4():
    immutable_list_0 = module_0.ImmutableList()
    var_0 = immutable_list_0.__len__()
    str_0 = immutable_list_0.__str__()
    immutable_list_1 = module_0.ImmutableList(tail=var_0)
    immutable_list_1.__str__()


def test_case_5():
    bool_0 = True
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList(bool_0, none_type_0)
    bool_1 = immutable_list_0.__len__()
    immutable_list_0.find(bool_0)


def test_case_6():
    bool_0 = False
    immutable_list_0 = module_0.ImmutableList(is_empty=bool_0)
    var_0 = immutable_list_0.to_list()
    immutable_list_0.filter(var_0)


def test_case_7():
    immutable_list_0 = module_0.ImmutableList()
    none_type_0 = None
    immutable_list_0.map(none_type_0)


def test_case_8():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_0.filter(immutable_list_0)


def test_case_9():
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList(none_type_0, none_type_0, none_type_0)
    var_0 = immutable_list_0.find(none_type_0)
    var_1 = immutable_list_0.reduce(var_0, none_type_0)
    immutable_list_1 = immutable_list_0.append(none_type_0)
    immutable_list_1.filter(var_1)


def test_case_10():
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList(none_type_0)
    var_0 = immutable_list_0.find(immutable_list_0)
    immutable_list_0.filter(var_0)


def test_case_11():
    immutable_list_0 = module_0.ImmutableList()
    var_0 = immutable_list_0.reduce(immutable_list_0, immutable_list_0)
    var_1 = immutable_list_0.find(immutable_list_0)
    immutable_list_0.filter(var_1)


def test_case_12():
    bytes_0 = b"\xbc"
    bool_0 = False
    list_0 = [bytes_0, bool_0]
    bool_1 = True
    immutable_list_0 = module_0.ImmutableList(is_empty=bool_1)
    immutable_list_1 = immutable_list_0.unshift(list_0)
    bool_2 = True
    immutable_list_2 = immutable_list_0.append(bool_1)
    immutable_list_3 = module_0.ImmutableList()
    immutable_list_4 = immutable_list_3.__add__(immutable_list_3)
    immutable_list_5 = immutable_list_2.append(bool_2)
    var_0 = immutable_list_0.find(immutable_list_0)
    immutable_list_1.reduce(var_0, immutable_list_2)


def test_case_13():
    immutable_list_0 = module_0.ImmutableList()


def test_case_14():
    bool_0 = False
    immutable_list_0 = module_0.ImmutableList(bool_0)
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    immutable_list_1.find(bool_0)


def test_case_15():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.__add__(immutable_list_0)
    var_0 = immutable_list_0.reduce(immutable_list_1, immutable_list_0)
    var_1 = immutable_list_1.__len__()
    immutable_list_2 = immutable_list_0.unshift(immutable_list_0)
    immutable_list_3 = module_0.ImmutableList(var_1)
    bool_0 = immutable_list_2.__eq__(immutable_list_3)
    immutable_list_4 = module_0.ImmutableList(var_0, is_empty=var_0)
    bool_1 = True
    var_2 = var_0.find(bool_1)
    var_3 = immutable_list_1.to_list()
    immutable_list_4.find(var_3)


def test_case_16():
    bool_0 = False
    immutable_list_0 = module_0.ImmutableList(bool_0, bool_0)
    immutable_list_1 = immutable_list_0.append(bool_0)
    immutable_list_1.find(bool_0)


def test_case_17():
    list_0 = []
    immutable_list_0 = module_0.ImmutableList(list_0)
    immutable_list_0.reduce(list_0, immutable_list_0)


def test_case_18():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.__add__(immutable_list_0)
    var_0 = immutable_list_0.reduce(immutable_list_1, immutable_list_0)
    var_1 = immutable_list_1.__len__()
    immutable_list_2 = immutable_list_0.unshift(immutable_list_0)
    bool_0 = immutable_list_0.__eq__(var_1)
    immutable_list_3 = module_0.ImmutableList(immutable_list_1, is_empty=var_1)
    bool_1 = immutable_list_2.__eq__(immutable_list_3)
    immutable_list_4 = module_0.ImmutableList(var_0, is_empty=var_0)
    bool_2 = True
    var_2 = var_0.find(bool_2)
    var_3 = immutable_list_3.__len__()
    immutable_list_4.filter(var_0)


def test_case_19():
    complex_0 = 728.123 - 71.3j
    immutable_list_0 = module_0.ImmutableList(tail=complex_0)
    none_type_0 = None
    var_0 = immutable_list_0.reduce(none_type_0, none_type_0)
    immutable_list_0.map(complex_0)


def test_case_20():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    var_0 = immutable_list_1.__len__()
    immutable_list_2 = immutable_list_0.unshift(immutable_list_0)
    immutable_list_2.find(immutable_list_2)


def test_case_21():
    bool_0 = False
    immutable_list_0 = module_0.ImmutableList(bool_0, bool_0)
    immutable_list_0.find(bool_0)


def test_case_22():
    bool_0 = False
    immutable_list_0 = module_0.ImmutableList(bool_0)
    immutable_list_0.find(bool_0)
