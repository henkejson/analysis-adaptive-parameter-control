# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pymonet.immutable_list as module_0


def test_case_0():
    immutable_list_0 = module_0.ImmutableList()
    bool_0 = immutable_list_0.__eq__(immutable_list_0)
    var_0 = immutable_list_0.__len__()
    immutable_list_1 = module_0.ImmutableList(var_0, var_0, var_0)
    immutable_list_1.find(immutable_list_1)


def test_case_1():
    immutable_list_0 = module_0.ImmutableList()
    bool_0 = immutable_list_0.__eq__(immutable_list_0)
    bool_1 = immutable_list_0.__eq__(bool_0)
    var_0 = immutable_list_0.__len__()
    immutable_list_1 = module_0.ImmutableList(var_0, var_0, var_0)
    immutable_list_1.find(immutable_list_1)


def test_case_2():
    bytes_0 = b"j\xd2\xe6\xb0`@]E\xa6_\xa2'"
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.append(bytes_0)


def test_case_3():
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList(is_empty=none_type_0)
    immutable_list_0.__add__(none_type_0)


def test_case_4():
    immutable_list_0 = module_0.ImmutableList()
    var_0 = immutable_list_0.__len__()
    immutable_list_1 = module_0.ImmutableList(var_0, var_0, var_0)
    immutable_list_1.find(immutable_list_1)


def test_case_5():
    str_0 = 'lomKk ".p*7zm^/fT).x'
    immutable_list_0 = module_0.ImmutableList(str_0)
    var_0 = immutable_list_0.__len__()
    immutable_list_0.map(var_0)


def test_case_6():
    immutable_list_0 = module_0.ImmutableList()
    var_0 = immutable_list_0.to_list()
    immutable_list_0.filter(var_0)


def test_case_7():
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList(tail=none_type_0, is_empty=none_type_0)
    var_0 = immutable_list_0.find(immutable_list_0)
    immutable_list_1 = module_0.ImmutableList()
    immutable_list_2 = immutable_list_1.append(immutable_list_0)
    var_1 = immutable_list_2.to_list()


def test_case_8():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_0.filter(immutable_list_0)


def test_case_9():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = module_0.ImmutableList(tail=immutable_list_0)
    str_0 = immutable_list_0.__str__()
    immutable_list_1.filter(str_0)


def test_case_10():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = module_0.ImmutableList(
        immutable_list_0, immutable_list_0, immutable_list_0
    )
    immutable_list_1.find(immutable_list_1)


def test_case_11():
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList(tail=none_type_0)
    immutable_list_1 = immutable_list_0.append(none_type_0)
    immutable_list_2 = immutable_list_0.unshift(immutable_list_0)
    var_0 = immutable_list_0.reduce(none_type_0, none_type_0)
    immutable_list_2.find(var_0)


def test_case_12():
    immutable_list_0 = module_0.ImmutableList()


def test_case_13():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = module_0.ImmutableList(
        immutable_list_0, immutable_list_0, immutable_list_0
    )
    str_0 = immutable_list_0.__str__()
    immutable_list_1.find(immutable_list_1)


def test_case_14():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)


def test_case_15():
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList(tail=none_type_0)
    immutable_list_1 = immutable_list_0.append(none_type_0)
    immutable_list_2 = immutable_list_0.unshift(immutable_list_0)
    immutable_list_3 = immutable_list_1.__add__(immutable_list_0)
    var_0 = immutable_list_0.reduce(none_type_0, none_type_0)
    immutable_list_2.find(var_0)


def test_case_16():
    str_0 = 'lomKk ".p*7zm^/fT).x'
    immutable_list_0 = module_0.ImmutableList(str_0)
    var_0 = immutable_list_0.__len__()
    immutable_list_0.find(str_0)


def test_case_17():
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.append(immutable_list_0)
    bool_0 = immutable_list_0.__eq__(immutable_list_1)
    immutable_list_2 = module_0.ImmutableList()
    immutable_list_3 = module_0.ImmutableList(immutable_list_2)
    immutable_list_3.find(none_type_0)


def test_case_18():
    none_type_0 = None
    bool_0 = True
    immutable_list_0 = module_0.ImmutableList()
    none_type_1 = None
    immutable_list_1 = module_0.ImmutableList(none_type_1, bool_0)
    immutable_list_2 = module_0.ImmutableList(immutable_list_0)
    immutable_list_3 = immutable_list_2.append(none_type_0)
    immutable_list_4 = module_0.ImmutableList()
    bool_1 = True
    immutable_list_5 = module_0.ImmutableList(is_empty=none_type_1)
    var_0 = immutable_list_2.__len__()
    immutable_list_2.reduce(var_0, bool_1)


def test_case_19():
    none_type_0 = None
    bool_0 = True
    immutable_list_0 = module_0.ImmutableList(none_type_0, bool_0)
    none_type_1 = None
    immutable_list_1 = module_0.ImmutableList(tail=none_type_1)
    immutable_list_2 = module_0.ImmutableList()
    immutable_list_3 = immutable_list_2.append(immutable_list_1)
    bool_1 = immutable_list_3.__eq__(immutable_list_0)
    immutable_list_4 = module_0.ImmutableList()
    immutable_list_5 = module_0.ImmutableList()
    immutable_list_6 = immutable_list_5.__add__(immutable_list_1)
    str_0 = immutable_list_6.__str__()
    var_0 = immutable_list_1.find(none_type_1)
    immutable_list_7 = immutable_list_2.unshift(str_0)
    immutable_list_7.reduce(var_0, immutable_list_7)


def test_case_20():
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList(tail=none_type_0)
    immutable_list_1 = immutable_list_0.append(none_type_0)
    immutable_list_2 = immutable_list_0.unshift(immutable_list_0)
    var_0 = immutable_list_2.__len__()
    immutable_list_2.find(var_0)


def test_case_21():
    bool_0 = True
    immutable_list_0 = module_0.ImmutableList(tail=bool_0)
    immutable_list_1 = immutable_list_0.unshift(bool_0)
    bool_1 = immutable_list_1.__eq__(immutable_list_0)
    immutable_list_1.find(bool_1)


def test_case_22():
    none_type_0 = None
    bool_0 = True
    immutable_list_0 = module_0.ImmutableList(none_type_0, bool_0)
    none_type_1 = None
    none_type_2 = None
    immutable_list_1 = module_0.ImmutableList(tail=none_type_2)
    immutable_list_2 = immutable_list_0.append(none_type_1)
    immutable_list_0.map(immutable_list_2)
