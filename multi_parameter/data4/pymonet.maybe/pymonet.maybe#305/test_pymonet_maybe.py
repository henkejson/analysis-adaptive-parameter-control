# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pymonet.maybe as module_0


def test_case_0():
    str_0 = "\n        Transform Lazy into Box with constructor_fn result.\n\n        :returns: Box monad with constructor_fn result\n        :rtype: Box[A]\n        "
    maybe_0 = module_0.Maybe(str_0, str_0)


def test_case_1():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)


def test_case_2():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    bool_0 = maybe_0.__eq__(maybe_0)
    var_0 = maybe_0.to_lazy()
    var_1 = var_0.map(var_0)
    maybe_0.map(none_type_0)


def test_case_3():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    bool_0 = maybe_0.__eq__(maybe_0)
    var_0 = maybe_0.to_lazy()
    bool_1 = maybe_0.__eq__(bool_0)
    var_1 = var_0.map(var_0)
    maybe_0.map(none_type_0)


def test_case_4():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    var_0 = maybe_0.to_either()
    none_type_1 = None
    maybe_1 = module_0.Maybe(none_type_1, maybe_0)
    bool_0 = maybe_1.__eq__(maybe_1)
    var_1 = maybe_0.to_either()
    var_2 = maybe_1.map(none_type_0)
    var_3 = maybe_1.map(none_type_0)
    var_4 = var_3.get_or_else(var_3)
    var_5 = var_3.bind(var_4)


def test_case_5():
    int_0 = 0
    maybe_0 = module_0.Maybe(int_0, int_0)
    maybe_1 = module_0.Maybe(int_0, int_0)
    var_0 = maybe_1.to_box()
    maybe_1.map(maybe_1)


def test_case_6():
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    maybe_0.bind(bool_0)


def test_case_7():
    bytes_0 = b"L\x95\xf3\xc0Lj\x05pY\xf7\xa7"
    maybe_0 = module_0.Maybe(bytes_0, bytes_0)
    var_0 = maybe_0.map(bytes_0)
    var_1 = maybe_0.ap(maybe_0)
    bool_0 = maybe_0.__eq__(bytes_0)
    var_2 = maybe_0.map(maybe_0)
    var_3 = maybe_0.to_validation()
    maybe_1 = module_0.Maybe(maybe_0, bool_0)
    var_4 = var_2.filter(bool_0)
    var_5 = maybe_0.to_validation()
    var_6 = var_2.to_either()
    maybe_1.filter(var_5)


def test_case_8():
    int_0 = -1333
    none_type_0 = None
    maybe_0 = module_0.Maybe(int_0, none_type_0)
    var_0 = maybe_0.to_lazy()
    complex_0 = 3442.72057 + 681.8365j
    bool_0 = False
    var_1 = var_0.to_validation()
    maybe_1 = module_0.Maybe(complex_0, bool_0)
    maybe_1.ap(int_0)


def test_case_9():
    none_type_0 = None
    bytes_0 = b"\x03\x81=J&@\xd8\x8c-\xa5q\xef\xa2T"
    bool_0 = False
    tuple_0 = (bytes_0, bool_0)
    none_type_1 = None
    bool_1 = True
    maybe_0 = module_0.Maybe(none_type_1, bool_1)
    var_0 = maybe_0.get_or_else(tuple_0)
    var_0.filter(none_type_0)


def test_case_10():
    str_0 = "\\3Wc>y=<%Yew%EOTqKKv"
    bool_0 = False
    set_0 = {bool_0}
    tuple_0 = (bool_0, set_0)
    maybe_0 = module_0.Maybe(tuple_0, bool_0)
    var_0 = maybe_0.get_or_else(str_0)
    var_0.to_either()


def test_case_11():
    bool_0 = True
    tuple_0 = (bool_0,)
    dict_0 = {tuple_0: bool_0, tuple_0: bool_0, bool_0: tuple_0}
    maybe_0 = module_0.Maybe(dict_0, dict_0)
    var_0 = maybe_0.to_either()


def test_case_12():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    var_0 = maybe_0.to_either()
    maybe_1 = module_0.Maybe(none_type_0, none_type_0)
    bool_0 = maybe_1.__eq__(maybe_1)
    var_1 = maybe_1.to_lazy()
    var_2 = var_1.map(var_1)
    maybe_1.map(none_type_0)


def test_case_13():
    int_0 = -2555
    bytes_0 = b"\x85Q\xad\xe4.\x81=\x81\xfc\xf2\xafH\x95\x0bD\xfayk$\x15"
    bool_0 = True
    maybe_0 = module_0.Maybe(bytes_0, bool_0)
    var_0 = maybe_0.to_box()
    var_0.ap(int_0)


def test_case_14():
    bool_0 = True
    list_0 = []
    none_type_0 = None
    maybe_0 = module_0.Maybe(list_0, none_type_0)
    var_0 = maybe_0.to_box()
    var_1 = maybe_0.to_lazy()
    var_0.map(bool_0)


def test_case_15():
    bytes_0 = b"L\x95\xf3\xc0Lj\x05pY\xf7\xa7"
    maybe_0 = module_0.Maybe(bytes_0, bytes_0)
    var_0 = maybe_0.ap(bytes_0)
    var_1 = var_0.to_box()
    var_2 = maybe_0.filter(var_1)
    bool_0 = var_2.__eq__(bytes_0)
    bool_1 = True
    bool_2 = maybe_0.__eq__(bool_1)
    bool_3 = False
    maybe_1 = module_0.Maybe(var_0, bool_3)
    maybe_2 = module_0.Maybe(var_0, bool_3)
    var_3 = var_0.to_lazy()
    maybe_1.bind(var_1)


def test_case_16():
    int_0 = -1677
    maybe_0 = module_0.Maybe(int_0, int_0)
    var_0 = maybe_0.to_try()
    var_0.to_validation()


def test_case_17():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    var_0 = maybe_0.to_try()
    var_0.to_box()


def test_case_18():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    bool_0 = maybe_0.__eq__(maybe_0)
    var_0 = maybe_0.to_lazy()
    var_1 = var_0.map(var_0)
    var_2 = var_1.map(var_0)
    var_3 = maybe_0.to_validation()
    var_3.get_or_else(bool_0)


def test_case_19():
    bytes_0 = b"L\x95\xf3\xc0Lj\x05pY\xf7\xa7"
    maybe_0 = module_0.Maybe(bytes_0, bytes_0)
    var_0 = maybe_0.to_box()
    var_1 = var_0.to_either()
    var_2 = maybe_0.ap(bytes_0)
    var_3 = var_2.get_or_else(maybe_0)
    var_4 = var_3.to_lazy()
    bool_0 = var_4.__eq__(bytes_0)
    bool_1 = maybe_0.__eq__(bool_0)
    var_5 = var_3.ap(bytes_0)
    var_6 = var_2.map(var_3)
    var_7 = var_4.bind(var_3)
    var_8 = var_3.filter(var_4)
    maybe_1 = module_0.Maybe(maybe_0, bool_0)
    var_9 = var_5.to_either()
    var_10 = var_6.filter(bool_0)
    var_11 = var_4.to_try()
    var_12 = maybe_0.to_validation()
    bool_2 = var_9.__eq__(bytes_0)
    var_13 = var_10.filter(var_11)
    var_14 = var_10.get_or_else(maybe_1)
    var_15 = var_8.to_either()
    maybe_1.filter(var_12)


def test_case_20():
    bytes_0 = b"L\x95\xf3\xc0Lj\x05pY\xf7\xa7"
    maybe_0 = module_0.Maybe(bytes_0, bytes_0)
    var_0 = maybe_0.to_either()
    var_1 = maybe_0.ap(bytes_0)
    var_2 = var_0.bind(bytes_0)
    var_3 = maybe_0.filter(var_2)
    bool_0 = var_3.__eq__(bytes_0)
    bool_1 = maybe_0.__eq__(var_1)
    var_4 = var_2.ap(bytes_0)
    var_5 = var_1.map(var_2)
    var_6 = var_3.bind(var_2)
    var_7 = var_5.map(maybe_0)
    maybe_1 = module_0.Maybe(maybe_0, bool_0)
    var_8 = var_3.to_try()
    var_9 = maybe_0.to_validation()
    bool_2 = var_8.__eq__(bytes_0)
    var_10 = var_6.filter(var_8)
    var_11 = var_6.filter(maybe_0)
    var_12 = var_7.to_either()
    maybe_1.filter(var_9)
