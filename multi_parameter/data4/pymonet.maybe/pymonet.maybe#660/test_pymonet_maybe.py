# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pymonet.maybe as module_0


def test_case_0():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)


def test_case_1():
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bool_0)


def test_case_2():
    none_type_0 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_0.ap(bool_0)
    var_1 = var_0.ap(maybe_0)
    tuple_0 = ()
    maybe_1 = module_0.Maybe(tuple_0, var_1)
    var_2 = var_0.filter(var_0)
    var_3 = maybe_1.to_lazy()
    var_4 = var_3.to_try()
    var_5 = maybe_0.to_validation()
    bool_1 = maybe_1.__eq__(var_5)
    var_6 = var_0.map(var_4)
    maybe_2 = module_0.Maybe(var_6, bool_1)
    var_7 = maybe_2.to_try()


def test_case_3():
    list_0 = []
    maybe_0 = module_0.Maybe(list_0, list_0)
    var_0 = maybe_0.to_box()
    maybe_0.map(var_0)


def test_case_4():
    none_type_0 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_0.ap(bool_0)
    none_type_1 = None
    str_0 = " 2P/'OC'h|tc"
    maybe_1 = module_0.Maybe(none_type_1, str_0)
    var_1 = var_0.filter(none_type_0)
    var_2 = var_0.to_lazy()
    var_3 = maybe_1.filter(none_type_0)
    var_4 = var_2.to_try()
    var_5 = var_3.ap(none_type_1)
    var_6 = var_5.bind(var_0)
    var_7 = var_6.to_lazy()
    var_8 = maybe_0.bind(none_type_0)
    var_9 = var_5.to_either()
    var_10 = var_3.to_validation()
    var_11 = maybe_0.to_lazy()


def test_case_5():
    none_type_0 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_0.to_lazy()
    var_1 = var_0.to_box()
    none_type_1 = None
    bool_1 = False
    none_type_2 = None
    maybe_1 = module_0.Maybe(bool_1, none_type_2)
    maybe_1.bind(none_type_1)


def test_case_6():
    bool_0 = True
    int_0 = -30
    bool_1 = False
    maybe_0 = module_0.Maybe(int_0, bool_1)
    str_0 = "D\\`xxMhn3+]'g%E\\4"
    bool_2 = True
    maybe_1 = module_0.Maybe(str_0, bool_2)
    var_0 = maybe_1.ap(maybe_0)
    var_1 = var_0.ap(bool_0)
    bool_3 = False
    maybe_2 = module_0.Maybe(bool_3, bool_3)
    maybe_2.filter(var_1)


def test_case_7():
    bool_0 = False
    bool_1 = False
    maybe_0 = module_0.Maybe(bool_1, bool_1)
    maybe_0.ap(bool_0)


def test_case_8():
    list_0 = []
    maybe_0 = module_0.Maybe(list_0, list_0)
    maybe_1 = module_0.Maybe(list_0, maybe_0)
    var_0 = maybe_1.filter(list_0)
    maybe_0.filter(maybe_1)


def test_case_9():
    complex_0 = 2083 + 47j
    none_type_0 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    bytes_0 = b"B(4\xab\xb3\x11"
    tuple_0 = (maybe_0, maybe_0, maybe_0, bytes_0)
    maybe_1 = module_0.Maybe(tuple_0, tuple_0)
    var_0 = maybe_1.get_or_else(complex_0)


def test_case_10():
    none_type_0 = None
    bool_0 = False
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_0.to_validation()
    var_1 = maybe_0.to_either()
    var_2 = maybe_0.get_or_else(var_0)
    var_3 = var_1.to_box()
    maybe_0.filter(var_3)


def test_case_11():
    none_type_0 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_0.to_either()
    var_1 = var_0.to_validation()


def test_case_12():
    list_0 = []
    maybe_0 = module_0.Maybe(list_0, list_0)
    bool_0 = True
    maybe_1 = module_0.Maybe(list_0, bool_0)
    var_0 = maybe_1.filter(list_0)
    var_1 = var_0.to_box()
    var_1.bind(var_1)


def test_case_13():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    var_0 = maybe_0.to_box()
    var_0.bind(var_0)


def test_case_14():
    str_0 = "qY"
    maybe_0 = module_0.Maybe(str_0, str_0)
    var_0 = maybe_0.to_lazy()
    var_1 = var_0.map(var_0)
    bool_0 = maybe_0.__eq__(var_0)


def test_case_15():
    bytes_0 = b"\xa8\xe6\x8a\xbb\xe0\xbbYa~[Y\x99"
    bool_0 = False
    maybe_0 = module_0.Maybe(bytes_0, bool_0)
    var_0 = maybe_0.to_lazy()
    int_0 = -565
    tuple_0 = ()
    list_0 = [tuple_0, tuple_0]
    maybe_1 = module_0.Maybe(list_0, tuple_0)
    var_1 = maybe_1.to_lazy()
    var_2 = var_0.to_box()
    var_3 = maybe_1.to_lazy()
    var_4 = var_3.bind(int_0)
    var_4.filter(var_0)


def test_case_16():
    complex_0 = -744.15 - 950.43j
    maybe_0 = module_0.Maybe(complex_0, complex_0)
    var_0 = maybe_0.to_box()
    var_1 = maybe_0.get_or_else(var_0)
    var_2 = maybe_0.to_try()
    var_3 = maybe_0.to_try()
    var_4 = var_0.to_either()
    var_3.to_box()


def test_case_17():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    var_0 = maybe_0.to_validation()
    var_1 = var_0.to_box()
    tuple_0 = ()
    maybe_1 = module_0.Maybe(tuple_0, tuple_0)
    var_2 = maybe_1.to_try()
    var_2.to_lazy()


def test_case_18():
    list_0 = []
    maybe_0 = module_0.Maybe(list_0, list_0)
    bool_0 = True
    maybe_1 = module_0.Maybe(list_0, bool_0)
    var_0 = maybe_1.filter(list_0)
    var_1 = maybe_1.to_validation()
    maybe_0.filter(var_1)


def test_case_19():
    none_type_0 = None
    bool_0 = False
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_0.to_validation()
    var_1 = var_0.to_box()
    maybe_0.filter(var_1)


def test_case_20():
    none_type_0 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_0.ap(bool_0)
    var_1 = var_0.map(none_type_0)
    var_2 = maybe_0.to_either()
    var_3 = maybe_0.to_either()
    maybe_1 = module_0.Maybe(var_2, var_2)
    var_4 = maybe_1.to_lazy()
    maybe_2 = module_0.Maybe(none_type_0, var_2)
    var_5 = var_0.get_or_else(var_2)
    var_6 = var_4.to_validation()
    bool_1 = maybe_0.__eq__(maybe_0)
    var_7 = var_5.map(var_5)
    var_8 = var_2.map(var_2)
    var_6.to_validation()


def test_case_21():
    none_type_0 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_0.ap(bool_0)
    var_1 = var_0.get_or_else(none_type_0)
    var_2 = maybe_0.to_either()
    str_0 = "[Cj"
    maybe_1 = module_0.Maybe(str_0, str_0)
    var_3 = maybe_1.to_lazy()
    maybe_2 = module_0.Maybe(none_type_0, var_1)
    var_4 = maybe_2.get_or_else(var_1)
    var_5 = var_0.get_or_else(maybe_0)
    var_6 = var_5.to_validation()
    bool_1 = maybe_2.__eq__(var_5)
    var_7 = maybe_1.bind(var_2)
    bool_2 = maybe_0.__eq__(maybe_0)
    var_8 = module_0.Maybe(maybe_2, var_1)
    var_4.map(var_5)


def test_case_22():
    none_type_0 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_0.ap(bool_0)
    var_1 = var_0.get_or_else(none_type_0)
    var_2 = maybe_0.to_either()
    var_3 = maybe_0.to_lazy()
    maybe_1 = module_0.Maybe(none_type_0, var_1)
    var_4 = maybe_1.get_or_else(var_1)
    var_5 = var_0.get_or_else(maybe_0)
    var_6 = var_5.to_validation()
    bool_1 = var_0.__eq__(var_6)
    var_7 = var_0.bind(var_6)
    bool_2 = var_7.__eq__(var_5)
    bool_3 = maybe_1.__eq__(maybe_1)
    var_8 = maybe_0.map(var_5)
    var_4.map(var_7)
