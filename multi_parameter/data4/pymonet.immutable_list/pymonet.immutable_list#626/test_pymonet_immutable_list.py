# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pymonet.immutable_list as module_0


def test_case_0():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.append(immutable_list_0)
    bool_0 = immutable_list_0.__eq__(immutable_list_1)
    var_0 = immutable_list_0.__len__()
    bool_1 = immutable_list_0.__eq__(immutable_list_0)
    immutable_list_2 = var_0.__add__(bool_1)
    immutable_list_1.filter(immutable_list_0)


def test_case_1():
    immutable_list_0 = module_0.ImmutableList()
    none_type_0 = None
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    bool_0 = immutable_list_0.__eq__(none_type_0)
    immutable_list_2 = immutable_list_1.unshift(none_type_0)
    immutable_list_1.find(none_type_0)


def test_case_2():
    immutable_list_0 = module_0.ImmutableList()
    var_0 = immutable_list_0.append(immutable_list_0)
    var_0.filter(immutable_list_0)


def test_case_3():
    none_type_0 = None
    none_type_1 = None
    immutable_list_0 = module_0.ImmutableList(none_type_1)
    immutable_list_0.__add__(none_type_0)


def test_case_4():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    var_0 = immutable_list_0.__len__()
    immutable_list_1.find(var_0)


def test_case_5():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    var_0 = immutable_list_1.__len__()
    immutable_list_2 = immutable_list_0.unshift(immutable_list_1)
    immutable_list_1.find(var_0)


def test_case_6():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = module_0.ImmutableList(tail=immutable_list_0)
    str_0 = immutable_list_0.__str__()


def test_case_7():
    none_type_0 = None
    none_type_1 = None
    immutable_list_0 = module_0.ImmutableList(none_type_1)
    immutable_list_1 = immutable_list_0.append(none_type_0)
    str_0 = immutable_list_1.__str__()


def test_case_8():
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList()
    var_0 = immutable_list_0.__len__()
    bool_0 = immutable_list_0.__eq__(none_type_0)
    immutable_list_0.map(var_0)


def test_case_9():
    none_type_0 = None
    none_type_1 = None
    immutable_list_0 = module_0.ImmutableList(none_type_1)
    var_0 = immutable_list_0.__len__()
    immutable_list_1 = var_0.__add__(none_type_0)
    immutable_list_2 = module_0.ImmutableList()
    var_1 = immutable_list_2.to_list()
    bool_0 = True
    immutable_list_3 = module_0.ImmutableList(tail=bool_0)
    immutable_list_3.map(var_1)


def test_case_10():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_0.filter(immutable_list_0)


def test_case_11():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    var_0 = immutable_list_0.find(immutable_list_1)
    var_1 = immutable_list_0.__len__()
    immutable_list_1.find(var_1)


def test_case_12():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = module_0.ImmutableList(
        immutable_list_0, is_empty=immutable_list_0
    )
    immutable_list_1.find(immutable_list_1)


def test_case_13():
    immutable_list_0 = module_0.ImmutableList()
    var_0 = immutable_list_0.reduce(immutable_list_0, immutable_list_0)
    var_0.filter(immutable_list_0)


def test_case_14():
    immutable_list_0 = module_0.ImmutableList()


def test_case_15():
    immutable_list_0 = module_0.ImmutableList()
    none_type_0 = None
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    immutable_list_1.find(none_type_0)


def test_case_16():
    immutable_list_0 = module_0.ImmutableList()
    none_type_0 = None
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    var_0 = immutable_list_1.append(none_type_0)
    immutable_list_2 = immutable_list_1.unshift(none_type_0)
    immutable_list_1.find(none_type_0)


def test_case_17():
    bool_0 = False
    immutable_list_0 = module_0.ImmutableList(bool_0)
    immutable_list_1 = immutable_list_0.append(bool_0)
    var_0 = immutable_list_1.__len__()
    immutable_list_1.find(var_0)


def test_case_18():
    list_0 = []
    immutable_list_0 = module_0.ImmutableList(list_0)
    var_0 = immutable_list_0.__len__()
    immutable_list_0.reduce(var_0, var_0)


def test_case_19():
    bool_0 = True
    immutable_list_0 = module_0.ImmutableList(is_empty=bool_0)
    int_0 = 898
    immutable_list_1 = immutable_list_0.append(int_0)
    dict_0 = {bool_0: bool_0, int_0: immutable_list_0}
    bool_1 = immutable_list_0.__eq__(dict_0)
    immutable_list_2 = immutable_list_0.unshift(immutable_list_0)
    str_0 = immutable_list_0.__str__()
    immutable_list_3 = immutable_list_0.append(bool_0)
    immutable_list_4 = module_0.ImmutableList(immutable_list_0, immutable_list_0)
    bool_2 = immutable_list_3.__eq__(immutable_list_0)
    var_0 = immutable_list_0.append(str_0)
    int_1 = 177
    var_1 = immutable_list_0.reduce(var_0, int_1)
    var_2 = immutable_list_3.find(var_1)
    immutable_list_5 = immutable_list_4.unshift(var_2)
    immutable_list_4.reduce(bool_2, var_2)


def test_case_20():
    immutable_list_0 = module_0.ImmutableList()
    none_type_0 = None
    immutable_list_1 = immutable_list_0.append(immutable_list_0)
    immutable_list_2 = immutable_list_1.unshift(none_type_0)
    var_0 = immutable_list_1.__len__()
    var_1 = immutable_list_0.reduce(var_0, none_type_0)
    var_2 = immutable_list_1.to_list()
    bool_0 = immutable_list_2.__eq__(immutable_list_1)
    var_3 = immutable_list_0.find(var_1)
    set_0 = {var_3}
    immutable_list_1.filter(set_0)
