# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pymonet.immutable_list as module_0


def test_case_0():
    bool_0 = True
    immutable_list_0 = module_0.ImmutableList(bool_0)
    bool_1 = immutable_list_0.__eq__(immutable_list_0)
    immutable_list_0.find(immutable_list_0)


def test_case_1():
    bool_0 = True
    immutable_list_0 = module_0.ImmutableList(bool_0)
    bool_1 = immutable_list_0.__eq__(bool_0)
    immutable_list_0.find(immutable_list_0)


def test_case_2():
    bool_0 = True
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.unshift(bool_0)
    var_0 = immutable_list_1.__len__()


def test_case_3():
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_0.__add__(none_type_0)


def test_case_4():
    bool_0 = True
    immutable_list_0 = module_0.ImmutableList()
    var_0 = immutable_list_0.to_list()
    bool_1 = var_0.__eq__(bool_0)


def test_case_5():
    bool_0 = False
    immutable_list_0 = module_0.ImmutableList(bool_0)
    immutable_list_1 = immutable_list_0.__add__(immutable_list_0)
    str_0 = immutable_list_1.__str__()
    str_1 = immutable_list_0.__str__()
    immutable_list_0.find(immutable_list_1)


def test_case_6():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.append(immutable_list_0)
    immutable_list_2 = module_0.ImmutableList()
    immutable_list_2.map(immutable_list_2)


def test_case_7():
    none_type_0 = None
    none_type_1 = None
    bool_0 = False
    immutable_list_0 = module_0.ImmutableList(is_empty=none_type_0)
    immutable_list_1 = module_0.ImmutableList()
    immutable_list_2 = immutable_list_1.append(none_type_1)
    var_0 = immutable_list_2.reduce(bool_0, immutable_list_1)
    bool_1 = immutable_list_1.__eq__(bool_0)
    bool_2 = immutable_list_1.__eq__(immutable_list_0)
    str_0 = immutable_list_1.__str__()
    immutable_list_3 = module_0.ImmutableList()
    str_1 = immutable_list_1.__str__()
    immutable_list_4 = immutable_list_2.append(none_type_0)
    int_0 = 2260
    immutable_list_5 = module_0.ImmutableList(is_empty=int_0)
    var_1 = immutable_list_5.to_list()
    var_2 = immutable_list_1.find(bool_1)
    str_2 = immutable_list_3.__str__()
    var_3 = immutable_list_2.to_list()
    var_4 = immutable_list_2.__len__()
    immutable_list_4.map(var_1)


def test_case_8():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_0.filter(immutable_list_0)


def test_case_9():
    none_type_0 = None
    bool_0 = True
    immutable_list_0 = module_0.ImmutableList(none_type_0, is_empty=bool_0)
    none_type_1 = None
    immutable_list_1 = module_0.ImmutableList()
    immutable_list_2 = immutable_list_1.unshift(immutable_list_1)
    bool_1 = immutable_list_1.__eq__(none_type_1)
    bool_2 = immutable_list_2.__eq__(bool_0)
    str_0 = immutable_list_1.__str__()
    var_0 = immutable_list_2.__len__()
    var_1 = immutable_list_1.to_list()
    immutable_list_2.filter(var_0)


def test_case_10():
    immutable_list_0 = module_0.ImmutableList()
    var_0 = immutable_list_0.find(immutable_list_0)
    var_1 = immutable_list_0.find(var_0)
    immutable_list_0.filter(var_0)


def test_case_11():
    bool_0 = False
    immutable_list_0 = module_0.ImmutableList(bool_0)
    immutable_list_0.find(immutable_list_0)


def test_case_12():
    bool_0 = True
    immutable_list_0 = module_0.ImmutableList(bool_0)
    immutable_list_1 = module_0.ImmutableList()
    bool_1 = immutable_list_1.reduce(immutable_list_0, immutable_list_1)
    immutable_list_0.find(immutable_list_0)


def test_case_13():
    immutable_list_0 = module_0.ImmutableList()


def test_case_14():
    bool_0 = False
    immutable_list_0 = module_0.ImmutableList(bool_0)
    str_0 = immutable_list_0.__str__()
    immutable_list_0.find(immutable_list_0)


def test_case_15():
    int_0 = -1558
    int_1 = -1300
    immutable_list_0 = module_0.ImmutableList(tail=int_1)
    immutable_list_1 = immutable_list_0.unshift(int_1)
    immutable_list_2 = immutable_list_0.append(int_0)


def test_case_16():
    bool_0 = True
    immutable_list_0 = module_0.ImmutableList(bool_0)
    immutable_list_1 = module_0.ImmutableList()
    bool_1 = immutable_list_1.__eq__(immutable_list_0)
    immutable_list_0.find(immutable_list_0)


def test_case_17():
    bool_0 = False
    immutable_list_0 = module_0.ImmutableList(bool_0)
    var_0 = immutable_list_0.__len__()
    immutable_list_0.find(var_0)


def test_case_18():
    none_type_0 = None
    bool_0 = True
    immutable_list_0 = module_0.ImmutableList(none_type_0, is_empty=bool_0)
    none_type_1 = None
    immutable_list_1 = module_0.ImmutableList()
    immutable_list_2 = immutable_list_1.unshift(immutable_list_1)
    bool_1 = immutable_list_1.__eq__(none_type_1)
    immutable_list_2.reduce(immutable_list_2, bool_1)


def test_case_19():
    bool_0 = False
    list_0 = []
    immutable_list_0 = module_0.ImmutableList(list_0, list_0)
    immutable_list_0.find(bool_0)


def test_case_20():
    bool_0 = True
    immutable_list_0 = module_0.ImmutableList(bool_0)
    immutable_list_1 = immutable_list_0.__add__(immutable_list_0)
    bool_1 = immutable_list_1.__eq__(immutable_list_0)
    str_0 = immutable_list_1.__str__()
    immutable_list_0.find(immutable_list_0)


def test_case_21():
    bool_0 = False
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = module_0.ImmutableList(bool_0, is_empty=none_type_0)
    immutable_list_2 = immutable_list_0.append(bool_0)
    var_0 = immutable_list_1.to_list()
    immutable_list_1.reduce(immutable_list_1, bool_0)
