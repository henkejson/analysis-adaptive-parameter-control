# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pymonet.immutable_list as module_0
import typing as module_1


def test_case_0():
    list_0 = []
    float_0 = 233.80074
    immutable_list_0 = module_0.ImmutableList(float_0)
    str_0 = immutable_list_0.__str__()
    bool_0 = immutable_list_0.__eq__(immutable_list_0)
    immutable_list_0.find(list_0)


def test_case_1():
    list_0 = []
    float_0 = 233.80074
    immutable_list_0 = module_0.ImmutableList(float_0)
    str_0 = immutable_list_0.__str__()
    none_type_0 = None
    bool_0 = immutable_list_0.__eq__(none_type_0)
    immutable_list_0.find(list_0)


def test_case_2():
    bool_0 = False
    immutable_list_0 = module_0.ImmutableList(is_empty=bool_0)
    var_0 = immutable_list_0.__len__()
    immutable_list_1 = immutable_list_0.__add__(immutable_list_0)
    var_1 = immutable_list_0.to_list()
    var_1.to_list()


def test_case_3():
    generic_0 = module_1.Generic()
    immutable_list_0 = module_0.ImmutableList(generic_0)
    immutable_list_1 = immutable_list_0.unshift(generic_0)
    immutable_list_1.__add__(generic_0)


def test_case_4():
    immutable_list_0 = module_0.ImmutableList()
    var_0 = immutable_list_0.__len__()
    immutable_list_0.filter(var_0)


def test_case_5():
    bool_0 = False
    immutable_list_0 = module_0.ImmutableList(bool_0)
    var_0 = immutable_list_0.__len__()
    immutable_list_0.find(var_0)


def test_case_6():
    immutable_list_0 = module_0.ImmutableList()
    var_0 = immutable_list_0.to_list()
    immutable_list_0.filter(var_0)


def test_case_7():
    list_0 = []
    float_0 = 233.80074
    immutable_list_0 = module_0.ImmutableList(float_0)
    immutable_list_1 = immutable_list_0.unshift(float_0)
    str_0 = immutable_list_1.__str__()
    immutable_list_0.find(list_0)


def test_case_8():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    var_0 = immutable_list_0.__len__()
    immutable_list_0.map(var_0)


def test_case_9():
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList(none_type_0)
    var_0 = immutable_list_0.__len__()
    var_1 = immutable_list_0.reduce(var_0, var_0)
    immutable_list_1 = module_0.ImmutableList(none_type_0)
    immutable_list_2 = immutable_list_1.__add__(immutable_list_1)
    immutable_list_3 = immutable_list_2.append(none_type_0)
    immutable_list_3.map(immutable_list_0)


def test_case_10():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_0.filter(immutable_list_0)


def test_case_11():
    immutable_list_0 = module_0.ImmutableList()
    var_0 = immutable_list_0.__len__()
    str_0 = immutable_list_0.__str__()
    immutable_list_1 = immutable_list_0.append(immutable_list_0)
    var_1 = immutable_list_0.find(var_0)
    var_2 = immutable_list_0.__len__()
    immutable_list_2 = module_0.ImmutableList(tail=var_1, is_empty=immutable_list_0)
    immutable_list_3 = module_0.ImmutableList()
    var_3 = immutable_list_1.__len__()
    immutable_list_1.filter(var_1)


def test_case_12():
    immutable_list_0 = module_0.ImmutableList()
    var_0 = immutable_list_0.find(immutable_list_0)


def test_case_13():
    list_0 = []
    float_0 = 233.80074
    immutable_list_0 = module_0.ImmutableList(float_0)
    immutable_list_0.find(list_0)


def test_case_14():
    immutable_list_0 = module_0.ImmutableList()
    var_0 = immutable_list_0.to_list()
    bool_0 = True
    str_0 = immutable_list_0.__str__()
    bool_1 = False
    immutable_list_1 = module_0.ImmutableList(is_empty=bool_1)
    str_1 = immutable_list_1.__str__()
    var_1 = immutable_list_1.reduce(bool_0, bool_0)
    var_1.map(var_0)


def test_case_15():
    none_type_0 = None
    bool_0 = True
    list_0 = []
    immutable_list_0 = module_0.ImmutableList(list_0, none_type_0)
    str_0 = immutable_list_0.__str__()
    immutable_list_1 = immutable_list_0.unshift(none_type_0)
    immutable_list_2 = module_0.ImmutableList(none_type_0, is_empty=bool_0)
    var_0 = immutable_list_1.reduce(bool_0, immutable_list_2)
    immutable_list_0.reduce(var_0, immutable_list_2)


def test_case_16():
    immutable_list_0 = module_0.ImmutableList()


def test_case_17():
    bool_0 = False
    immutable_list_0 = module_0.ImmutableList(is_empty=bool_0)
    str_0 = immutable_list_0.__str__()
    var_0 = immutable_list_0.__len__()
    var_0.to_list()


def test_case_18():
    str_0 = "+JNpL{]ln&N\x0b$u*#\tk;"
    dict_0 = {}
    immutable_list_0 = module_0.ImmutableList(dict_0)
    immutable_list_1 = immutable_list_0.append(str_0)
    immutable_list_1.find(str_0)


def test_case_19():
    bool_0 = True
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.unshift(bool_0)
    immutable_list_1.find(none_type_0)


def test_case_20():
    immutable_list_0 = module_0.ImmutableList()
    str_0 = immutable_list_0.__str__()
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    none_type_0 = None
    bool_0 = immutable_list_1.__eq__(immutable_list_0)
    immutable_list_1.find(none_type_0)


def test_case_21():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.append(immutable_list_0)
    var_0 = immutable_list_0.to_list()
    var_1 = immutable_list_0.reduce(var_0, var_0)
    immutable_list_2 = immutable_list_1.__add__(immutable_list_1)
    immutable_list_3 = var_1.__add__(var_0)
    immutable_list_4 = module_0.ImmutableList(immutable_list_0, var_0)
    var_2 = immutable_list_0.__len__()
    str_0 = immutable_list_0.__str__()
    bool_0 = immutable_list_0.__eq__(var_0)
    var_3 = immutable_list_0.reduce(bool_0, var_2)
    bytes_0 = b"\x15\xb1\xdf\x8cPW\xcf\x90\x94!\x1d"
    var_2.filter(bytes_0)


def test_case_22():
    int_0 = -729
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList(int_0)
    bool_0 = immutable_list_0.__eq__(immutable_list_0)
    immutable_list_1 = immutable_list_0.append(int_0)
    immutable_list_2 = immutable_list_0.append(none_type_0)
    immutable_list_1.reduce(int_0, none_type_0)


def test_case_23():
    bytes_0 = b"{\xd5u\xd9\xd6F\xa55\xf0a\xc4 \x16"
    bool_0 = False
    immutable_list_0 = module_0.ImmutableList(bytes_0, bool_0)
    immutable_list_0.__len__()


def test_case_24():
    immutable_list_0 = module_0.ImmutableList()
    var_0 = immutable_list_0.__len__()
    var_1 = immutable_list_0.__len__()
    immutable_list_1 = immutable_list_0.append(immutable_list_0)
    var_2 = immutable_list_0.find(immutable_list_0)
    immutable_list_2 = immutable_list_0.unshift(var_2)
    bool_0 = immutable_list_2.__eq__(immutable_list_0)
    immutable_list_3 = immutable_list_1.append(var_1)
    var_3 = immutable_list_0.__len__()
    immutable_list_4 = module_0.ImmutableList(tail=var_2, is_empty=immutable_list_0)
    immutable_list_5 = immutable_list_0.unshift(immutable_list_2)
    var_4 = immutable_list_1.reduce(var_1, immutable_list_5)
    var_4.find(immutable_list_0)
