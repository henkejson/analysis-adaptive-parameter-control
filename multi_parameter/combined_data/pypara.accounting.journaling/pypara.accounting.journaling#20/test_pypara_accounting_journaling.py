# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pypara.accounting.journaling as module_0
import pypara.commons.numbers as module_1
import dataclasses as module_2


def test_case_0():
    pass


def test_case_1():
    bytes_0 = b"L\xb8ON\x1a\xd7/N\xd4\x81\xba`"
    journal_entry_0 = module_0.JournalEntry(bytes_0, bytes_0, bytes_0)
    none_type_0 = journal_entry_0.validate()


def test_case_2():
    bytes_0 = b"\xe7o\x1aH|\xd6\x12D\xa4hsEBk\xce"
    var_0 = module_1.isum(bytes_0)
    journal_entry_0 = module_0.JournalEntry(var_0, var_0, bytes_0)
    journal_entry_1 = journal_entry_0.post(var_0, var_0, var_0)


def test_case_3():
    bytes_0 = b"\xe7o\x1aH|\xd6\x12D\xa4hsEBk\xce"
    var_0 = module_1.isum(bytes_0)
    journal_entry_0 = module_0.JournalEntry(var_0, var_0, var_0)
    journal_entry_1 = journal_entry_0.post(var_0, var_0, var_0)
    journal_entry_0.validate()


def test_case_4():
    bytes_0 = b""
    var_0 = module_1.isum(bytes_0)
    journal_entry_0 = module_0.JournalEntry(var_0, var_0, var_0)
    none_type_0 = journal_entry_0.validate()
    journal_entry_1 = journal_entry_0.post(var_0, var_0, var_0)
    none_type_1 = journal_entry_0.validate()


def test_case_5():
    bytes_0 = b"\x0b\x00"
    var_0 = module_1.isum(bytes_0)
    var_1 = module_2.field(compare=bytes_0)
    journal_entry_0 = module_0.JournalEntry(bytes_0, var_1, var_0)
    journal_entry_1 = journal_entry_0.post(journal_entry_0, var_1, var_0)
    journal_entry_0.validate()
