# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pymonet.validation as module_0


def test_case_0():
    int_0 = 1764
    str_0 = "5"
    validation_0 = module_0.Validation(str_0, str_0)
    var_0 = validation_0.to_maybe()
    var_1 = validation_0.__eq__(int_0)
    int_1 = 1
    validation_1 = module_0.Validation(int_1, int_1)
    validation_1.is_success()


def test_case_1():
    list_0 = []
    validation_0 = module_0.Validation(list_0, list_0)
    var_0 = validation_0.__str__()
    var_0.to_try()


def test_case_2():
    str_0 = "\n        Transform Maybe to Either.\n\n        :returns: Right monad with previous value when Maybe is not empty, in other case Left with None\n        :rtype: Either[A | None]\n        "
    validation_0 = module_0.Validation(str_0, str_0)
    none_type_0 = None
    var_0 = validation_0.to_either()
    var_1 = validation_0.to_lazy()
    validation_0.map(none_type_0)


def test_case_3():
    int_0 = 1764
    str_0 = ""
    validation_0 = module_0.Validation(str_0, str_0)
    var_0 = validation_0.to_maybe()
    var_1 = validation_0.__eq__(int_0)
    var_0.to_maybe()


def test_case_4():
    none_type_0 = None
    validation_0 = module_0.Validation(none_type_0, none_type_0)


def test_case_5():
    none_type_0 = None
    bool_0 = True
    validation_0 = module_0.Validation(none_type_0, bool_0)
    validation_0.is_success()


def test_case_6():
    none_type_0 = None
    complex_0 = -2787.770708 - 1455.172398j
    validation_0 = module_0.Validation(none_type_0, complex_0)
    validation_0.is_fail()


def test_case_7():
    bool_0 = False
    list_0 = [bool_0]
    validation_0 = module_0.Validation(list_0, bool_0)
    var_0 = validation_0.to_box()
    var_1 = var_0.to_either()
    none_type_0 = None
    var_2 = var_0.__eq__(none_type_0)
    validation_0.map(var_0)


def test_case_8():
    float_0 = 121.96444
    none_type_0 = None
    set_0 = {none_type_0, none_type_0, none_type_0, none_type_0}
    bytes_0 = b"1L\xdd\xaab\xf2\xbcf\xf9I\xfc\x0b\xc8"
    validation_0 = module_0.Validation(float_0, bytes_0)
    var_0 = validation_0.to_box()
    validation_1 = module_0.Validation(none_type_0, set_0)
    validation_1.bind(float_0)


def test_case_9():
    complex_0 = -1533.9 + 255.752j
    validation_0 = module_0.Validation(complex_0, complex_0)
    validation_0.ap(complex_0)


def test_case_10():
    float_0 = -993.8
    tuple_0 = ()
    dict_0 = {float_0: tuple_0, float_0: tuple_0, tuple_0: tuple_0}
    none_type_0 = None
    validation_0 = module_0.Validation(dict_0, none_type_0)
    int_0 = 1
    dict_1 = {int_0: int_0, int_0: int_0, int_0: int_0, int_0: int_0}
    validation_1 = module_0.Validation(dict_1, dict_1)
    var_0 = validation_1.is_fail()
    var_1 = validation_1.to_box()


def test_case_11():
    float_0 = 2078.227
    validation_0 = module_0.Validation(float_0, float_0)
    validation_0.to_try()


def test_case_12():
    str_0 = ""
    validation_0 = module_0.Validation(str_0, str_0)
    var_0 = validation_0.__eq__(validation_0)


def test_case_13():
    str_0 = "\n        Transform Maybe to Either.\n\n        :returns: Right monad with previXus value when Maybe is not empty, in other case Left with None\n        :rtype: Either[A | None]H        "
    validation_0 = module_0.Validation(str_0, str_0)
    none_type_0 = None
    var_0 = validation_0.__str__()
    var_1 = validation_0.to_lazy()
    validation_0.map(none_type_0)


def test_case_14():
    str_0 = "\n        Transform Maybe to Either.\n\n        :returns: Right monad with previous value when Maybe is not empty, in other case Left with None\n        :rtype: Either[A | None]\n        "
    validation_0 = module_0.Validation(str_0, str_0)
    var_0 = validation_0.to_either()
    var_1 = validation_0.to_lazy()
    var_2 = validation_0.__eq__(var_0)
    var_3 = var_1.to_box()
    var_4 = validation_0.to_maybe()
    validation_1 = module_0.Validation(var_4, var_3)
    var_3.to_box()


def test_case_15():
    str_0 = ""
    validation_0 = module_0.Validation(str_0, str_0)
    var_0 = validation_0.to_try()
    var_1 = validation_0.to_either()
    var_0.to_maybe()


def test_case_16():
    bytes_0 = b"\xfbi\t\r"
    int_0 = 1011
    str_0 = ""
    validation_0 = module_0.Validation(int_0, str_0)
    none_type_0 = None
    validation_1 = module_0.Validation(none_type_0, none_type_0)
    validation_2 = module_0.Validation(none_type_0, bytes_0)
    var_0 = validation_0.__eq__(validation_2)
    var_1 = validation_0.to_either()
    var_2 = validation_2.to_box()
    var_0.to_maybe()
