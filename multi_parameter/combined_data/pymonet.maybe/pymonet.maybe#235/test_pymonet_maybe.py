# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pymonet.maybe as module_0
import builtins as module_1


def test_case_0():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)


def test_case_1():
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bool_0)


def test_case_2():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    bool_0 = maybe_0.__eq__(maybe_0)
    var_0 = maybe_0.to_either()
    var_1 = maybe_0.to_validation()


def test_case_3():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.to_either()
    var_1 = maybe_0.to_lazy()
    bool_1 = maybe_0.__eq__(var_1)
    var_2 = var_0.to_lazy()
    var_0.to_either()


def test_case_4():
    str_0 = "\r7uJVk(5d"
    str_1 = "JJ3bK\x0c{LOc-t"
    none_type_0 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_0.bind(str_1)
    var_1 = var_0.map(str_0)
    str_2 = "0"
    maybe_1 = module_0.Maybe(str_2, str_2)
    var_2 = maybe_1.bind(str_1)
    var_3 = var_2.to_try()
    var_3.to_try()


def test_case_5():
    int_0 = -300
    bytes_0 = b"\xf4\x1d+\xdfi\x81\xba\xa8\xd5\xbb\x15\xe2\x16\x93{"
    float_0 = -1096.75
    tuple_0 = (float_0,)
    maybe_0 = module_0.Maybe(bytes_0, tuple_0)
    var_0 = maybe_0.bind(int_0)


def test_case_6():
    bool_0 = False
    complex_0 = 150.010979 + 329j
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    maybe_0.bind(complex_0)


def test_case_7():
    int_0 = 201
    int_1 = -1259
    maybe_0 = module_0.Maybe(int_1, int_1)
    var_0 = maybe_0.ap(int_0)
    bool_0 = True
    maybe_1 = module_0.Maybe(bool_0, bool_0)
    var_1 = maybe_1.to_try()
    var_2 = var_1.bind(var_0)
    var_2.to_lazy()


def test_case_8():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    maybe_0.ap(none_type_0)


def test_case_9():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    maybe_1 = module_0.Maybe(maybe_0, maybe_0)
    var_0 = maybe_1.filter(maybe_0)
    maybe_0.filter(maybe_1)


def test_case_10():
    tuple_0 = ()
    maybe_0 = module_0.Maybe(tuple_0, tuple_0)
    var_0 = maybe_0.to_try()
    maybe_0.filter(var_0)


def test_case_11():
    bytes_0 = b'\x94\xc3pO=S\x90"\x16\x17\x8a\x1d\x8f\xd1.U\xf6\\\x8f'
    dict_0 = {bytes_0: bytes_0, bytes_0: bytes_0, bytes_0: bytes_0}
    maybe_0 = module_0.Maybe(dict_0, bytes_0)
    var_0 = maybe_0.to_either()
    var_1 = maybe_0.map(var_0)
    var_2 = maybe_0.map(var_0)
    var_3 = var_2.get_or_else(var_1)
    maybe_1 = module_0.Maybe(var_3, var_0)
    bool_0 = False
    maybe_2 = module_0.Maybe(var_0, bool_0)
    maybe_2.filter(var_3)


def test_case_12():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    bool_0 = maybe_0.__eq__(maybe_0)
    var_0 = maybe_0.get_or_else(none_type_0)
    maybe_0.map(var_0)


def test_case_13():
    none_type_0 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_0.to_box()
    var_1 = maybe_0.to_either()


def test_case_14():
    bytes_0 = b"gL\xb8\xdb\xb0\xfd\xd4\xbee\x80j"
    none_type_0 = None
    maybe_0 = module_0.Maybe(bytes_0, none_type_0)
    var_0 = maybe_0.to_box()


def test_case_15():
    none_type_0 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_0.to_try()
    var_1 = var_0.get_or_else(var_0)
    var_2 = maybe_0.to_lazy()
    var_3 = var_2.bind(none_type_0)
    maybe_1 = module_0.Maybe(bool_0, bool_0)
    var_4 = var_3.ap(maybe_0)
    var_0.to_either()


def test_case_16():
    str_0 = ""
    bool_0 = False
    maybe_0 = module_0.Maybe(str_0, bool_0)
    var_0 = maybe_0.to_lazy()
    var_1 = maybe_0.to_box()
    var_2 = var_0.to_try()
    var_3 = maybe_0.to_lazy()
    var_3.to_lazy()


def test_case_17():
    int_0 = 0
    maybe_0 = module_0.Maybe(int_0, int_0)
    var_0 = maybe_0.to_box()
    bool_0 = False
    maybe_1 = module_0.Maybe(bool_0, bool_0)
    maybe_2 = module_0.Maybe(bool_0, maybe_1)
    var_1 = maybe_1.to_try()


def test_case_18():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    bool_1 = maybe_0.__eq__(bool_0)
    var_0 = maybe_0.to_lazy()
    var_1 = maybe_0.to_validation()
    bool_2 = var_1.__eq__(var_0)
    var_1.ap(var_0)


def test_case_19():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.to_validation()
    var_1 = maybe_0.to_lazy()
    bool_1 = maybe_0.__eq__(var_1)
    var_2 = maybe_0.bind(var_0)
    var_3 = maybe_0.to_lazy()
    var_4 = maybe_0.to_either()
    var_5 = var_4.to_validation()
    var_6 = var_5.to_box()
    var_7 = var_2.filter(var_1)
    var_8 = var_2.bind(bool_1)
    var_9 = var_6.to_either()
    var_10 = var_3.to_either()
    maybe_1 = module_0.Maybe(var_5, bool_0)


def test_case_20():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    maybe_1 = module_0.Maybe(maybe_0, maybe_0)
    var_0 = maybe_1.bind(maybe_0)
    var_1 = var_0.filter(none_type_0)
    bool_0 = var_1.__eq__(var_1)
    maybe_0.filter(maybe_0)


def test_case_21():
    object_0 = module_1.object()
    set_0 = {object_0, object_0, object_0, object_0}
    maybe_0 = module_0.Maybe(set_0, object_0)
    none_type_0 = None
    maybe_1 = module_0.Maybe(none_type_0, none_type_0)
    maybe_1.ap(maybe_1)


def test_case_22():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    var_0 = maybe_0.to_try()
    object_0 = module_1.object()
    maybe_1 = module_0.Maybe(var_0, var_0)
    maybe_2 = module_0.Maybe(var_0, var_0)
    var_1 = maybe_1.bind(maybe_2)
    var_2 = var_1.filter(var_0)
    bool_0 = maybe_2.__eq__(var_2)
    var_3 = maybe_0.to_box()
    maybe_0.filter(maybe_0)
