# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pymonet.maybe as module_0
import builtins as module_1


def test_case_0():
    int_0 = -995
    maybe_0 = module_0.Maybe(int_0, int_0)


def test_case_1():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)


def test_case_2():
    bytes_0 = b"t\xae\xa3t\xac\xf6\x82\x88QjA\x12\xc0\xd3\xa2"
    none_type_0 = None
    maybe_0 = module_0.Maybe(bytes_0, none_type_0)
    var_0 = maybe_0.to_lazy()
    var_1 = var_0.to_try()
    var_2 = var_0.to_either()
    var_3 = var_2.to_box()
    maybe_1 = module_0.Maybe(maybe_0, var_1)
    bool_0 = maybe_0.__eq__(maybe_0)


def test_case_3():
    float_0 = 1695.9
    none_type_0 = None
    none_type_1 = None
    maybe_0 = module_0.Maybe(float_0, none_type_1)
    bool_0 = maybe_0.__eq__(none_type_0)
    list_0 = []
    list_0.ap(float_0)


def test_case_4():
    complex_0 = 2585.866669 - 3186.68619j
    set_0 = {complex_0, complex_0}
    maybe_0 = module_0.Maybe(set_0, complex_0)
    var_0 = maybe_0.to_lazy()
    var_1 = var_0.to_try()
    none_type_0 = None
    var_2 = maybe_0.ap(maybe_0)
    var_3 = maybe_0.map(complex_0)
    bool_0 = var_3.__eq__(var_2)
    var_4 = var_0.ap(var_3)
    maybe_1 = module_0.Maybe(var_1, maybe_0)
    module_1.object(**none_type_0)


def test_case_5():
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    maybe_0.map(bool_0)


def test_case_6():
    bool_0 = True
    int_0 = 1
    maybe_0 = module_0.Maybe(int_0, int_0)
    var_0 = maybe_0.to_either()
    var_1 = var_0.to_validation()
    str_0 = "k\\\rwBXN~Nf8`>zZn|"
    maybe_1 = module_0.Maybe(str_0, str_0)
    var_2 = maybe_1.bind(var_1)
    var_3 = var_2.to_try()
    var_3.ap(bool_0)


def test_case_7():
    bool_0 = False
    bool_1 = True
    maybe_0 = module_0.Maybe(bool_1, bool_0)
    maybe_0.bind(bool_0)


def test_case_8():
    int_0 = -1035
    maybe_0 = module_0.Maybe(int_0, int_0)
    bool_0 = maybe_0.__eq__(int_0)
    var_0 = maybe_0.ap(int_0)
    var_1 = var_0.to_box()
    var_2 = maybe_0.to_lazy()
    var_1.filter(int_0)


def test_case_9():
    dict_0 = {}
    int_0 = -1309
    bool_0 = False
    maybe_0 = module_0.Maybe(int_0, bool_0)
    maybe_0.ap(dict_0)


def test_case_10():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    bool_1 = False
    var_0 = maybe_0.filter(bool_1)
    bool_0.to_either()


def test_case_11():
    none_type_0 = None
    bool_0 = True
    tuple_0 = ()
    maybe_0 = module_0.Maybe(bool_0, none_type_0)
    maybe_0.filter(tuple_0)


def test_case_12():
    bool_0 = False
    bool_1 = True
    maybe_0 = module_0.Maybe(bool_0, bool_1)
    var_0 = maybe_0.get_or_else(bool_0)
    var_0.to_box()


def test_case_13():
    float_0 = 1695.9
    none_type_0 = None
    none_type_1 = None
    maybe_0 = module_0.Maybe(float_0, none_type_1)
    bool_0 = maybe_0.__eq__(none_type_0)
    bool_1 = True
    maybe_1 = module_0.Maybe(bool_0, none_type_1)
    maybe_2 = module_0.Maybe(none_type_0, bool_1)
    var_0 = maybe_1.to_either()
    maybe_3 = module_0.Maybe(var_0, var_0)
    var_1 = maybe_0.to_validation()
    var_2 = maybe_0.to_validation()
    var_3 = maybe_0.to_either()
    var_4 = maybe_2.to_either()
    maybe_4 = module_0.Maybe(maybe_1, var_1)
    var_5 = maybe_4.bind(maybe_3)
    var_6 = var_5.filter(var_0)
    var_7 = maybe_1.get_or_else(var_4)


def test_case_14():
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.to_either()
    bool_1 = True
    maybe_1 = module_0.Maybe(bool_0, bool_1)
    var_1 = maybe_0.to_box()
    bool_2 = False
    var_2 = maybe_1.filter(bool_2)
    var_3 = maybe_0.to_lazy()
    bool_1.to_either()


def test_case_15():
    str_0 = "=,r\\1"
    none_type_0 = None
    bool_0 = False
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_0.to_validation()
    bool_1 = True
    maybe_1 = module_0.Maybe(str_0, bool_1)
    var_1 = maybe_1.to_try()
    var_2 = maybe_1.filter(maybe_1)
    var_3 = var_2.to_lazy()
    var_4 = var_3.map(var_3)
    var_5 = var_2.to_validation()
    var_6 = maybe_1.filter(var_5)
    var_7 = maybe_1.to_box()


def test_case_16():
    bool_0 = True
    bytes_0 = b"t\xae\xa3t\xac\xf6\x82\x88QjA\x12\xc0\xd3\xa2"
    none_type_0 = None
    maybe_0 = module_0.Maybe(bytes_0, none_type_0)
    var_0 = maybe_0.to_lazy()
    var_1 = var_0.to_try()
    var_2 = var_0.ap(bool_0)


def test_case_17():
    int_0 = -1035
    maybe_0 = module_0.Maybe(int_0, int_0)
    var_0 = maybe_0.to_try()


def test_case_18():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    var_0 = maybe_0.to_try()
    var_0.to_box()


def test_case_19():
    int_0 = -1035
    maybe_0 = module_0.Maybe(int_0, int_0)
    var_0 = maybe_0.to_validation()


def test_case_20():
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.to_either()
    var_1 = maybe_0.to_validation()
    bool_1 = True
    maybe_1 = module_0.Maybe(var_1, bool_1)
    var_2 = maybe_0.to_box()
    var_2.map(var_1)


def test_case_21():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.bind(bool_0)
    bool_1 = False
    var_1 = maybe_0.to_lazy()
    var_2 = maybe_0.to_box()
    var_3 = maybe_0.to_validation()
    var_4 = maybe_0.bind(var_1)
    bool_2 = var_2.__eq__(bool_0)
    var_5 = maybe_0.to_validation()
    none_type_0 = None
    bool_3 = var_0.__eq__(maybe_0)
    var_6 = var_5.to_try()
    bool_4 = True
    var_7 = var_0.bind(bool_3)
    var_8 = var_1.ap(var_7)
    maybe_1 = module_0.Maybe(maybe_0, bool_4)
    maybe_2 = module_0.Maybe(bool_1, none_type_0)
    bool_5 = maybe_2.__eq__(var_0)
    var_9 = var_0.ap(var_0)
    var_10 = var_0.get_or_else(var_3)
    maybe_3 = module_0.Maybe(var_2, var_10)
    var_2.bind(var_5)
