# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pymonet.maybe as module_0


def test_case_0():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)


def test_case_1():
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bool_0)


def test_case_2():
    bool_0 = False
    bool_1 = False
    maybe_0 = module_0.Maybe(bool_0, bool_1)
    bool_2 = maybe_0.__eq__(maybe_0)
    int_0 = -2344
    maybe_1 = module_0.Maybe(int_0, int_0)
    maybe_2 = module_0.Maybe(bool_2, int_0)
    bool_3 = maybe_2.__eq__(maybe_1)
    var_0 = maybe_1.to_box()


def test_case_3():
    str_0 = "IVCgZ\\.f;\x0bKiP"
    bool_0 = False
    maybe_0 = module_0.Maybe(str_0, bool_0)
    maybe_1 = module_0.Maybe(str_0, maybe_0)
    none_type_0 = None
    bool_1 = True
    maybe_2 = module_0.Maybe(bool_1, bool_1)
    var_0 = maybe_2.ap(none_type_0)
    maybe_3 = module_0.Maybe(bool_1, none_type_0)
    bool_2 = False
    maybe_4 = module_0.Maybe(var_0, bool_2)
    bool_3 = maybe_4.__eq__(none_type_0)
    var_1 = maybe_2.to_box()


def test_case_4():
    bool_0 = False
    none_type_0 = None
    bool_1 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_1)
    var_0 = maybe_0.to_box()
    var_1 = var_0.to_try()
    bool_2 = True
    maybe_1 = module_0.Maybe(bool_0, bool_2)
    bool_3 = maybe_1.__eq__(maybe_1)
    int_0 = -2344
    maybe_2 = module_0.Maybe(int_0, int_0)
    none_type_1 = None
    var_2 = maybe_1.map(none_type_1)
    var_1.bind(bool_0)


def test_case_5():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    none_type_1 = None
    bool_0 = False
    maybe_1 = module_0.Maybe(none_type_1, bool_0)
    maybe_1.ap(maybe_0)


def test_case_6():
    bool_0 = True
    bool_1 = True
    maybe_0 = module_0.Maybe(bool_0, bool_1)
    var_0 = maybe_0.to_either()
    var_1 = var_0.to_box()
    str_0 = "QSY%KAnlsZPHI@\x0c"
    maybe_1 = module_0.Maybe(str_0, str_0)
    var_2 = maybe_1.bind(var_1)


def test_case_7():
    str_0 = "\n        Returns True when errors list are empty.\n\n        :returns: True for empty errors list\n        :rtype: Boolean\n        "
    none_type_0 = None
    maybe_0 = module_0.Maybe(str_0, none_type_0)
    maybe_0.bind(none_type_0)


def test_case_8():
    none_type_0 = None
    bool_0 = False
    bytes_0 = b"\xb5\xbb\x8eL\xc7\x80\x03q\xbcP\n}"
    maybe_0 = module_0.Maybe(bool_0, bytes_0)
    var_0 = maybe_0.ap(none_type_0)


def test_case_9():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.filter(bool_0)


def test_case_10():
    none_type_0 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_0.filter(maybe_0)
    var_1 = var_0.to_either()
    maybe_1 = module_0.Maybe(none_type_0, none_type_0)
    maybe_1.filter(var_1)


def test_case_11():
    bool_0 = True
    dict_0 = {bool_0: bool_0, bool_0: bool_0}
    maybe_0 = module_0.Maybe(dict_0, bool_0)
    var_0 = maybe_0.to_box()
    str_0 = "!=\x0cT<S*+ky+(e"
    tuple_0 = (maybe_0, str_0)
    maybe_1 = module_0.Maybe(tuple_0, dict_0)
    bytes_0 = b'"\x19o1\xfd_`\x93Mv8KyR'
    var_1 = maybe_0.get_or_else(dict_0)
    var_2 = maybe_0.to_lazy()
    tuple_0.ap(bytes_0)


def test_case_12():
    none_type_0 = None
    set_0 = set()
    bool_0 = False
    bool_1 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_1)
    var_0 = maybe_0.ap(none_type_0)
    maybe_1 = module_0.Maybe(bool_0, bool_0)
    var_1 = maybe_1.get_or_else(set_0)
    var_1.ap(none_type_0)


def test_case_13():
    none_type_0 = None
    bool_0 = False
    dict_0 = {bool_0: bool_0, bool_0: bool_0}
    maybe_0 = module_0.Maybe(dict_0, bool_0)
    var_0 = maybe_0.to_either()
    var_0.get_or_else(none_type_0)


def test_case_14():
    none_type_0 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.to_box()
    bool_1 = var_0.__eq__(none_type_0)


def test_case_15():
    bytes_0 = b"\x8e\x16bR\x8c\xd4\xf7\x19\xc1\xb1\x9fG\xf6lc\x88"
    set_0 = {bytes_0}
    bool_0 = False
    maybe_0 = module_0.Maybe(set_0, bool_0)
    var_0 = maybe_0.to_box()
    var_1 = var_0.to_either()


def test_case_16():
    str_0 = "_s'1\\1vOvio9"
    set_0 = {str_0}
    tuple_0 = (str_0, set_0)
    bool_0 = False
    maybe_0 = module_0.Maybe(tuple_0, bool_0)
    var_0 = maybe_0.to_lazy()
    var_1 = var_0.to_box()
    var_1.to_box()


def test_case_17():
    int_0 = -340
    maybe_0 = module_0.Maybe(int_0, int_0)
    var_0 = maybe_0.to_try()
    var_1 = var_0.filter(int_0)
    var_1.to_box()


def test_case_18():
    tuple_0 = ()
    bool_0 = False
    maybe_0 = module_0.Maybe(tuple_0, bool_0)
    var_0 = maybe_0.to_try()
    maybe_1 = module_0.Maybe(tuple_0, tuple_0)
    maybe_2 = module_0.Maybe(tuple_0, tuple_0)


def test_case_19():
    tuple_0 = ()
    bool_0 = False
    set_0 = {bool_0, bool_0, bool_0, bool_0}
    bytes_0 = b"\\\x8c\x8e\xd3&\x19\xaf\xef\xa6+\x9ad5I\x0b\xf6\xca\x98"
    maybe_0 = module_0.Maybe(bytes_0, bytes_0)
    var_0 = maybe_0.filter(set_0)
    var_1 = maybe_0.to_validation()
    var_1.get_or_else(tuple_0)


def test_case_20():
    none_type_0 = None
    bool_0 = False
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_0.to_validation()
    var_0.filter(none_type_0)


def test_case_21():
    int_0 = 1007
    set_0 = {int_0}
    bool_0 = True
    maybe_0 = module_0.Maybe(set_0, bool_0)
    var_0 = maybe_0.to_lazy()
    var_1 = var_0.bind(set_0)
    var_1.to_validation()


def test_case_22():
    bool_0 = False
    none_type_0 = None
    bool_1 = False
    maybe_0 = module_0.Maybe(none_type_0, bool_1)
    var_0 = maybe_0.to_box()
    bool_2 = False
    maybe_1 = module_0.Maybe(bool_0, bool_2)
    bool_3 = maybe_1.__eq__(maybe_1)
    int_0 = -2344
    maybe_2 = module_0.Maybe(int_0, int_0)
    var_1 = maybe_0.to_validation()
    var_2 = maybe_1.ap(maybe_2)
    bool_4 = True
    maybe_3 = module_0.Maybe(var_0, bool_4)
    maybe_4 = module_0.Maybe(var_0, none_type_0)
    bool_5 = maybe_1.__eq__(var_2)
    var_1.filter(var_1)
