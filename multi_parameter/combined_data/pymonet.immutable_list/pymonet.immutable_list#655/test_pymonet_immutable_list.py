# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pymonet.immutable_list as module_0


def test_case_0():
    immutable_list_0 = module_0.ImmutableList()
    bool_0 = immutable_list_0.__eq__(immutable_list_0)
    immutable_list_1 = immutable_list_0.unshift(bool_0)
    immutable_list_1.find(bool_0)


def test_case_1():
    immutable_list_0 = module_0.ImmutableList()
    none_type_0 = None
    bool_0 = immutable_list_0.__eq__(none_type_0)
    immutable_list_1 = immutable_list_0.unshift(bool_0)
    immutable_list_1.find(bool_0)


def test_case_2():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    immutable_list_1.find(immutable_list_1)


def test_case_3():
    bool_0 = False
    dict_0 = {bool_0: bool_0, bool_0: bool_0}
    int_0 = -1082
    immutable_list_0 = module_0.ImmutableList(is_empty=int_0)
    immutable_list_0.__add__(dict_0)


def test_case_4():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    immutable_list_2 = immutable_list_1.__len__()
    immutable_list_1.find(immutable_list_2)


def test_case_5():
    bool_0 = False
    immutable_list_0 = module_0.ImmutableList(is_empty=bool_0)
    str_0 = immutable_list_0.__str__()


def test_case_6():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    var_0 = immutable_list_1.to_list()
    immutable_list_1.find(var_0)


def test_case_7():
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList(tail=none_type_0, is_empty=none_type_0)
    str_0 = immutable_list_0.__str__()
    var_0 = immutable_list_0.to_list()
    bool_0 = immutable_list_0.__eq__(none_type_0)
    immutable_list_0.map(var_0)


def test_case_8():
    int_0 = -2540
    dict_0 = {int_0: int_0, int_0: int_0}
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.append(int_0)
    var_0 = immutable_list_1.to_list()
    immutable_list_2 = var_0.append(dict_0)
    set_0 = {int_0, int_0, int_0}
    tuple_0 = ()
    immutable_list_3 = module_0.ImmutableList(tuple_0, is_empty=tuple_0)
    immutable_list_4 = module_0.ImmutableList(is_empty=set_0)
    immutable_list_5 = immutable_list_0.unshift(immutable_list_0)
    bool_0 = immutable_list_5.__eq__(immutable_list_2)
    str_0 = immutable_list_5.__str__()
    bool_1 = immutable_list_0.reduce(immutable_list_1, var_0)
    immutable_list_6 = immutable_list_4.unshift(var_0)
    var_1 = immutable_list_5.to_list()
    immutable_list_1.map(immutable_list_3)


def test_case_9():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_0.filter(immutable_list_0)


def test_case_10():
    none_type_0 = None
    none_type_1 = None
    dict_0 = {}
    immutable_list_0 = module_0.ImmutableList(none_type_1, is_empty=dict_0)
    none_type_2 = None
    immutable_list_1 = module_0.ImmutableList(none_type_2, none_type_2)
    immutable_list_2 = immutable_list_1.__add__(immutable_list_0)
    immutable_list_2.filter(none_type_0)


def test_case_11():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.append(immutable_list_0)
    var_0 = immutable_list_0.find(immutable_list_1)
    immutable_list_2 = immutable_list_1.unshift(immutable_list_1)
    immutable_list_2.find(immutable_list_1)


def test_case_12():
    immutable_list_0 = module_0.ImmutableList()
    var_0 = immutable_list_0.reduce(immutable_list_0, immutable_list_0)
    immutable_list_1 = immutable_list_0.unshift(var_0)
    immutable_list_1.find(var_0)


def test_case_13():
    bytes_0 = b"oJ\xefm\x1d\xec\xaa\x80vO|\x93\xf3\xce\x92Xv^"
    none_type_0 = None
    int_0 = 3797
    set_0 = {int_0, int_0}
    none_type_1 = None
    immutable_list_0 = module_0.ImmutableList(set_0, set_0, none_type_1)
    immutable_list_0.reduce(bytes_0, none_type_0)


def test_case_14():
    immutable_list_0 = module_0.ImmutableList()


def test_case_15():
    immutable_list_0 = module_0.ImmutableList()
    var_0 = immutable_list_0.append(immutable_list_0)
    immutable_list_1 = var_0.unshift(var_0)
    immutable_list_1.find(var_0)


def test_case_16():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.unshift(immutable_list_0)
    immutable_list_2 = immutable_list_1.append(immutable_list_0)
    immutable_list_1.find(immutable_list_2)


def test_case_17():
    tuple_0 = ()
    immutable_list_0 = module_0.ImmutableList(tuple_0, is_empty=tuple_0)
    immutable_list_0.find(tuple_0)


def test_case_18():
    tuple_0 = ()
    immutable_list_0 = module_0.ImmutableList(tuple_0, is_empty=tuple_0)
    bool_0 = immutable_list_0.__len__()
    bool_1 = immutable_list_0.__eq__(immutable_list_0)
    immutable_list_0.find(bool_0)


def test_case_19():
    tuple_0 = ()
    immutable_list_0 = module_0.ImmutableList(tuple_0, is_empty=tuple_0)
    immutable_list_1 = immutable_list_0.append(immutable_list_0)
    bool_0 = immutable_list_1.__eq__(immutable_list_0)
    immutable_list_0.find(bool_0)


def test_case_20():
    immutable_list_0 = module_0.ImmutableList()
    immutable_list_1 = immutable_list_0.append(immutable_list_0)
    immutable_list_2 = immutable_list_1.unshift(immutable_list_1)
    bool_0 = immutable_list_2.__eq__(immutable_list_0)
    immutable_list_2.find(immutable_list_1)


def test_case_21():
    bool_0 = False
    int_0 = 1283
    none_type_0 = None
    immutable_list_0 = module_0.ImmutableList(none_type_0, int_0)
    immutable_list_1 = module_0.ImmutableList(int_0)
    immutable_list_1.reduce(bool_0, bool_0)
