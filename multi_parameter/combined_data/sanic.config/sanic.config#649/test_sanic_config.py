# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import sanic.config as module_0
import inspect as module_1


def test_case_0():
    config_0 = module_0.Config()


def test_case_1():
    config_0 = module_0.Config()
    var_0 = module_1.getmembers(config_0)
    var_1 = config_0.__repr__()
    module_0.Config(converters=var_1)


def test_case_2():
    complex_0 = 1423.7 - 2332j
    none_type_0 = None
    config_0 = module_0.Config(env_prefix=none_type_0, keep_alive=complex_0)
    config_0.update_config(none_type_0)


def test_case_3():
    bool_0 = True
    module_0.Config(bool_0)


def test_case_4():
    none_type_0 = None
    config_0 = module_0.Config(none_type_0)
    none_type_1 = config_0.update()
    config_1 = module_0.Config(none_type_0)
    config_2 = module_0.Config(keep_alive=none_type_0, converters=none_type_1)
    config_3 = module_0.Config()
    config_3.__getattr__(config_2)


def test_case_5():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)


def test_case_6():
    config_0 = module_0.Config()
    none_type_0 = config_0.update(**config_0)
    none_type_1 = config_0.register_type(none_type_0)


def test_case_7():
    bytes_0 = b"G}\xc6ZR\x8dC\xf7\x92$\xb3"
    list_0 = [bytes_0]
    config_0 = module_0.Config(converters=list_0)
    list_0.__subclasscheck__(list_0, bytes_0)


def test_case_8():
    str_0 = "2'A"
    config_0 = module_0.Config(env_prefix=str_0)
    int_0 = 429
    list_0 = [int_0, int_0, int_0]
    module_0.DescriptorMeta(list_0, *list_0)


def test_case_9():
    str_0 = "2'A"
    config_0 = module_0.Config(env_prefix=str_0)
    config_0.update_config(str_0)


def test_case_10():
    config_0 = module_0.Config()
    str_0 = "o[sB\nRho)C6"
    str_1 = "LOCAL_CERT_CREATOR"
    config_0.__setitem__(str_1, str_0)


def test_case_11():
    config_0 = module_0.Config()
    none_type_0 = None
    config_0.update_config(none_type_0)


def test_case_12():
    str_0 = "_CONVERTERS"
    str_1 = "_FALLBACK_ERROR_FORMAT"
    str_2 = 'ubvJJ7X{C#G7mS"\\?'
    dict_0 = {str_0: str_0, str_1: str_1, str_2: str_0}
    module_0.Config(dict_0)
