# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import sanic.config as module_0
import inspect as module_1


def test_case_0():
    config_0 = module_0.Config()


def test_case_1():
    int_0 = -2120
    none_type_0 = None
    config_0 = module_0.Config(keep_alive=none_type_0)
    config_1 = module_0.Config(none_type_0, keep_alive=int_0)
    module_0.DescriptorMeta(none_type_0)


def test_case_2():
    dict_0 = {}
    config_0 = module_0.Config(dict_0)
    bool_0 = True
    var_0 = config_0.load_environment_vars()
    none_type_0 = config_0.update()
    config_1 = module_0.Config(env_prefix=var_0)
    none_type_1 = config_1.register_type(bool_0)
    config_0.update(*var_0)


def test_case_3():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)
    config_0.__getattr__(var_0)


def test_case_4():
    none_type_0 = None
    config_0 = module_0.Config(converters=none_type_0)
    str_0 = ". Parameter names cannot use '__'."
    config_0.update_config(str_0)


def test_case_5():
    config_0 = module_0.Config()
    none_type_0 = config_0.register_type(config_0)


def test_case_6():
    none_type_0 = None
    config_0 = module_0.Config(converters=none_type_0)
    var_0 = module_1.isclass(none_type_0)
    none_type_1 = config_0.register_type(config_0)
    float_0 = -4047.2
    config_0.__setitem__(var_0, float_0)


def test_case_7():
    str_0 = "DEPRECATION_FILTER"
    config_0 = module_0.Config(env_prefix=str_0)
    int_0 = -2358
    tuple_0 = (int_0,)
    none_type_0 = config_0.register_type(tuple_0)
    var_0 = config_0.update_config(config_0)
    config_0.__getattr__(tuple_0)


def test_case_8():
    config_0 = module_0.Config()
    float_0 = 2929.5
    module_0.Config(converters=float_0)


def test_case_9():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)


def test_case_10():
    config_0 = module_0.Config()
    none_type_0 = None
    config_0.update_config(none_type_0)


def test_case_11():
    int_0 = -2120
    config_0 = module_0.Config()
    config_1 = module_0.Config(config_0, keep_alive=int_0)
    int_1 = -2358
    none_type_0 = config_1.register_type(config_1)
    config_0.update_config(int_1)


def test_case_12():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)
    config_0.__getattr__(config_0)


def test_case_13():
    none_type_0 = None
    config_0 = module_0.Config(keep_alive=none_type_0)
    none_type_1 = config_0.register_type(none_type_0)
    config_0.register_type(none_type_0)


def test_case_14():
    str_0 = "DEPRECATION_FILTER"
    module_0.Config(env_prefix=str_0, converters=str_0)


def test_case_15():
    int_0 = -2120
    int_1 = -1642
    none_type_0 = None
    bool_0 = True
    str_0 = "lA2U\\"
    config_0 = module_0.Config(keep_alive=bool_0, converters=str_0)
    config_1 = module_0.Config(keep_alive=none_type_0)
    none_type_1 = config_1.register_type(int_1)
    var_0 = config_1.update_config(config_1)
    config_1.update_config(int_0)


def test_case_16():
    int_0 = -2123
    config_0 = module_0.Config()
    str_0 = "NhH?M+F}IXCeL|o0c^"
    none_type_0 = config_0.__setitem__(str_0, int_0)
    str_1 = "LOCAL_CERT_CREATOR"
    config_0.__setitem__(str_1, str_1)


def test_case_17():
    str_0 = ""
    none_type_0 = None
    dict_0 = {str_0: str_0, str_0: none_type_0, str_0: str_0}
    config_0 = module_0.Config(dict_0)
    config_0.load_environment_vars(str_0)
