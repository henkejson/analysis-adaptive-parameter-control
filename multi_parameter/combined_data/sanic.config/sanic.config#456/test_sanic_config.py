# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import sanic.config as module_0


def test_case_0():
    config_0 = module_0.Config()


def test_case_1():
    str_0 = ">K`9gMidX4Q)_C|7>"
    none_type_0 = None
    module_0.Config(keep_alive=none_type_0, converters=str_0)


def test_case_2():
    none_type_0 = None
    config_0 = module_0.Config(env_prefix=none_type_0)
    config_0.update_config(none_type_0)


def test_case_3():
    config_0 = module_0.Config()
    config_0.__getattr__(config_0)


def test_case_4():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)


def test_case_5():
    str_0 = "blueprints"
    bytes_0 = b"f]\xecU\x14\xf9\xa5\xdc{\xca&\xd0"
    config_0 = module_0.Config(converters=bytes_0)
    none_type_0 = config_0.__setitem__(str_0, str_0)
    none_type_1 = None
    config_1 = module_0.Config(none_type_1, keep_alive=none_type_1)
    list_0 = [config_1]
    none_type_2 = config_1.register_type(config_1)
    var_0 = config_1.load_environment_vars()
    none_type_3 = config_1.update(*list_0)
    var_1 = config_1.update_config(config_1)
    config_1.update_config(none_type_1)


def test_case_6():
    none_type_0 = None
    config_0 = module_0.Config(none_type_0, keep_alive=none_type_0)
    list_0 = [config_0]
    none_type_1 = config_0.update(*list_0, **config_0)
    module_0.Config(env_prefix=config_0, converters=list_0)


def test_case_7():
    none_type_0 = None
    config_0 = module_0.Config(none_type_0, keep_alive=none_type_0)
    none_type_1 = config_0.register_type(config_0)
    config_0.update_config(none_type_0)


def test_case_8():
    none_type_0 = None
    config_0 = module_0.Config(none_type_0, keep_alive=none_type_0)
    list_0 = []
    none_type_1 = config_0.update(*list_0)
    var_0 = config_0.update_config(config_0)
    config_0.__getattr__(none_type_0)


def test_case_9():
    str_0 = "_FALLBACK_ERROR_FORMAT"
    none_type_0 = None
    str_1 = "--workers"
    dict_0 = {str_0: none_type_0, str_0: none_type_0, str_1: str_1}
    module_0.Config(dict_0)


def test_case_10():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)
    bytes_0 = b"Failed Dependency"
    config_0.update_config(bytes_0)


def test_case_11():
    none_type_0 = None
    config_0 = module_0.Config(none_type_0, keep_alive=none_type_0)
    list_0 = [config_0]
    none_type_1 = config_0.register_type(config_0)
    none_type_2 = config_0.update(*list_0)
    module_0.Config(config_0, list_0, config_0, converters=list_0)


def test_case_12():
    none_type_0 = None
    config_0 = module_0.Config(none_type_0, keep_alive=none_type_0)
    str_0 = ""
    config_0.load_environment_vars(str_0)
