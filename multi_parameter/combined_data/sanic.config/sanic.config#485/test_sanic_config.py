# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import sanic.config as module_0
import inspect as module_1


def test_case_0():
    config_0 = module_0.Config()


def test_case_1():
    bool_0 = True
    config_0 = module_0.Config(keep_alive=bool_0)
    var_0 = config_0.update_config(config_0)
    config_1 = module_0.Config(var_0, var_0)
    module_0.DescriptorMeta(var_0, *bool_0)


def test_case_2():
    none_type_0 = None
    bool_0 = True
    module_0.Config(none_type_0, bool_0)


def test_case_3():
    float_0 = 1309.159544
    module_0.Config(float_0, float_0, converters=float_0)


def test_case_4():
    config_0 = module_0.Config()
    var_0 = module_1.getmembers(config_0)
    config_0.update_config(var_0)


def test_case_5():
    config_0 = module_0.Config()
    var_0 = module_1.isclass(config_0)
    config_0.update_config(var_0)


def test_case_6():
    str_0 = "S"
    config_0 = module_0.Config(env_prefix=str_0)
    config_0.update_config(str_0)


def test_case_7():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)


def test_case_8():
    config_0 = module_0.Config()
    none_type_0 = None
    none_type_1 = config_0.register_type(none_type_0)
    var_0 = module_1.isclass(config_0)
    var_1 = config_0.update_config(config_0)
    config_0.__setitem__(var_0, config_0)


def test_case_9():
    config_0 = module_0.Config()
    none_type_0 = None
    none_type_1 = config_0.register_type(none_type_0)
    config_0.__getattr__(config_0)


def test_case_10():
    config_0 = module_0.Config()
    none_type_0 = config_0.update(**config_0)
    str_0 = "\n|Ld_4%#]%"
    none_type_1 = config_0.__setitem__(str_0, none_type_0)
    config_0.update_config(none_type_1)


def test_case_11():
    config_0 = module_0.Config()
    str_0 = "L+vWfHao.E_k7^k])2\"'"
    none_type_0 = None
    none_type_1 = config_0.__setitem__(str_0, none_type_0)
    bool_0 = False
    none_type_2 = config_0.register_type(bool_0)
    config_0.register_type(bool_0)


def test_case_12():
    bytes_0 = b"\x01\xbao\xe5b\x84\x98\xad\xc7]\xe8"
    int_0 = -880
    var_0 = module_1.isclass(int_0)
    config_0 = module_0.Config(converters=bytes_0)
    var_0.register(config_0, int_0)


def test_case_13():
    config_0 = module_0.Config()
    bool_0 = False
    none_type_0 = config_0.register_type(bool_0)
    none_type_1 = config_0.update()
    none_type_2 = None
    str_0 = "LOCAL_CERT_CREATOR"
    config_0.__setattr__(str_0, none_type_2)


def test_case_14():
    str_0 = "S"
    config_0 = module_0.Config(env_prefix=str_0)
    var_0 = module_1.getmembers(config_0)
    set_0 = {str_0, str_0}
    var_1 = module_1.isclass(set_0)
    var_1.__setitem__(str_0, var_0)


def test_case_15():
    str_0 = "S"
    config_0 = module_0.Config(env_prefix=str_0)
    int_0 = -1082
    none_type_0 = config_0.register_type(int_0)
    var_0 = config_0.load_environment_vars()
    none_type_1 = config_0.__setitem__(str_0, config_0)
    config_0.load_environment_vars(str_0)


def test_case_16():
    str_0 = "_"
    config_0 = module_0.Config(env_prefix=str_0)
    none_type_0 = None
    config_0.update_config(none_type_0)
