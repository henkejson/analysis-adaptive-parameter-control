# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import sanic.config as module_0
import inspect as module_1


def test_case_0():
    config_0 = module_0.Config()


def test_case_1():
    bool_0 = False
    none_type_0 = None
    config_0 = module_0.Config(env_prefix=none_type_0, keep_alive=bool_0)
    var_0 = module_1.getmembers(bool_0)
    module_0.Config(var_0)


def test_case_2():
    float_0 = 1.1
    bytes_0 = b"\x03\xcaO\xaa<\x0bZT{\xa6\x8c<FH\xfb\x94"
    config_0 = module_0.Config(keep_alive=bytes_0)
    config_0.__getattr__(float_0)


def test_case_3():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)


def test_case_4():
    config_0 = module_0.Config()
    str_0 = ">9iZ_'1"
    config_0.update_config(str_0)


def test_case_5():
    str_0 = "REQUEST_TIMEOUT"
    config_0 = module_0.Config(env_prefix=str_0)
    var_0 = config_0.update_config(config_0)
    bool_0 = False
    none_type_0 = None
    config_0.__setattr__(bool_0, none_type_0)


def test_case_6():
    none_type_0 = None
    config_0 = module_0.Config(none_type_0)
    none_type_1 = None
    dict_0 = {}
    none_type_2 = None
    config_1 = module_0.Config(env_prefix=none_type_2, keep_alive=none_type_1)
    config_2 = module_0.Config(keep_alive=none_type_1)
    config_2.__getattr__(dict_0)


def test_case_7():
    config_0 = module_0.Config()
    bool_0 = False
    config_0.update_config(bool_0)


def test_case_8():
    config_0 = module_0.Config()
    str_0 = ""
    none_type_0 = None
    none_type_1 = config_0.__setitem__(str_0, none_type_0)
    config_0.load_environment_vars(str_0)


def test_case_9():
    config_0 = module_0.Config()
    bool_0 = False
    none_type_0 = config_0.register_type(bool_0)


def test_case_10():
    bytes_0 = b"\x03\xcaO\xaa<\x0bZT{\xa6\x8c<FH\xfb\x94"
    module_0.Config(converters=bytes_0)


def test_case_11():
    str_0 = "websocket.receive"
    set_0 = {str_0}
    tuple_0 = (set_0,)
    config_0 = module_0.Config(converters=tuple_0)
    str_1 = "w.D%g/n=g%."
    config_1 = module_1.getmembers(str_1)
    config_1.update_config(config_1)


def test_case_12():
    config_0 = module_0.Config()
    str_0 = ""
    config_0.load_environment_vars(str_0)


def test_case_13():
    str_0 = "b:DDDl\"/$Y7q!Ut'kY(:"
    config_0 = module_0.Config()
    var_0 = config_0.load_environment_vars(str_0)
    var_1 = config_0.update_config(config_0)
    str_1 = "LOCAL_CERT_CREATOR"
    config_0.__setitem__(str_1, var_1)
