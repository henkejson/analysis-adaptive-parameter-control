# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import sanic.config as module_0
import inspect as module_1


def test_case_0():
    config_0 = module_0.Config()


def test_case_1():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)
    config_1 = module_0.Config(var_0, converters=config_0)


def test_case_2():
    str_0 = "HdnJctvUm\x0c@0c"
    config_0 = module_0.Config(env_prefix=str_0)


def test_case_3():
    str_0 = "T"
    dict_0 = {str_0: str_0, str_0: str_0}
    config_0 = module_0.Config(dict_0)


def test_case_4():
    none_type_0 = None
    str_0 = "88\x0ccj D\nyYa>\t"
    bool_0 = True
    config_0 = module_0.Config(none_type_0, str_0, bool_0, converters=none_type_0)
    config_0.__getattr__(none_type_0)


def test_case_5():
    none_type_0 = None
    config_0 = module_0.Config(env_prefix=none_type_0)
    var_0 = module_1.isdatadescriptor(none_type_0)
    config_1 = module_0.Config(config_0, converters=config_0)


def test_case_6():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)
    config_0.update_config(var_0)


def test_case_7():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)


def test_case_8():
    none_type_0 = None
    dict_0 = {none_type_0: none_type_0, none_type_0: none_type_0}
    config_0 = module_0.Config(converters=dict_0)
    config_0.__getattr__(config_0)


def test_case_9():
    bool_0 = True
    none_type_0 = None
    config_0 = module_0.Config(keep_alive=bool_0, converters=none_type_0)


def test_case_10():
    bytes_0 = b"\xbb"
    bytes_1 = b"?"
    config_0 = module_0.Config(keep_alive=bytes_1, converters=bytes_1)
    config_0.update_config(bytes_0)


def test_case_11():
    str_0 = "0T"
    config_0 = module_0.Config()
    none_type_0 = config_0.__setattr__(str_0, str_0)
    bool_0 = False
    config_1 = module_0.Config(env_prefix=bool_0)
    var_0 = config_1.load_environment_vars()
    none_type_1 = config_1.__setitem__(str_0, none_type_0)
    none_type_2 = None
    config_2 = module_0.Config(bool_0, str_0, none_type_2, converters=none_type_2)
    config_2.__setattr__(none_type_2, config_2)


def test_case_12():
    bool_0 = False
    config_0 = module_0.Config(env_prefix=bool_0)
    str_0 = ":v4\r@[[U\nP%hqra*7=RE"
    module_0.Config(bool_0, str_0, str_0, converters=str_0)


def test_case_13():
    none_type_0 = None
    config_0 = module_0.Config(env_prefix=none_type_0)
    str_0 = "LOCAL_CERT_CREATOR"
    config_0.__setattr__(str_0, str_0)
