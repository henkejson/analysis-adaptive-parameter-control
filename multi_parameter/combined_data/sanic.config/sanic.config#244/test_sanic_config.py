# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import sanic.config as module_0
import inspect as module_1
import sanic.utils as module_2
import sanic.constants as module_3


def test_case_0():
    config_0 = module_0.Config()


def test_case_1():
    none_type_0 = None
    config_0 = module_0.Config(none_type_0, none_type_0)
    config_1 = module_0.Config(none_type_0, keep_alive=config_0, converters=config_0)
    config_2 = module_0.Config(config_0, keep_alive=none_type_0)
    config_0.load_environment_vars(none_type_0)


def test_case_2():
    none_type_0 = None
    config_0 = module_0.Config(none_type_0, none_type_0)
    config_0.__getattr__(none_type_0)


def test_case_3():
    config_0 = module_0.Config()
    var_0 = module_1.getmembers(config_0)
    config_0.update_config(var_0)


def test_case_4():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)


def test_case_5():
    str_0 = "x-forwarded-port"
    config_0 = module_0.Config()
    var_0 = module_1.isclass(config_0)
    none_type_0 = config_0.register_type(var_0)
    none_type_1 = config_0.__setitem__(str_0, none_type_0)


def test_case_6():
    bool_0 = False
    set_0 = {bool_0, bool_0, bool_0}
    config_0 = module_0.Config()
    str_0 = "R{d2'`MDO`"
    config_1 = module_0.Config(env_prefix=str_0)
    none_type_0 = config_0.register_type(set_0)
    str_1 = "jQyc"
    module_2.load_module_from_file_location(set_0, str_1)


def test_case_7():
    none_type_0 = None
    config_0 = module_0.Config(none_type_0, none_type_0)
    var_0 = config_0.load_environment_vars()
    config_1 = module_0.Config()
    var_1 = config_1.update_config(config_1)
    local_cert_creator_0 = module_3.LocalCertCreator.TRUSTME
    module_0.Config(local_cert_creator_0, converters=var_0)


def test_case_8():
    none_type_0 = None
    config_0 = module_0.Config(keep_alive=none_type_0, converters=none_type_0)
    config_0.update_config(none_type_0)


def test_case_9():
    none_type_0 = None
    config_0 = module_0.Config(none_type_0, none_type_0)
    var_0 = config_0.load_environment_vars()
    config_0.__getattr__(config_0)


def test_case_10():
    none_type_0 = None
    config_0 = module_0.Config(none_type_0, none_type_0)
    config_1 = module_0.Config()
    config_2 = module_0.Config(env_prefix=none_type_0, converters=config_1)
    var_0 = config_1.update_config(config_1)
    var_1 = module_1.getmembers(config_0)
    config_0.update_config(var_0)


def test_case_11():
    str_0 = "hK07XSv4p<@~Fq*Ugp|"
    dict_0 = {str_0: str_0}
    config_0 = module_0.Config(dict_0)
    config_0.update_config(str_0)


def test_case_12():
    config_0 = module_0.Config()
    var_0 = module_1.isclass(config_0)
    none_type_0 = config_0.register_type(var_0)
    config_0.register_type(var_0)
