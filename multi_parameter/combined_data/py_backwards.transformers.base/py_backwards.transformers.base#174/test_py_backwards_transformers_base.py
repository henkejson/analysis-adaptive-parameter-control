# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import py_backwards.transformers.base as module_0
import typed_ast._ast3 as module_1
import typed_ast.ast3 as module_2


def test_case_0():
    none_type_0 = None
    base_import_rewrite_0 = module_0.BaseImportRewrite(none_type_0)


def test_case_1():
    name_0 = module_1.Name()
    base_import_rewrite_0 = module_0.BaseImportRewrite(name_0)
    list_0 = [name_0, base_import_rewrite_0]
    import_from_0 = module_1.ImportFrom(*list_0)
    base_import_rewrite_0.visit(import_from_0)


def test_case_2():
    name_0 = module_1.Name()
    none_type_0 = None
    base_import_rewrite_0 = module_0.BaseImportRewrite(none_type_0)
    base_import_rewrite_1 = module_0.BaseImportRewrite(name_0)
    base_node_transformer_0 = module_0.BaseNodeTransformer(none_type_0)
    list_0 = [none_type_0]
    not_in_0 = module_1.NotIn()
    import_from_0 = module_1.ImportFrom(*list_0)
    base_import_rewrite_1.visit_ImportFrom(import_from_0)


def test_case_3():
    name_0 = module_1.Name()
    var_0 = module_2.walk(name_0)
    base_import_rewrite_0 = module_0.BaseImportRewrite(name_0)
    list_0 = [var_0, var_0]
    import_from_0 = module_1.ImportFrom(*list_0)
    base_import_rewrite_0.visit(import_from_0)


def test_case_4():
    name_0 = module_1.Name()
    var_0 = module_2.iter_fields(name_0)
    base_import_rewrite_0 = module_0.BaseImportRewrite(name_0)
    list_0 = [var_0, var_0]
    import_from_0 = module_1.ImportFrom(*list_0)
    var_1 = base_import_rewrite_0.visit(import_from_0)
