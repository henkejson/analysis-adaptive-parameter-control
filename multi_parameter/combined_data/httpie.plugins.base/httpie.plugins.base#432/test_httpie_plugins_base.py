# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import httpie.plugins.base as module_0


def test_case_0():
    auth_plugin_0 = module_0.AuthPlugin()


def test_case_1():
    dict_0 = {}
    str_0 = "g3zEr:#E"
    auth_plugin_0 = module_0.AuthPlugin(**dict_0)
    auth_plugin_0.get_auth(password=str_0)


def test_case_2():
    transport_plugin_0 = module_0.TransportPlugin()
    transport_plugin_0.get_adapter()


def test_case_3():
    str_0 = "m,kG@1<'VN9^d"
    converter_plugin_0 = module_0.ConverterPlugin(str_0)


def test_case_4():
    bytes_0 = b"\n\n"
    str_0 = "  "
    converter_plugin_0 = module_0.ConverterPlugin(str_0)
    converter_plugin_0.convert(bytes_0)


def test_case_5():
    module_0.FormatterPlugin()


def test_case_6():
    str_0 = "Querystring parameter to the URL, e.g limit==50"
    tuple_0 = (str_0, str_0)
    str_1 = "format_options"
    dict_0 = {str_1: str_1, str_1: str_1, str_1: str_1, str_1: str_1}
    formatter_plugin_0 = module_0.FormatterPlugin(**dict_0)
    str_2 = formatter_plugin_0.format_metadata(tuple_0)
    str_3 = "']kB!_,XQa}_\x0c\rt3"
    none_type_0 = None
    module_0.AuthPlugin(*str_3, **none_type_0)


def test_case_7():
    str_0 = "format_options"
    dict_0 = {str_0: str_0, str_0: str_0, str_0: str_0, str_0: str_0}
    formatter_plugin_0 = module_0.FormatterPlugin(**dict_0)
    str_1 = "method"
    str_2 = "^-b~%q/"
    formatter_plugin_1 = module_0.FormatterPlugin(**dict_0)
    str_3 = formatter_plugin_1.format_headers(str_1)
    transport_plugin_0 = module_0.TransportPlugin()
    dict_1 = {str_2: str_2}
    module_0.FormatterPlugin(**dict_1)


def test_case_8():
    str_0 = "format_options"
    dict_0 = {str_0: str_0, str_0: str_0, str_0: str_0, str_0: str_0}
    formatter_plugin_0 = module_0.FormatterPlugin(**dict_0)
    str_1 = "']kB!_,XQa}_\x0c\rt3"
    str_2 = formatter_plugin_0.format_body(str_0, str_0)
    auth_plugin_0 = module_0.AuthPlugin()
    auth_plugin_0.get_auth(str_1, str_1)
