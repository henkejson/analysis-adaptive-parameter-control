# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import codetiming._timer as module_0
import codetiming._timers as module_1


def test_case_0():
    timer_error_0 = module_0.TimerError()


def test_case_1():
    timers_0 = module_1.Timers()
    var_0 = timers_0.__iter__()
    var_1 = var_0.__repr__()
    timer_0 = module_0.Timer(var_0, initial_text=var_0)
    float_arg_0 = module_0.FloatArg()
    timer_1 = timer_0.__enter__()
    timer_0.start()


def test_case_2():
    timer_0 = module_0.Timer()
    var_0 = timer_0.__call__(timer_0)
    timer_1 = timer_0.__enter__()
    module_0.FloatArg(*var_0)


def test_case_3():
    timer_0 = module_0.Timer()
    none_type_0 = timer_0.start()
    list_0 = [none_type_0, timer_0, timer_0, none_type_0]
    none_type_1 = timer_0.__exit__(*list_0)
    list_1 = [none_type_0, timer_0, timer_0]
    timer_error_0 = module_0.TimerError()
    timer_0.__exit__(*list_1)


def test_case_4():
    timer_0 = module_0.Timer()
    none_type_0 = timer_0.start()
    float_0 = timer_0.stop()


def test_case_5():
    timer_0 = module_0.Timer()
    none_type_0 = timer_0.start()
    list_0 = timer_0.__repr__()
    none_type_1 = timer_0.__exit__(*list_0)
    timer_error_0 = module_0.TimerError()


def test_case_6():
    str_0 = "@~'\x0c@"
    timer_0 = module_0.Timer(str_0, initial_text=str_0)
    timer_1 = timer_0.__enter__()
    timer_0.start()


def test_case_7():
    none_type_0 = None
    timer_0 = module_0.Timer(none_type_0, logger=none_type_0)
    none_type_1 = timer_0.start()
    none_type_2 = timer_0.__exit__()
    timer_1 = timer_0.__enter__()
    none_type_3 = timer_0.__exit__()
    timer_1.__exit__()


def test_case_8():
    timers_0 = module_1.Timers()
    var_0 = timers_0.__iter__()
    var_1 = var_0.__repr__()
    timer_0 = module_0.Timer(var_0, initial_text=var_0)
    float_arg_0 = module_0.FloatArg()
    timer_1 = timer_0.__enter__()
    none_type_0 = timer_1.__exit__()
    none_type_1 = timer_0.start()
    none_type_2 = None
    timer_2 = module_0.Timer(text=timer_0, initial_text=none_type_2)
    none_type_3 = timer_2.start()
    float_arg_1 = module_0.FloatArg()
    float_0 = timer_2.stop()
    timer_3 = timer_2.__enter__()
    timer_1.count(timer_1)


def test_case_9():
    timers_0 = module_1.Timers()
    var_0 = timers_0.values()
    var_1 = var_0.__iter__()
    timer_0 = module_0.Timer(var_1, initial_text=var_1)
    timer_1 = timer_0.__enter__()
    timer_2 = module_0.Timer(initial_text=timer_0)
    none_type_0 = timer_2.start()
    float_arg_0 = module_0.FloatArg()
    var_2 = timer_1.__repr__()
    float_0 = var_2.count(var_2)
    timer_0.__enter__()
