# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import codetiming._timer as module_0
import dataclasses as module_1


def test_case_0():
    timer_0 = module_0.Timer()


def test_case_1():
    str_0 = "7\x0b@~"
    timer_0 = module_0.Timer(str_0)
    timer_1 = timer_0.__enter__()
    timer_0.start()


def test_case_2():
    none_type_0 = None
    timer_0 = module_0.Timer(none_type_0)
    var_0 = module_1.field(compare=timer_0, kw_only=timer_0)
    timer_1 = timer_0.__enter__()


def test_case_3():
    timer_0 = module_0.Timer()
    timer_0.stop()


def test_case_4():
    none_type_0 = None
    timer_0 = module_0.Timer(none_type_0)
    var_0 = timer_0.__repr__()
    var_1 = timer_0.__repr__()
    timer_1 = module_0.Timer(logger=var_0)
    timer_0.__exit__()


def test_case_5():
    none_type_0 = None
    timer_0 = module_0.Timer(logger=none_type_0)
    timer_1 = timer_0.__enter__()
    str_0 = "o"
    timer_2 = module_0.Timer(initial_text=str_0)
    float_0 = timer_1.stop()
    timer_3 = timer_2.__enter__()


def test_case_6():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    float_0 = timer_1.stop()


def test_case_7():
    str_0 = "\x0b@~"
    timer_0 = module_0.Timer(initial_text=str_0)
    none_type_0 = timer_0.start()
    var_0 = timer_0.__repr__()
    float_0 = timer_0.stop()


def test_case_8():
    none_type_0 = None
    timer_0 = module_0.Timer(none_type_0, initial_text=none_type_0)
    str_0 = "775@o"
    none_type_1 = timer_0.start()
    timer_1 = module_0.Timer(str_0)
    none_type_2 = timer_1.start()
    float_0 = timer_1.stop()


def test_case_9():
    str_0 = "7\x0b@~"
    float_0 = 2767.87
    timer_0 = module_0.Timer(text=str_0, initial_text=float_0, logger=str_0)
    timer_0.start()


def test_case_10():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    float_0 = timer_0.stop()
    timer_2 = module_0.Timer(text=timer_1, initial_text=timer_1, logger=timer_1)
    timer_3 = timer_2.__enter__()
    none_type_0 = timer_0.start()
    float_1 = timer_2.stop()
    timer_4 = timer_2.__enter__()


def test_case_11():
    none_type_0 = None
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    float_0 = timer_1.stop()
    timer_2 = module_0.Timer(timer_1, none_type_0, timer_0)
    timer_3 = timer_2.__enter__()
    none_type_1 = timer_1.start()
    timer_2.start()
