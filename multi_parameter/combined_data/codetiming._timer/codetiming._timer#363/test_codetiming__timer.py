# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import codetiming._timer as module_0
import dataclasses as module_1


def test_case_0():
    timer_error_0 = module_0.TimerError()


def test_case_1():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    float_0 = timer_1.stop()


def test_case_2():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    float_0 = timer_1.stop()
    timer_0.stop()


def test_case_3():
    timer_0 = module_0.Timer()
    timer_0.__exit__()


def test_case_4():
    none_type_0 = None
    str_0 = ""
    timer_0 = module_0.Timer(none_type_0, str_0)
    none_type_1 = None
    timer_1 = module_0.Timer(none_type_1, none_type_1, logger=none_type_1)
    timer_2 = module_0.Timer()
    none_type_2 = timer_1.start()
    var_0 = timer_2.__repr__()
    var_1 = var_0.__eq__(timer_1)
    timer_error_0 = module_0.TimerError()
    var_2 = module_1.field(
        default_factory=none_type_1, hash=none_type_1, kw_only=none_type_1
    )
    float_0 = timer_1.stop()
    var_2.total(none_type_0)


def test_case_5():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    float_0 = timer_1.stop()
    none_type_0 = timer_0.start()
    timer_0.__enter__()


def test_case_6():
    str_0 = "SY4nf"
    timer_0 = module_0.Timer(str_0, str_0)
    timer_1 = timer_0.__enter__()
    float_0 = timer_1.stop()
    var_0 = timer_0.__repr__()
    var_1 = var_0.__repr__()
    var_2 = timer_1.__eq__(float_0)
    var_0.stop()


def test_case_7():
    str_0 = "\t}F5V`5QE*n<bWy"
    timer_0 = module_0.Timer(text=str_0)
    float_arg_0 = module_0.FloatArg()
    timer_1 = module_0.Timer(initial_text=str_0, logger=str_0)
    var_0 = timer_1.__call__(timer_1)
    var_1 = timer_1.__repr__()
    timer_1.__enter__()


def test_case_8():
    timer_0 = module_0.Timer()
    timer_1 = module_0.Timer(text=timer_0, initial_text=timer_0)
    timer_2 = timer_1.__enter__()
    var_0 = timer_1.__call__(timer_1)
    float_0 = timer_2.stop()
    timer_3 = timer_1.__enter__()
    var_1 = timer_2.__repr__()
    timer_3.start()


def test_case_9():
    float_arg_0 = module_0.FloatArg()
    str_0 = "SY4nf"
    timer_0 = module_0.Timer(float_arg_0, initial_text=float_arg_0)
    timer_1 = timer_0.__enter__()
    var_0 = timer_1.__repr__()
    float_0 = timer_0.stop()
    var_1 = timer_1.__repr__()
    str_0.start()
