# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import codetiming._timer as module_0


def test_case_0():
    timer_0 = module_0.Timer()


def test_case_1():
    none_type_0 = None
    timer_0 = module_0.Timer(none_type_0)
    timer_1 = timer_0.__enter__()


def test_case_2():
    timer_0 = module_0.Timer()
    timer_0.stop()


def test_case_3():
    bytes_0 = b"\x98\x8d\x98\x18\x94\xbc"
    none_type_0 = None
    timer_0 = module_0.Timer(none_type_0, logger=bytes_0)
    none_type_1 = timer_0.start()
    timer_0.__exit__()


def test_case_4():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    float_0 = timer_0.stop()
    timer_2 = timer_0.__enter__()
    timer_0.start()


def test_case_5():
    timer_0 = module_0.Timer()
    none_type_0 = timer_0.start()
    float_0 = timer_0.stop()


def test_case_6():
    timer_0 = module_0.Timer()
    timer_1 = module_0.Timer(initial_text=timer_0)
    none_type_0 = timer_1.start()
    timer_0.__exit__()


def test_case_7():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    timer_2 = module_0.Timer(initial_text=timer_1)
    none_type_0 = timer_2.start()
    float_arg_0 = module_0.FloatArg()
    none_type_1 = timer_0.__exit__()
    timer_3 = timer_0.__enter__()
    bytes_0 = b"\x98\x8d\xc3\x18\x94\xf5"
    timer_4 = module_0.Timer(bytes_0)
    none_type_2 = timer_4.start()
    none_type_3 = timer_4.__exit__()
    float_arg_1 = module_0.FloatArg()


def test_case_8():
    timer_0 = module_0.Timer()
    var_0 = timer_0.__call__(timer_0)
    var_1 = timer_0.__eq__(timer_0)
    none_type_0 = None
    var_2 = var_0.__eq__(none_type_0)
    timer_1 = timer_0.__enter__()
    timer_2 = module_0.Timer(initial_text=timer_1)
    none_type_1 = timer_2.start()
    float_arg_0 = module_0.FloatArg()
    none_type_2 = timer_0.__exit__()
    timer_3 = module_0.Timer(none_type_2, initial_text=none_type_1, logger=none_type_2)
    none_type_3 = timer_3.start()
    none_type_4 = timer_3.__exit__()
    timer_4 = module_0.Timer(text=none_type_4, initial_text=var_0)
    var_1.__enter__()


def test_case_9():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    float_0 = timer_0.stop()
    str_0 = ".Lc$0w?c,CWZ"
    timer_2 = module_0.Timer(text=str_0, initial_text=str_0)
    none_type_0 = timer_2.start()
    timer_0.__exit__()


def test_case_10():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    float_0 = timer_0.stop()
    timer_error_0 = timer_0.__repr__()
    timer_2 = module_0.Timer(timer_error_0, initial_text=float_0)
    none_type_0 = timer_2.start()
    timer_0.__exit__()


def test_case_11():
    timer_0 = module_0.Timer()
    var_0 = timer_0.__call__(timer_0)
    timer_1 = timer_0.__enter__()
    timer_2 = module_0.Timer(text=var_0, initial_text=timer_1)
    none_type_0 = timer_2.start()
    timer_2.__exit__()
