# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import codetiming._timer as module_0


def test_case_0():
    timer_0 = module_0.Timer()


def test_case_1():
    timer_0 = module_0.Timer()
    none_type_0 = timer_0.start()


def test_case_2():
    timer_0 = module_0.Timer()
    none_type_0 = timer_0.start()
    none_type_1 = timer_0.__exit__()
    var_0 = timer_0.__call__(none_type_1)
    timer_0.__exit__()


def test_case_3():
    timer_0 = module_0.Timer()
    none_type_0 = timer_0.start()
    none_type_1 = timer_0.__exit__()


def test_case_4():
    timer_0 = module_0.Timer()
    none_type_0 = timer_0.start()
    timer_0.__enter__()


def test_case_5():
    timer_0 = module_0.Timer()
    none_type_0 = timer_0.start()
    str_0 = "`PSMf2M\ri8UoGk&Y@+"
    none_type_1 = None
    list_0 = [none_type_1, none_type_1]
    none_type_2 = timer_0.__exit__(*list_0)
    timer_1 = module_0.Timer(text=str_0)
    timer_2 = module_0.Timer(none_type_1, initial_text=str_0)
    timer_3 = timer_0.__enter__()
    none_type_3 = timer_2.start()
    timer_4 = timer_1.__enter__()
    none_type_4 = None
    var_0 = timer_2.__repr__()
    timer_5 = module_0.Timer(logger=none_type_4)
    timer_6 = timer_5.__enter__()
    var_1 = timer_5.__repr__()
    none_type_5 = timer_5.__exit__()
    var_2 = timer_1.__call__(timer_6)
    none_type_6 = timer_2.__exit__()


def test_case_6():
    timer_0 = module_0.Timer()
    timer_1 = module_0.Timer(initial_text=timer_0, logger=timer_0)
    none_type_0 = timer_1.start()
    float_0 = timer_1.stop()


def test_case_7():
    str_0 = "?u^z\\Y%c:>Z("
    timer_0 = module_0.Timer(str_0)
    var_0 = timer_0.__eq__(str_0)
    none_type_0 = None
    var_1 = timer_0.__eq__(none_type_0)
    timer_1 = timer_0.__enter__()
    float_0 = timer_0.stop()
    var_2 = timer_0.__eq__(timer_1)
    var_3 = timer_1.__repr__()
    var_2.stop()


def test_case_8():
    timer_0 = module_0.Timer()
    none_type_0 = timer_0.start()
    str_0 = "`PSMf2M\ri8UoGk&Y@+"
    none_type_1 = None
    list_0 = [none_type_1, none_type_1]
    none_type_2 = timer_0.__exit__(*list_0)
    timer_1 = module_0.Timer(text=str_0)
    timer_2 = module_0.Timer(none_type_1, initial_text=str_0)
    timer_3 = timer_0.__eq__(timer_1)
    timer_4 = timer_2.__enter__()
    timer_2.start()


def test_case_9():
    int_0 = -1030
    bool_0 = True
    timer_0 = module_0.Timer(bool_0, initial_text=int_0)
    none_type_0 = timer_0.start()
    float_arg_0 = module_0.FloatArg()
