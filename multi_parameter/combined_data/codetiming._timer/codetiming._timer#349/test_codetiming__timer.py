# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import codetiming._timer as module_0
import dataclasses as module_1


def test_case_0():
    float_arg_0 = module_0.FloatArg()


def test_case_1():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()


def test_case_2():
    timer_0 = module_0.Timer()
    float_0 = timer_0.__call__(timer_0)
    timer_0.__exit__()


def test_case_3():
    timer_0 = module_0.Timer()
    float_arg_0 = module_0.FloatArg()
    none_type_0 = timer_0.start()
    var_0 = timer_0.__call__(timer_0)
    timer_0.start()


def test_case_4():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    none_type_0 = timer_1.__exit__()


def test_case_5():
    none_type_0 = None
    timer_0 = module_0.Timer(logger=none_type_0)
    var_0 = timer_0.__eq__(timer_0)
    none_type_1 = timer_0.start()
    timer_0.stdev(timer_0)


def test_case_6():
    float_arg_0 = module_0.FloatArg()
    timer_0 = module_0.Timer(text=float_arg_0, initial_text=float_arg_0)
    timer_1 = timer_0.__enter__()


def test_case_7():
    str_0 = "{n#cni+:\t="
    timer_0 = module_0.Timer(str_0)
    var_0 = timer_0.__repr__()
    timer_1 = timer_0.__enter__()
    none_type_0 = None
    timer_2 = module_0.Timer(text=none_type_0, logger=none_type_0)
    float_0 = timer_0.stop()


def test_case_8():
    timer_0 = module_0.Timer()
    float_arg_0 = timer_0.__repr__()
    timer_1 = timer_0.__enter__()
    timer_2 = module_0.Timer(initial_text=float_arg_0)
    none_type_0 = timer_1.__exit__()
    timer_2.__enter__()


def test_case_9():
    timer_0 = module_0.Timer()
    float_arg_0 = module_0.FloatArg()
    timer_1 = timer_0.__enter__()
    str_0 = "8n=YpEbl[G[z.6p&T"
    none_type_0 = None
    timer_2 = module_0.Timer(str_0, timer_0, logger=none_type_0)
    none_type_1 = timer_1.__exit__()
    timer_3 = timer_2.__enter__()
    float_0 = timer_2.stop()
    timer_3.__exit__()


def test_case_10():
    timer_0 = module_0.Timer()
    float_arg_0 = module_0.FloatArg()
    timer_1 = timer_0.__enter__()
    timer_2 = module_0.Timer(timer_0, timer_1, float_arg_0)
    none_type_0 = timer_1.__exit__()
    timer_3 = timer_2.__enter__()
    var_0 = module_1.dataclass(repr=float_arg_0)
    var_0.__ior__(timer_3)


def test_case_11():
    timer_0 = module_0.Timer()
    float_arg_0 = module_0.FloatArg()
    timer_1 = timer_0.__enter__()
    timer_2 = module_0.Timer(text=timer_1)
    none_type_0 = timer_1.__exit__()
    timer_3 = timer_2.__enter__()
    float_0 = timer_2.stop()
    timer_0.__exit__()
