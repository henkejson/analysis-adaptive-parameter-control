# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pymonet.maybe as module_0
import typing as module_1
import builtins as module_2


def test_case_0():
    int_0 = 949
    maybe_0 = module_0.Maybe(int_0, int_0)


def test_case_1():
    int_0 = 0
    maybe_0 = module_0.Maybe(int_0, int_0)


def test_case_2():
    none_type_0 = None
    bytes_0 = b"\x8a\x1b\x96\xb0\xa5UPd\xc2y9="
    maybe_0 = module_0.Maybe(bytes_0, bytes_0)
    bool_0 = maybe_0.__eq__(maybe_0)
    var_0 = maybe_0.to_box()
    bool_1 = maybe_0.__eq__(maybe_0)
    bool_2 = maybe_0.__eq__(bool_0)
    var_1 = maybe_0.to_validation()
    var_2 = maybe_0.map(var_1)
    var_3 = maybe_0.to_box()
    var_4 = maybe_0.to_box()
    var_5 = maybe_0.ap(none_type_0)
    var_1.bind(var_1)


def test_case_3():
    int_0 = -744
    dict_0 = {int_0: int_0, int_0: int_0}
    bytes_0 = b"Ee\x7f\xd5\xd7\x8b_\x81R\xe8i\xd41\x7f)\xe6\xc1\x1a"
    set_0 = {bytes_0, bytes_0}
    none_type_0 = None
    maybe_0 = module_0.Maybe(set_0, none_type_0)
    bool_0 = maybe_0.__eq__(dict_0)


def test_case_4():
    bytes_0 = b""
    bool_0 = True
    maybe_0 = module_0.Maybe(bytes_0, bool_0)
    var_0 = maybe_0.to_try()
    var_1 = maybe_0.map(var_0)
    var_2 = var_1.filter(maybe_0)
    bool_1 = False
    none_type_0 = None
    maybe_1 = module_0.Maybe(bool_1, none_type_0)
    var_3 = maybe_1.to_try()
    var_3.to_box()


def test_case_5():
    none_type_0 = None
    bytes_0 = b"~o_\xdd%\xf3\x7f\xb4\xbb\xad\xe6\xb4w\x195"
    none_type_1 = None
    maybe_0 = module_0.Maybe(bytes_0, none_type_1)
    maybe_0.map(none_type_0)


def test_case_6():
    bytes_0 = b"\xacyo\xe8\x19S\x04\xfc\xe2P\x95OH\xbcWD\xc7\xf6\xce\x18"
    maybe_0 = module_0.Maybe(bytes_0, bytes_0)
    none_type_0 = None
    var_0 = maybe_0.map(none_type_0)
    var_1 = maybe_0.to_box()
    var_2 = var_1.to_try()
    var_3 = maybe_0.filter(var_2)
    var_4 = maybe_0.ap(maybe_0)
    bool_0 = var_2.__eq__(var_2)
    var_5 = maybe_0.map(var_4)
    var_6 = maybe_0.ap(maybe_0)
    var_7 = var_4.to_lazy()
    var_8 = maybe_0.bind(var_4)
    var_9 = var_4.ap(var_2)
    bool_1 = var_5.__eq__(var_8)
    var_10 = var_5.filter(var_5)
    var_11 = var_6.to_box()
    var_12 = var_5.to_validation()
    var_13 = var_6.map(var_11)
    var_14 = var_7.to_box()
    var_12.filter(none_type_0)


def test_case_7():
    str_0 = "\n        Transform Either to Try.\n\n        :returns: Lazy monad with function returning previous value\n        :rtype: Lazy[Function() -> A]\n        "
    bool_0 = True
    bool_1 = False
    maybe_0 = module_0.Maybe(bool_0, bool_1)
    maybe_0.bind(str_0)


def test_case_8():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    bool_0 = True
    maybe_1 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_1.ap(none_type_0)
    var_1 = maybe_1.filter(var_0)
    maybe_0.filter(bool_0)


def test_case_9():
    bool_0 = False
    set_0 = {bool_0}
    bool_1 = True
    maybe_0 = module_0.Maybe(set_0, bool_1)
    var_0 = maybe_0.to_either()
    set_1 = set()
    none_type_0 = None
    maybe_1 = module_0.Maybe(set_1, none_type_0)
    var_1 = maybe_1.ap(var_0)


def test_case_10():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    bool_0 = True
    maybe_1 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_1.filter(maybe_0)
    maybe_0.filter(bool_0)


def test_case_11():
    generic_0 = module_1.Generic()
    object_0 = module_2.object()
    maybe_0 = module_0.Maybe(object_0, object_0)
    var_0 = maybe_0.get_or_else(generic_0)
    var_0.to_try()


def test_case_12():
    list_0 = []
    object_0 = module_2.object()
    bool_0 = False
    maybe_0 = module_0.Maybe(object_0, bool_0)
    var_0 = maybe_0.to_either()
    bool_1 = maybe_0.__eq__(list_0)
    var_1 = maybe_0.get_or_else(bool_1)


def test_case_13():
    tuple_0 = ()
    bool_0 = True
    maybe_0 = module_0.Maybe(tuple_0, bool_0)
    var_0 = maybe_0.to_box()


def test_case_14():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    var_0 = maybe_0.to_try()
    var_1 = maybe_0.to_box()
    var_2 = maybe_0.to_box()
    maybe_0.bind(var_1)


def test_case_15():
    generic_0 = module_1.Generic()
    none_type_0 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_0.to_lazy()
    var_0.filter(generic_0)


def test_case_16():
    bool_0 = False
    bytes_0 = b"R"
    none_type_0 = None
    maybe_0 = module_0.Maybe(bytes_0, none_type_0)
    var_0 = maybe_0.to_lazy()
    var_1 = var_0.bind(bool_0)
    var_2 = maybe_0.to_either()
    var_2.ap(bool_0)


def test_case_17():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    bool_0 = False
    bool_1 = maybe_0.__eq__(bool_0)
    var_0 = maybe_0.to_validation()
    var_1 = var_0.to_try()
    var_2 = var_0.to_box()
    var_3 = var_2.to_try()
    maybe_0.bind(var_0)


def test_case_18():
    bytes_0 = b"\xacyo\xe8\x19S\x04\xfc\xe2P\x95OH\xbcWD\xc7\xf6\xce\x18"
    maybe_0 = module_0.Maybe(bytes_0, bytes_0)
    none_type_0 = None
    var_0 = maybe_0.map(none_type_0)
    var_1 = maybe_0.to_box()
    var_2 = var_1.to_try()
    var_3 = maybe_0.ap(maybe_0)
    bool_0 = var_2.__eq__(var_2)
    var_4 = maybe_0.map(var_3)
    var_5 = maybe_0.ap(maybe_0)
    var_6 = var_3.to_lazy()
    var_7 = var_3.ap(var_2)
    var_8 = var_3.to_lazy()
    bool_1 = var_4.__eq__(var_7)
    var_9 = var_4.filter(var_4)
    var_10 = var_4.to_validation()
    var_11 = var_5.map(bool_0)
    var_12 = var_6.to_box()
    var_10.filter(none_type_0)


def test_case_19():
    float_0 = -2050.55039
    none_type_0 = None
    maybe_0 = module_0.Maybe(float_0, none_type_0)
    var_0 = maybe_0.to_lazy()
    var_1 = var_0.to_try()
    var_1.map(none_type_0)


def test_case_20():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    bool_0 = False
    maybe_1 = module_0.Maybe(bool_0, bool_0)
    bool_1 = maybe_1.__eq__(bool_0)
    bool_2 = maybe_0.__eq__(maybe_0)
    var_0 = maybe_0.to_try()
    maybe_1.map(none_type_0)


def test_case_21():
    bytes_0 = b"\xacy\xb5\xe8\x19S\x04\xfc\xe2P\x95OH\xbcWD\xc7\xf6\xce\x18"
    maybe_0 = module_0.Maybe(bytes_0, bytes_0)
    var_0 = maybe_0.to_box()
    var_1 = var_0.to_try()
    var_2 = maybe_0.filter(var_1)
    var_3 = maybe_0.ap(maybe_0)
    bool_0 = var_1.__eq__(var_1)
    var_4 = maybe_0.map(var_3)
    var_5 = var_3.to_lazy()
    var_6 = var_3.ap(var_1)
    var_7 = var_0.to_validation()
    var_8 = var_3.to_either()
    bool_1 = var_2.__eq__(maybe_0)
    var_9 = var_3.to_try()
    bool_2 = var_3.__eq__(var_7)
    bool_3 = False
    maybe_1 = module_0.Maybe(var_6, bool_3)
    maybe_1.filter(bool_3)
