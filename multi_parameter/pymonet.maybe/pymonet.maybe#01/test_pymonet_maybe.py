# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pymonet.maybe as module_0


def test_case_0():
    int_0 = 985
    var_0 = module_0.Maybe(int_0, int_0)


def test_case_1():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)


def test_case_2():
    str_0 = "R^@v"
    bool_0 = False
    maybe_0 = module_0.Maybe(str_0, str_0)
    bool_1 = maybe_0.__eq__(maybe_0)
    var_0 = maybe_0.bind(str_0)
    bool_2 = var_0.__eq__(bool_0)
    var_1 = maybe_0.to_lazy()
    var_2 = maybe_0.get_or_else(str_0)
    var_3 = maybe_0.to_validation()
    var_2.map(maybe_0)


def test_case_3():
    str_0 = "R^@v"
    bool_0 = True
    maybe_0 = module_0.Maybe(str_0, str_0)
    bool_1 = maybe_0.__eq__(maybe_0)
    var_0 = maybe_0.bind(str_0)
    var_1 = maybe_0.to_try()
    var_2 = var_0.map(var_1)
    bool_2 = var_0.__eq__(bool_0)
    var_3 = maybe_0.to_lazy()
    var_4 = var_2.to_box()
    var_5 = maybe_0.get_or_else(str_0)
    var_6 = maybe_0.to_validation()
    var_5.map(maybe_0)


def test_case_4():
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    maybe_0.ap(maybe_0)


def test_case_5():
    str_0 = "YD5_x=)*}uT0\nY\x0b&1"
    maybe_0 = module_0.Maybe(str_0, str_0)
    var_0 = maybe_0.filter(maybe_0)
    var_1 = maybe_0.get_or_else(maybe_0)
    maybe_1 = module_0.Maybe(str_0, str_0)
    var_2 = maybe_1.ap(maybe_1)
    var_3 = maybe_1.bind(var_2)


def test_case_6():
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.to_lazy()
    maybe_1 = module_0.Maybe(maybe_0, maybe_0)
    maybe_0.bind(bool_0)


def test_case_7():
    list_0 = []
    none_type_0 = None
    maybe_0 = module_0.Maybe(list_0, list_0)
    maybe_0.ap(none_type_0)


def test_case_8():
    bool_0 = True
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_0.filter(none_type_0)
    var_1 = maybe_0.to_either()
    var_2 = var_0.to_box()


def test_case_9():
    int_0 = 0
    maybe_0 = module_0.Maybe(int_0, int_0)
    var_0 = maybe_0.get_or_else(maybe_0)
    maybe_0.filter(var_0)


def test_case_10():
    bool_0 = True
    none_type_0 = None
    bool_1 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_1)
    var_0 = maybe_0.get_or_else(maybe_0)
    var_1 = maybe_0.filter(bool_0)


def test_case_11():
    int_0 = 1584
    bool_0 = True
    bool_1 = False
    maybe_0 = module_0.Maybe(bool_0, bool_1)
    var_0 = maybe_0.to_either()
    var_1 = var_0.to_lazy()
    var_2 = maybe_0.get_or_else(int_0)


def test_case_12():
    list_0 = []
    none_type_0 = None
    bool_0 = False
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_0.to_box()
    var_0.ap(list_0)


def test_case_13():
    int_0 = 2767
    maybe_0 = module_0.Maybe(int_0, int_0)
    var_0 = maybe_0.to_try()


def test_case_14():
    bool_0 = False
    none_type_0 = None
    maybe_0 = module_0.Maybe(bool_0, none_type_0)
    var_0 = maybe_0.to_try()


def test_case_15():
    bool_0 = True
    none_type_0 = None
    bool_1 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_1)
    var_0 = maybe_0.to_validation()
    maybe_1 = module_0.Maybe(bool_0, none_type_0)
    maybe_2 = module_0.Maybe(none_type_0, none_type_0)
    var_1 = maybe_1.to_either()
    var_2 = maybe_1.to_validation()


def test_case_16():
    int_0 = 2767
    int_0.to_try()


def test_case_17():
    str_0 = "R^@v"
    bool_0 = False
    maybe_0 = module_0.Maybe(str_0, str_0)
    bool_1 = maybe_0.__eq__(maybe_0)
    var_0 = maybe_0.bind(str_0)
    bool_2 = var_0.__eq__(bool_0)
    var_1 = maybe_0.to_lazy()
    var_2 = var_1.to_box()
    bool_3 = True
    maybe_1 = module_0.Maybe(var_0, bool_3)
    maybe_2 = module_0.Maybe(var_1, bool_2)
    var_3 = var_0.ap(bool_1)
    var_4 = var_0.filter(var_3)


def test_case_18():
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.to_lazy()
    none_type_0 = None
    bool_1 = False
    maybe_1 = module_0.Maybe(none_type_0, bool_1)
    var_1 = maybe_1.get_or_else(var_0)
    none_type_1 = None
    maybe_2 = module_0.Maybe(none_type_1, none_type_1)
    var_2 = maybe_2.to_lazy()
    var_3 = var_2.to_box()
    var_3.bind(var_1)


def test_case_19():
    int_0 = -1731
    maybe_0 = module_0.Maybe(int_0, int_0)
    var_0 = maybe_0.filter(int_0)
    none_type_0 = None
    var_1 = maybe_0.ap(none_type_0)
    bool_0 = maybe_0.__eq__(var_0)
    var_2 = var_0.to_lazy()
    maybe_1 = module_0.Maybe(int_0, int_0)
    var_3 = var_1.to_try()
    var_4 = var_1.to_box()
    var_5 = var_1.map(var_2)
    float_0 = -2434.0
    var_6 = var_0.to_box()
    var_7 = var_5.bind(float_0)
    var_8 = var_0.filter(var_7)
    bool_1 = maybe_1.__eq__(int_0)
    var_9 = var_7.to_validation()
    bool_2 = var_0.__eq__(var_3)
    var_10 = maybe_0.bind(maybe_0)
    var_11 = var_0.bind(maybe_0)
    none_type_1 = None
    var_12 = var_0.get_or_else(none_type_1)
    bool_3 = var_1.__eq__(float_0)
    var_13 = var_11.to_either()
    var_14 = maybe_1.map(var_13)
    var_15 = var_4.to_lazy()
    var_16 = maybe_1.to_either()
    maybe_2 = module_0.Maybe(var_7, var_12)
    maybe_2.filter(var_12)


def test_case_20():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    bool_0 = maybe_0.__eq__(maybe_0)
    maybe_0.ap(none_type_0)
