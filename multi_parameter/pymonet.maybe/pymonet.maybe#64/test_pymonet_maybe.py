# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pymonet.maybe as module_0
import builtins as module_1


def test_case_0():
    float_0 = -725.102
    maybe_0 = module_0.Maybe(float_0, float_0)


def test_case_1():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)


def test_case_2():
    bool_0 = True
    none_type_0 = None
    bool_1 = False
    bool_2 = False
    maybe_0 = module_0.Maybe(bool_1, bool_2)
    maybe_1 = module_0.Maybe(bool_0, none_type_0)
    bool_3 = maybe_1.__eq__(maybe_1)


def test_case_3():
    bytes_0 = b"p\x9c|A\xc6\xca\xdfj\x8cc{\x8dI\xee\xed\xb4G\x8egh"
    maybe_0 = module_0.Maybe(bytes_0, bytes_0)
    var_0 = maybe_0.to_box()
    bool_0 = maybe_0.__eq__(var_0)
    maybe_1 = module_0.Maybe(var_0, var_0)
    var_1 = maybe_0.filter(bytes_0)
    var_2 = var_1.to_try()
    var_3 = maybe_1.to_box()


def test_case_4():
    none_type_0 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    maybe_1 = module_0.Maybe(maybe_0, bool_0)
    var_0 = maybe_0.map(bool_0)
    bool_1 = var_0.__eq__(none_type_0)
    var_1 = maybe_1.to_try()


def test_case_5():
    bool_0 = False
    bool_1 = False
    maybe_0 = module_0.Maybe(bool_0, bool_1)
    maybe_0.ap(maybe_0)


def test_case_6():
    bool_0 = True
    int_0 = -2942
    none_type_0 = None
    maybe_0 = module_0.Maybe(int_0, bool_0)
    var_0 = maybe_0.bind(none_type_0)
    bool_1 = True
    maybe_1 = module_0.Maybe(int_0, bool_1)
    var_1 = maybe_1.map(bool_0)
    var_2 = var_1.filter(maybe_1)


def test_case_7():
    str_0 = "6-pJ\\cb(!\tyE4l<YP"
    bool_0 = True
    maybe_0 = module_0.Maybe(str_0, bool_0)
    var_0 = maybe_0.ap(maybe_0)
    var_1 = maybe_0.to_validation()
    none_type_0 = None
    var_2 = maybe_0.bind(str_0)
    var_3 = maybe_0.to_validation()
    bool_1 = maybe_0.__eq__(bool_0)
    maybe_1 = module_0.Maybe(bool_0, none_type_0)


def test_case_8():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    maybe_0.ap(none_type_0)


def test_case_9():
    object_0 = module_1.object()
    none_type_0 = None
    maybe_0 = module_0.Maybe(object_0, none_type_0)
    none_type_1 = None
    maybe_1 = module_0.Maybe(object_0, none_type_1)
    var_0 = maybe_1.to_try()
    maybe_1.filter(var_0)


def test_case_10():
    str_0 = "6-pJ\\cb(!\tyE4l<YP"
    bool_0 = True
    maybe_0 = module_0.Maybe(str_0, bool_0)
    var_0 = maybe_0.ap(maybe_0)
    none_type_0 = None
    var_1 = maybe_0.ap(none_type_0)
    var_2 = maybe_0.bind(var_1)
    var_3 = maybe_0.to_validation()
    var_4 = maybe_0.filter(var_1)
    bool_1 = maybe_0.__eq__(bool_0)
    var_5 = var_1.get_or_else(bool_1)


def test_case_11():
    bool_0 = True
    none_type_0 = None
    maybe_0 = module_0.Maybe(bool_0, none_type_0)
    var_0 = maybe_0.to_lazy()
    var_1 = var_0.to_box()
    var_2 = var_1.to_either()
    bool_1 = False
    maybe_1 = module_0.Maybe(bool_0, bool_1)
    var_3 = maybe_1.get_or_else(bool_0)
    maybe_2 = module_0.Maybe(bool_0, bool_0)
    var_4 = maybe_2.get_or_else(maybe_2)
    var_5 = maybe_2.map(bool_0)


def test_case_12():
    none_type_0 = None
    int_0 = -2321
    maybe_0 = module_0.Maybe(int_0, int_0)
    var_0 = maybe_0.to_either()
    var_0.filter(none_type_0)


def test_case_13():
    none_type_0 = None
    bool_0 = False
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_0.to_either()
    var_0.to_either()


def test_case_14():
    bool_0 = True
    none_type_0 = None
    bool_1 = False
    maybe_0 = module_0.Maybe(none_type_0, bool_1)
    var_0 = maybe_0.to_box()
    var_0.get_or_else(bool_0)


def test_case_15():
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.to_validation()
    maybe_1 = module_0.Maybe(var_0, maybe_0)
    var_1 = maybe_1.to_lazy()
    var_2 = var_1.to_either()
    var_3 = var_2.to_validation()
    maybe_2 = module_0.Maybe(var_3, maybe_1)
    var_4 = maybe_2.to_validation()
    var_5 = var_4.to_either()


def test_case_16():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.to_try()
    var_0.to_either()


def test_case_17():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.to_validation()


def test_case_18():
    str_0 = "\n    Immutable list is data structure that doesn't allow to mutate instances\n    "
    bool_0 = False
    maybe_0 = module_0.Maybe(str_0, bool_0)
    var_0 = maybe_0.to_validation()
    var_1 = var_0.to_lazy()


def test_case_19():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    maybe_1 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_1.filter(maybe_1)
    bool_1 = True
    var_1 = maybe_1.bind(bool_1)
    var_2 = var_1.ap(maybe_1)
    var_3 = var_0.to_lazy()
    maybe_2 = module_0.Maybe(bool_1, var_3)
    var_4 = var_2.to_try()
    var_5 = maybe_1.to_either()
    var_6 = maybe_1.get_or_else(var_1)
    var_7 = var_2.to_try()
    var_8 = var_1.map(var_6)
    var_9 = var_8.bind(var_7)
    var_10 = maybe_1.map(var_7)
    bool_2 = var_9.__eq__(bool_0)
    var_11 = var_5.to_box()
    var_12 = var_1.filter(var_2)
    bool_3 = maybe_2.__eq__(var_10)
    var_13 = var_6.to_validation()
    var_14 = var_3.to_validation()
    var_15 = var_12.ap(var_5)
    var_16 = maybe_0.to_try()
    var_17 = var_1.filter(var_10)
    var_18 = var_1.ap(var_1)
    var_19 = var_2.get_or_else(bool_1)
    var_20 = var_2.to_validation()
    var_21 = var_10.to_lazy()
    bool_4 = var_0.__eq__(var_11)
    var_22 = var_12.get_or_else(maybe_0)


def test_case_20():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    maybe_1 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_1.filter(maybe_1)
    bool_1 = True
    var_1 = maybe_1.bind(bool_1)
    bool_2 = var_0.__eq__(maybe_1)
    var_2 = var_1.ap(maybe_1)
    var_3 = var_0.to_lazy()
    bool_3 = maybe_1.__eq__(bool_0)
    var_4 = maybe_1.to_either()
    var_5 = maybe_1.get_or_else(var_1)
    var_6 = var_2.to_try()
    var_7 = var_4.bind(var_6)
    var_8 = var_5.map(var_3)
    bool_4 = maybe_0.__eq__(var_1)
    var_9 = var_7.to_box()
    var_10 = var_2.to_either()
    var_11 = var_8.filter(var_10)
    var_12 = var_3.to_box()
    var_13 = var_3.to_try()
    var_14 = var_1.bind(var_2)
    var_15 = var_11.to_try()
    var_13.map(var_9)


def test_case_21():
    object_0 = module_1.object()
    none_type_0 = None
    maybe_0 = module_0.Maybe(object_0, none_type_0)
    maybe_0.bind(maybe_0)
