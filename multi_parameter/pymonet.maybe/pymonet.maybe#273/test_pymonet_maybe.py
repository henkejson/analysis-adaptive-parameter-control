# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pymonet.maybe as module_0
import typing as module_1
import builtins as module_2


def test_case_0():
    complex_0 = -2038.2387 - 3020.789j
    var_0 = module_0.Maybe(complex_0, complex_0)


def test_case_1():
    bytes_0 = b""
    maybe_0 = module_0.Maybe(bytes_0, bytes_0)


def test_case_2():
    generic_0 = module_1.Generic()
    bool_0 = True
    maybe_0 = module_0.Maybe(generic_0, bool_0)
    complex_0 = 223 - 1160.617j
    bool_1 = True
    maybe_1 = module_0.Maybe(complex_0, bool_1)
    none_type_0 = None
    bool_2 = True
    bool_3 = False
    maybe_2 = module_0.Maybe(bool_2, bool_3)
    var_0 = maybe_2.to_either()
    bool_4 = maybe_2.__eq__(bool_2)
    var_1 = var_0.to_try()
    var_2 = maybe_2.get_or_else(none_type_0)
    var_0.get_or_else(var_1)


def test_case_3():
    bool_0 = False
    list_0 = [bool_0, bool_0, bool_0, bool_0]
    maybe_0 = module_0.Maybe(list_0, list_0)
    var_0 = maybe_0.to_lazy()
    var_1 = maybe_0.map(maybe_0)
    var_2 = maybe_0.bind(var_0)
    var_3 = maybe_0.to_try()
    var_4 = maybe_0.to_either()
    var_5 = maybe_0.map(var_3)
    var_6 = maybe_0.get_or_else(var_3)
    var_3.to_either()


def test_case_4():
    tuple_0 = ()
    dict_0 = {tuple_0: tuple_0, tuple_0: tuple_0, tuple_0: tuple_0}
    float_0 = -1266.0
    bool_0 = False
    maybe_0 = module_0.Maybe(float_0, bool_0)
    maybe_0.map(dict_0)


def test_case_5():
    none_type_0 = None
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    maybe_0.bind(none_type_0)


def test_case_6():
    bool_0 = True
    bool_1 = False
    bool_2 = True
    maybe_0 = module_0.Maybe(bool_1, bool_2)
    var_0 = maybe_0.ap(bool_0)
    maybe_1 = module_0.Maybe(var_0, var_0)


def test_case_7():
    none_type_0 = None
    none_type_1 = None
    bool_0 = False
    maybe_0 = module_0.Maybe(none_type_1, none_type_0)
    maybe_1 = module_0.Maybe(none_type_1, bool_0)
    maybe_1.ap(none_type_0)


def test_case_8():
    bool_0 = True
    none_type_0 = None
    bool_1 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_1)
    var_0 = maybe_0.filter(bool_0)


def test_case_9():
    bytes_0 = b"\xc6\x11\x1d\x0c\xf3\xb7V"
    bool_0 = True
    maybe_0 = module_0.Maybe(bytes_0, bool_0)
    var_0 = maybe_0.filter(maybe_0)
    none_type_0 = None
    maybe_1 = module_0.Maybe(none_type_0, none_type_0)
    maybe_1.filter(none_type_0)


def test_case_10():
    bool_0 = False
    none_type_0 = None
    none_type_1 = None
    maybe_0 = module_0.Maybe(none_type_1, none_type_1)
    var_0 = maybe_0.get_or_else(bool_0)
    var_0.bind(none_type_0)


def test_case_11():
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.to_either()
    var_1 = var_0.to_validation()
    var_2 = var_1.to_lazy()
    var_2.to_lazy()


def test_case_12():
    dict_0 = {}
    none_type_0 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_0.to_validation()
    bool_1 = True
    maybe_1 = module_0.Maybe(var_0, bool_1)
    var_1 = maybe_1.to_box()
    var_2 = var_1.to_lazy()
    var_2.get_or_else(dict_0)


def test_case_13():
    none_type_0 = None
    none_type_1 = None
    none_type_2 = None
    maybe_0 = module_0.Maybe(none_type_2, none_type_0)
    var_0 = maybe_0.to_box()
    var_1 = var_0.to_either()
    var_1.filter(none_type_1)


def test_case_14():
    dict_0 = {}
    bool_0 = False
    maybe_0 = module_0.Maybe(dict_0, bool_0)
    var_0 = maybe_0.to_lazy()
    tuple_0 = ()
    maybe_0.ap(tuple_0)


def test_case_15():
    set_0 = set()
    maybe_0 = module_0.Maybe(set_0, set_0)
    var_0 = maybe_0.to_try()


def test_case_16():
    bytes_0 = b"\x87\xbd\xcd\xed5Hn\x93\xe7\xdf\xb0"
    bool_0 = True
    maybe_0 = module_0.Maybe(bytes_0, bool_0)
    var_0 = maybe_0.to_validation()
    var_1 = var_0.to_box()
    var_1.to_box()


def test_case_17():
    bool_0 = False
    object_0 = module_2.object()
    none_type_0 = None
    bytes_0 = b"I2A\xac\x8b\xb0\xd2"
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_0.to_validation()
    bool_1 = False
    maybe_1 = module_0.Maybe(bytes_0, bool_1)
    var_1 = var_0.to_either()
    maybe_0.filter(var_0)


def test_case_18():
    complex_0 = 4189.6 + 4410.89349j
    complex_0.to_lazy()


def test_case_19():
    none_type_0 = None
    bytes_0 = b"\xb3\xc6p\x8aj}}H\xd54\x99 m"
    maybe_0 = module_0.Maybe(none_type_0, bytes_0)
    var_0 = maybe_0.to_validation()
    var_1 = maybe_0.to_lazy()
    var_2 = maybe_0.map(var_1)
    var_3 = var_1.to_try()
    var_4 = maybe_0.map(bytes_0)
    var_0.get_or_else(var_2)


def test_case_20():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    none_type_0 = None
    var_0 = maybe_0.ap(none_type_0)
    float_0 = 3218.0
    var_1 = maybe_0.to_box()
    float_1 = -331.4
    maybe_1 = module_0.Maybe(float_0, float_0)
    var_2 = maybe_1.to_lazy()
    var_3 = maybe_0.bind(var_2)
    var_4 = var_2.to_either()
    var_5 = var_1.to_either()
    var_6 = maybe_1.filter(var_5)
    var_7 = var_4.to_try()
    none_type_1 = None
    maybe_2 = module_0.Maybe(none_type_1, none_type_1)
    bool_1 = maybe_2.__eq__(maybe_2)
    var_8 = maybe_2.to_box()
    none_type_2 = None
    maybe_3 = module_0.Maybe(none_type_2, float_1)
    var_9 = maybe_3.bind(var_8)
    var_10 = var_9.filter(float_0)
    var_11 = var_10.to_lazy()
    var_12 = var_11.to_box()
    var_13 = var_12.to_either()
    maybe_4 = module_0.Maybe(float_0, float_1)


def test_case_21():
    bool_0 = False
    list_0 = [bool_0, bool_0, bool_0, bool_0]
    maybe_0 = module_0.Maybe(list_0, list_0)
    var_0 = maybe_0.ap(list_0)
    var_1 = maybe_0.map(maybe_0)
    var_2 = var_0.ap(var_1)
    bool_1 = maybe_0.__eq__(list_0)
    var_3 = var_1.to_box()
    var_4 = maybe_0.map(var_3)
    var_5 = var_4.bind(var_0)
    var_6 = var_1.filter(var_0)
    var_7 = var_1.to_either()
    var_8 = var_7.bind(var_5)
    maybe_1 = module_0.Maybe(bool_1, bool_1)
    maybe_2 = module_0.Maybe(var_2, var_2)
    bool_2 = maybe_2.__eq__(var_6)
    var_9 = maybe_0.to_box()
    var_10 = var_4.ap(var_4)
    var_11 = var_5.ap(var_0)
    var_12 = var_2.bind(maybe_0)
    var_13 = var_8.to_validation()
    bool_3 = var_0.__eq__(var_0)
    var_14 = var_4.bind(var_7)
    var_15 = var_3.to_try()
    maybe_1.filter(var_12)


def test_case_22():
    bool_0 = False
    none_type_0 = None
    maybe_0 = module_0.Maybe(bool_0, none_type_0)
    var_0 = maybe_0.to_lazy()
    var_1 = var_0.to_either()
    var_1.to_either()
