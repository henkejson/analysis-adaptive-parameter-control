# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pymonet.maybe as module_0


def test_case_0():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)


def test_case_1():
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bool_0)


def test_case_2():
    complex_0 = 4848.541033 + 826.79306j
    bool_0 = False
    maybe_0 = module_0.Maybe(complex_0, bool_0)
    maybe_1 = module_0.Maybe(complex_0, bool_0)
    int_0 = -380
    bool_1 = maybe_0.__eq__(maybe_0)
    bool_2 = maybe_1.__eq__(int_0)
    var_0 = maybe_1.to_try()
    var_1 = maybe_1.to_either()
    var_0.filter(var_1)


def test_case_3():
    none_type_0 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.ap(none_type_0)
    var_1 = maybe_0.to_box()
    var_2 = var_1.ap(maybe_0)
    var_3 = maybe_0.to_lazy()
    var_4 = maybe_0.to_try()
    var_5 = var_0.filter(var_3)
    var_6 = var_0.filter(var_0)
    bool_1 = False
    maybe_1 = module_0.Maybe(bool_1, bool_1)
    var_7 = var_5.to_validation()
    var_8 = var_0.get_or_else(var_5)
    maybe_1.filter(var_3)


def test_case_4():
    str_0 = ""
    maybe_0 = module_0.Maybe(str_0, str_0)
    maybe_0.map(str_0)


def test_case_5():
    none_type_0 = None
    none_type_1 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(none_type_1, bool_0)
    var_0 = maybe_0.to_either()
    var_1 = maybe_0.filter(var_0)
    var_2 = maybe_0.bind(var_0)
    var_3 = maybe_0.ap(none_type_0)
    var_4 = maybe_0.to_box()
    var_5 = maybe_0.to_either()
    var_6 = maybe_0.to_lazy()
    var_7 = var_3.filter(var_6)
    var_8 = var_3.filter(var_3)
    bool_1 = False
    maybe_1 = module_0.Maybe(bool_1, bool_1)
    var_9 = maybe_1.to_try()
    var_10 = var_3.get_or_else(var_7)
    maybe_1.filter(var_6)


def test_case_6():
    none_type_0 = None
    int_0 = 1
    bool_0 = False
    maybe_0 = module_0.Maybe(int_0, bool_0)
    maybe_0.bind(none_type_0)


def test_case_7():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.ap(maybe_0)
    var_1 = var_0.filter(bool_0)
    bool_1 = False
    maybe_1 = module_0.Maybe(bool_1, bool_1)
    maybe_1.filter(bool_0)


def test_case_8():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    float_0 = 1452.98
    maybe_0.ap(float_0)


def test_case_9():
    none_type_0 = None
    bytes_0 = b"}\x07\x94\n\x92\xac\xba\xc5k\x87N\xc1"
    bool_0 = True
    maybe_0 = module_0.Maybe(bytes_0, bool_0)
    var_0 = maybe_0.get_or_else(none_type_0)


def test_case_10():
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.get_or_else(bool_0)


def test_case_11():
    bool_0 = False
    bool_1 = True
    maybe_0 = module_0.Maybe(bool_0, bool_1)
    var_0 = maybe_0.to_either()
    var_1 = maybe_0.to_validation()
    var_2 = maybe_0.to_box()
    var_2.get_or_else(bool_0)


def test_case_12():
    bool_0 = True
    none_type_0 = None
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.to_box()
    var_0.bind(none_type_0)


def test_case_13():
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    bool_1 = True
    var_0 = maybe_0.get_or_else(bool_1)
    var_1 = maybe_0.to_box()


def test_case_14():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.ap(maybe_0)
    var_1 = maybe_0.to_box()
    var_2 = maybe_0.to_lazy()
    var_3 = var_0.filter(var_2)
    bool_1 = False
    maybe_1 = module_0.Maybe(bool_1, bool_1)
    var_4 = var_0.get_or_else(var_3)
    maybe_1.filter(var_2)


def test_case_15():
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    bool_1 = True
    var_0 = maybe_0.get_or_else(bool_1)
    var_1 = maybe_0.to_lazy()
    var_2 = maybe_0.to_box()


def test_case_16():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.ap(maybe_0)
    var_1 = maybe_0.to_try()
    var_2 = var_0.filter(bool_0)
    bool_1 = False
    maybe_1 = module_0.Maybe(bool_1, bool_1)
    maybe_1.filter(bool_0)


def test_case_17():
    int_0 = -1281
    none_type_0 = None
    maybe_0 = module_0.Maybe(int_0, none_type_0)
    var_0 = maybe_0.to_try()


def test_case_18():
    float_0 = 1073.03
    none_type_0 = None
    none_type_1 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(none_type_1, bool_0)
    var_0 = maybe_0.ap(none_type_0)
    var_1 = maybe_0.to_box()
    var_2 = maybe_0.to_lazy()
    var_3 = maybe_0.to_try()
    var_4 = var_0.filter(float_0)
    list_0 = [float_0, var_4, none_type_0, bool_0]
    maybe_1 = module_0.Maybe(list_0, var_2)
    var_5 = var_4.to_try()
    var_6 = var_0.filter(var_0)
    bool_1 = False
    maybe_2 = module_0.Maybe(bool_1, bool_1)
    var_7 = maybe_2.to_try()
    bool_2 = maybe_0.__eq__(bool_0)
    var_8 = maybe_2.to_validation()
    var_9 = var_8.to_lazy()
    var_8.to_validation()


def test_case_19():
    none_type_0 = None
    none_type_1 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(none_type_1, bool_0)
    var_0 = maybe_0.ap(none_type_0)
    var_1 = maybe_0.to_box()
    var_2 = maybe_0.to_lazy()
    var_3 = maybe_0.to_try()
    var_4 = var_0.filter(var_2)
    list_0 = [var_2, var_4, none_type_0, bool_0]
    maybe_1 = module_0.Maybe(list_0, var_2)
    var_5 = var_4.to_try()
    var_6 = var_0.filter(var_0)
    bool_1 = False
    maybe_2 = module_0.Maybe(bool_1, bool_1)
    var_7 = maybe_2.to_try()
    bool_2 = maybe_0.__eq__(bool_0)
    var_8 = var_2.to_try()
    var_9 = var_4.filter(var_3)


def test_case_20():
    none_type_0 = None
    bool_0 = True
    bool_1 = True
    bool_2 = False
    maybe_0 = module_0.Maybe(none_type_0, bool_2)
    var_0 = maybe_0.to_lazy()
    set_0 = {bool_0, none_type_0, bool_1, none_type_0}
    maybe_1 = module_0.Maybe(bool_2, bool_1)
    var_1 = maybe_1.to_lazy()
    var_2 = maybe_0.to_lazy()
    bool_3 = True
    maybe_2 = module_0.Maybe(set_0, bool_3)
    maybe_3 = module_0.Maybe(set_0, bool_3)
    var_3 = var_1.to_box()
    var_4 = var_3.to_try()
    var_5 = maybe_2.to_try()
    var_6 = var_0.to_box()
    var_7 = var_1.ap(var_4)
    var_8 = maybe_0.to_box()
    var_9 = maybe_1.filter(var_1)
    maybe_4 = module_0.Maybe(var_5, var_9)
    var_10 = maybe_3.to_either()
    var_11 = var_1.to_try()
    var_8.to_box()


def test_case_21():
    none_type_0 = None
    none_type_1 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(none_type_1, bool_0)
    var_0 = maybe_0.ap(none_type_0)
    var_1 = maybe_0.to_box()
    var_2 = maybe_0.to_lazy()
    var_3 = maybe_0.to_try()
    var_4 = var_0.filter(var_2)
    list_0 = [var_2, var_4, none_type_0, bool_0]
    maybe_1 = module_0.Maybe(list_0, var_2)
    var_5 = var_4.to_try()
    var_6 = var_5.filter(var_0)
    bool_1 = False
    maybe_2 = module_0.Maybe(var_2, bool_1)
    var_7 = var_0.to_try()
    bool_2 = maybe_2.__eq__(maybe_1)
    var_5.to_try()


def test_case_22():
    tuple_0 = ()
    bool_0 = True
    maybe_0 = module_0.Maybe(tuple_0, bool_0)
    var_0 = maybe_0.to_try()
    maybe_1 = module_0.Maybe(tuple_0, bool_0)
    var_1 = maybe_1.to_either()
    var_2 = var_1.bind(var_1)
    var_3 = maybe_1.bind(var_1)
    bool_1 = var_3.__eq__(maybe_1)
    float_0 = -1017.23017
    var_4 = var_3.map(var_1)
    var_5 = maybe_1.to_try()
    var_6 = var_3.map(tuple_0)
    var_7 = var_6.to_try()
    var_8 = var_1.to_box()
    maybe_2 = module_0.Maybe(float_0, var_8)
    var_9 = var_2.to_try()
    var_10 = maybe_1.to_try()
    var_5.to_box()
