# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pytutils.lazy.lazy_import as module_0


def test_case_0():
    int_0 = -3532
    illegal_use_of_scope_replacer_0 = module_0.IllegalUseOfScopeReplacer(
        int_0, int_0, int_0
    )
    illegal_use_of_scope_replacer_0.__repr__()


def test_case_1():
    bytes_0 = b"\xfbWU\xea\x92"
    illegal_use_of_scope_replacer_0 = module_0.IllegalUseOfScopeReplacer(
        bytes_0, bytes_0
    )
    illegal_use_of_scope_replacer_0.__str__()


def test_case_2():
    str_0 = "Make lazy_compil the default compile mode for regex compilation.\n\n    This overrides re.compile with lay_comCile. To restore the original\n    functionality, call reset_compile().\n    "
    illegal_use_of_scope_replacer_0 = module_0.IllegalUseOfScopeReplacer(str_0, str_0)
    var_0 = illegal_use_of_scope_replacer_0.__eq__(str_0)
    module_0.lazy_import(illegal_use_of_scope_replacer_0, str_0)


def test_case_3():
    dict_0 = {}
    none_type_0 = None
    import_replacer_0 = module_0.ImportReplacer(
        dict_0, none_type_0, dict_0, dict_0, dict_0
    )
    module_0.lazy_import(import_replacer_0, import_replacer_0, none_type_0)


def test_case_4():
    complex_0 = -152.09242 + 2370.1j
    module_0.ImportReplacer(complex_0, complex_0, complex_0)


def test_case_5():
    import_processor_0 = module_0.ImportProcessor()


def test_case_6():
    str_0 = "Make lazy_compile the default compile mode for regex compilation.\n\n    This overrides re.compile with lazy_compile. To restore the original\n    functionality, call reset_compile().\n    "
    module_0.lazy_import(str_0, str_0, str_0)


def test_case_7():
    str_0 = "Make lazy_compile the default compile mode fo^ regex compilation.\n\n    This override re.compile with lazy_compile. To restore the original\n    functionality, call reset_compile().\n    "
    module_0.lazy_import(str_0, str_0)


def test_case_8():
    var_0 = module_0.disallow_proxying()


def test_case_9():
    bytes_0 = b"b\x00\x13W\x8c\xea\xe7\xf4\xd6w\xab>\x17,\x12\xcaa~n"
    none_type_0 = None
    module_0.ScopeReplacer(bytes_0, bytes_0, none_type_0)


def test_case_10():
    dict_0 = {}
    bool_0 = True
    module_0.ImportReplacer(dict_0, dict_0, dict_0, bool_0, bool_0)


def test_case_11():
    var_0 = module_0.disallow_proxying()
    str_0 = ""
    module_0.lazy_import(var_0, str_0)


def test_case_12():
    bytes_0 = b"\xfbWU\xea\x92"
    illegal_use_of_scope_replacer_0 = module_0.IllegalUseOfScopeReplacer(
        bytes_0, bytes_0
    )
    var_0 = illegal_use_of_scope_replacer_0.__eq__(illegal_use_of_scope_replacer_0)
    illegal_use_of_scope_replacer_0.__str__()


def test_case_13():
    dict_0 = {}
    none_type_0 = None
    import_replacer_0 = module_0.ImportReplacer(dict_0, none_type_0, none_type_0)
    module_0.lazy_import(none_type_0, import_replacer_0, dict_0)


def test_case_14():
    str_0 = "ZI|6;pjV;HH_l{`m#V"
    module_0.lazy_import(str_0, str_0, str_0)


def test_case_15():
    str_0 = "NcSg](\\s-\nI\rq;X3!,"
    module_0.lazy_import(str_0, str_0)


def test_case_16():
    str_0 = "Upon request import 'module_path' as the name 'module_name'.\n        When imported, prepare children to also be imported.\n\n        :param scope: The scope that objects should be imported into.\n            Typically this is globals()\n        :param name: The variable name. Often this is the same as the\n            module_path. 'bzrlib'\n        :param module_path: A list for the fully specified module path\n            ['bzrlib', 'foo', 'bar']\n        :param member: The member inside the module to import, often this is\n            None, indicating the module is being imported.\n        :param children: Children entries to be imported later.\n            This should be a map of children specifications.\n            ::\n            \n                {'foo':(['bzrlib', 'foo'], None,\n                    {'bar':(['bzrlib', 'foo', 'bar'], None {})})\n                }\n\n        Examples::\n\n            import foo => name='foo' module_path='foo',\n                          member=None, children={}\n            import foo.bar => name='foo' module_path='foo', member=None,\n                              children={'bar':(['foo', 'bar'], None, {}}\n            from foo import bar => name='bar' module_path='foo', member='bar'\n                                   children={}\n            from foo import bar, baz would get translated into 2 import\n            requests. On for 'name=bar' and one for 'name=baz'\n        "
    module_0.lazy_import(str_0, str_0, str_0)


def test_case_17():
    dict_0 = {}
    none_type_0 = None
    import_replacer_0 = module_0.ImportReplacer(dict_0, none_type_0, none_type_0)
    scope_replacer_0 = module_0.ScopeReplacer(dict_0, import_replacer_0, none_type_0)
    scope_replacer_0.__str__()


def test_case_18():
    str_0 = "Mae lazy_compie the default compile mode fp regex comp`laion.\n\n   ?his overrids re.compile with lazT_compile. To rest}re the#originl\n   functionality, call=reset_compile().\n    "
    dict_0 = {str_0: str_0, str_0: str_0}
    import_replacer_0 = module_0.ImportReplacer(dict_0, str_0, str_0)
    module_0.lazy_import(dict_0, import_replacer_0)


def test_case_19():
    str_0 = "Mae lazy_compie the default compile mode fp regex comp`laion.\n\n   ?his overrids re.compile with lazT_compile. To rest}re the#originl\n   functionality, call=reset_compile().\n    "
    dict_0 = {str_0: str_0, str_0: str_0}
    list_0 = []
    import_replacer_0 = module_0.ImportReplacer(dict_0, str_0, list_0, dict_0)
    import_replacer_0.__call__()
