# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import builtins as module_0
import sanic.helpers as module_1
import inspect as module_2


def test_case_0():
    object_0 = module_0.object()
    module_1.has_message_body(object_0)


def test_case_1():
    object_0 = module_0.object()
    module_1.remove_entity_headers(object_0)


def test_case_2():
    str_0 = "G5=Zd8EH\x0c\rn<D"
    dict_0 = {str_0: str_0, str_0: str_0, str_0: str_0, str_0: str_0}
    var_0 = module_1.remove_entity_headers(dict_0)


def test_case_3():
    bool_0 = module_1.is_atty()


def test_case_4():
    bool_0 = module_1.is_atty()
    module_1.is_entity_header(bool_0)


def test_case_5():
    str_0 = "W5=Zd8EH\x0c\rn<D"
    var_0 = module_1.is_hop_by_hop_header(str_0)
    dict_0 = {var_0: var_0, var_0: str_0, str_0: str_0, str_0: var_0}
    module_1.remove_entity_headers(dict_0)


def test_case_6():
    none_type_0 = None
    module_1.remove_entity_headers(none_type_0, none_type_0)


def test_case_7():
    default_0 = module_1.Default()
    var_0 = default_0.__repr__()
    str_0 = "G5=Zd8EH\x0c\rn<D"
    dict_0 = {str_0: str_0, str_0: str_0, str_0: str_0, str_0: str_0}
    var_1 = module_1.remove_entity_headers(dict_0)


def test_case_8():
    bool_0 = module_1.is_atty()
    default_0 = module_1.Default()
    str_0 = default_0.__str__()
    module_1.import_string(bool_0, bool_0)


def test_case_9():
    int_0 = -1164
    var_0 = module_1.has_message_body(int_0)
    module_1.remove_entity_headers(var_0, int_0)


def test_case_10():
    int_0 = 167
    var_0 = module_1.has_message_body(int_0)
    str_0 = var_0.__str__()
    bool_0 = module_1.is_atty()
    module_1.is_entity_header(bool_0)


def test_case_11():
    int_0 = 204
    var_0 = module_1.has_message_body(int_0)
    bool_0 = False
    str_0 = var_0.__str__()
    var_1 = module_1.has_message_body(bool_0)
    var_2 = module_1.has_message_body(var_1)
    dict_0 = {}
    var_3 = module_1.remove_entity_headers(dict_0, dict_0)
    str_1 = var_3.__str__()
    default_0 = module_1.Default(*var_3, **var_3)
    var_4 = var_1.__repr__()
    str_2 = default_0.__str__()
    var_5 = module_2.ismodule(str_0)
    module_1.is_hop_by_hop_header(var_0)
