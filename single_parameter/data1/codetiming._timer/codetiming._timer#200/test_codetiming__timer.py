# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import codetiming._timer as module_0
import dataclasses as module_1


def test_case_0():
    float_arg_0 = module_0.FloatArg()


def test_case_1():
    timer_0 = module_0.Timer()
    none_type_0 = timer_0.start()
    timer_0.__enter__()


def test_case_2():
    bool_0 = False
    set_0 = {bool_0, bool_0, bool_0, bool_0}
    timer_0 = module_0.Timer(set_0)
    float_arg_0 = module_0.FloatArg()
    timer_1 = module_0.Timer(initial_text=float_arg_0, logger=float_arg_0)
    var_0 = timer_1.__eq__(float_arg_0)
    timer_2 = module_0.Timer(timer_1)
    var_1 = timer_2.__repr__()
    timer_1.stop()


def test_case_3():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    none_type_0 = timer_1.__exit__()


def test_case_4():
    timer_error_0 = module_0.TimerError()
    timer_0 = module_0.Timer()
    none_type_0 = timer_0.start()
    bool_0 = False
    var_0 = timer_0.__repr__()
    timer_1 = module_0.Timer(logger=bool_0)
    float_0 = timer_0.stop()
    none_type_1 = timer_1.start()
    var_1 = timer_0.__repr__()
    var_1.copy()


def test_case_5():
    dict_0 = {}
    float_arg_0 = module_0.FloatArg(**dict_0)
    float_arg_1 = module_0.FloatArg(*dict_0)
    timer_0 = module_0.Timer(initial_text=float_arg_1)
    none_type_0 = timer_0.start()


def test_case_6():
    timer_0 = module_0.Timer()
    none_type_0 = timer_0.start()
    none_type_1 = timer_0.__exit__()
    var_0 = timer_0.__repr__()
    timer_1 = module_0.Timer(initial_text=var_0)
    timer_1.start()


def test_case_7():
    str_0 = "29fniKdYL)9PLpl"
    timer_0 = module_0.Timer(initial_text=str_0)
    timer_1 = module_0.Timer(str_0, logger=timer_0)
    none_type_0 = timer_0.start()
    timer_2 = timer_1.__enter__()
    none_type_1 = timer_1.__exit__()
    list_0 = [timer_2]
    timer_2.__exit__(*list_0)


def test_case_8():
    timer_0 = module_0.Timer()
    var_0 = timer_0.__eq__(timer_0)
    timer_error_0 = module_0.TimerError()
    var_1 = timer_0.__call__(var_0)
    timer_1 = timer_0.__enter__()
    var_2 = timer_1.__repr__()
    timer_2 = module_0.Timer(var_0, initial_text=timer_1)
    none_type_0 = timer_2.start()
    var_3 = timer_1.__repr__()
    timer_error_1 = module_0.TimerError()
    float_0 = timer_0.stop()
    var_3.__exit__()


def test_case_9():
    bytes_0 = b""
    str_0 = ""
    none_type_0 = None
    timer_0 = module_0.Timer(none_type_0, str_0, logger=str_0)
    timer_1 = timer_0.__enter__()
    none_type_1 = timer_0.__exit__()
    var_0 = module_1.field(compare=str_0)
    var_0.__contains__(bytes_0)
