# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import codetiming._timer as module_0


def test_case_0():
    timer_error_0 = module_0.TimerError()


def test_case_1():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    timer_1.__enter__()


def test_case_2():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()
    none_type_0 = timer_0.__exit__()


def test_case_3():
    timer_0 = module_0.Timer()
    timer_0.__exit__()


def test_case_4():
    none_type_0 = None
    timer_0 = module_0.Timer(logger=none_type_0)
    timer_1 = timer_0.__enter__()
    var_0 = timer_0.__call__(none_type_0)
    timer_0.start()


def test_case_5():
    bool_0 = True
    none_type_0 = None
    timer_0 = module_0.Timer(none_type_0, bool_0, bool_0)
    float_arg_0 = module_0.FloatArg()
    timer_1 = timer_0.__enter__()
    timer_0.stop()


def test_case_6():
    float_arg_0 = module_0.FloatArg()
    bool_0 = True
    list_0 = [float_arg_0, float_arg_0, bool_0]
    timer_error_0 = module_0.TimerError(*list_0)
    none_type_0 = None
    none_type_1 = None
    timer_0 = module_0.Timer(none_type_1, bool_0, bool_0, none_type_0)
    float_arg_1 = module_0.FloatArg()
    timer_1 = module_0.Timer(list_0, initial_text=float_arg_1)
    var_0 = timer_0.__call__(float_arg_1)
    timer_2 = timer_0.__enter__()
    float_0 = timer_0.stop()
    module_0.FloatArg(*list_0, **var_0)


def test_case_7():
    float_arg_0 = module_0.FloatArg()
    bool_0 = True
    list_0 = [float_arg_0, float_arg_0, bool_0]
    timer_error_0 = module_0.TimerError(*list_0)
    none_type_0 = None
    timer_0 = module_0.Timer(none_type_0, bool_0, bool_0)
    float_arg_1 = module_0.FloatArg()
    timer_1 = module_0.Timer(list_0, initial_text=float_arg_1)
    timer_2 = timer_1.__enter__()
    timer_2.stop()


def test_case_8():
    float_arg_0 = module_0.FloatArg()
    bool_0 = True
    list_0 = [float_arg_0, float_arg_0, bool_0]
    timer_error_0 = module_0.TimerError(*list_0)
    none_type_0 = None
    timer_0 = module_0.Timer(none_type_0, bool_0, bool_0)
    float_arg_1 = timer_0.__repr__()
    timer_1 = module_0.Timer(list_0, initial_text=float_arg_1)
    timer_2 = timer_1.__enter__()
    timer_2.stop()


def test_case_9():
    float_arg_0 = module_0.FloatArg()
    bool_0 = True
    list_0 = [float_arg_0, float_arg_0, float_arg_0, bool_0]
    timer_error_0 = module_0.TimerError(*list_0)
    none_type_0 = None
    timer_0 = module_0.Timer(none_type_0, bool_0, bool_0)
    var_0 = timer_0.__eq__(bool_0)
    timer_1 = module_0.Timer(list_0, timer_0, var_0)
    timer_2 = module_0.Timer()
    timer_3 = timer_1.__enter__()
    timer_3.__exit__(*list_0)
