# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import codetiming._timer as module_0
import dataclasses as module_1


def test_case_0():
    float_arg_0 = module_0.FloatArg()


def test_case_1():
    float_arg_0 = module_0.FloatArg()
    float_arg_1 = module_0.FloatArg()
    timer_0 = module_0.Timer(float_arg_1)
    timer_1 = timer_0.__enter__()
    timer_0.start()


def test_case_2():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()


def test_case_3():
    timer_0 = module_0.Timer()
    none_type_0 = timer_0.start()
    float_0 = timer_0.stop()
    timer_0.stop()


def test_case_4():
    timer_0 = module_0.Timer()
    none_type_0 = timer_0.start()
    float_0 = timer_0.stop()


def test_case_5():
    timer_error_0 = module_0.TimerError()
    timer_0 = module_0.Timer(text=timer_error_0, initial_text=timer_error_0)
    list_0 = module_1.dataclass(eq=timer_0, kw_only=timer_error_0)
    none_type_0 = timer_0.start()
    timer_0.__exit__()


def test_case_6():
    none_type_0 = None
    timer_0 = module_0.Timer(logger=none_type_0)
    float_arg_0 = module_0.FloatArg()
    var_0 = timer_0.__repr__()
    timer_1 = timer_0.__enter__()
    var_1 = module_1.dataclass(init=none_type_0, repr=none_type_0, frozen=none_type_0)
    var_2 = var_1.__eq__(none_type_0)
    float_0 = timer_0.stop()
    var_0.__delitem__(var_1)


def test_case_7():
    str_0 = '!\ru"@\\,[ER6Tt@\\gDR"'
    timer_0 = module_0.Timer(initial_text=str_0)
    timer_1 = timer_0.__enter__()


def test_case_8():
    bytes_0 = b"\xc7;\xa6\xb5\xe2\x90H\x85T2"
    timer_0 = module_0.Timer(bytes_0, initial_text=bytes_0)
    none_type_0 = timer_0.start()
    module_0.TimerError(**bytes_0)


def test_case_9():
    timer_0 = module_0.Timer()
    timer_1 = module_0.Timer(text=timer_0, initial_text=timer_0)
    var_0 = timer_1.__repr__()
    none_type_0 = timer_0.start()
    float_0 = timer_0.stop()
    var_1 = timer_0.__repr__()
    none_type_1 = timer_1.start()
    float_1 = timer_1.stop()
    var_1.items()


def test_case_10():
    bool_0 = True
    timer_0 = module_0.Timer(bool_0)
    timer_1 = timer_0.__call__(bool_0)
    timer_2 = module_0.Timer(text=timer_1, initial_text=timer_1)
    timer_1.__call__(timer_2)
