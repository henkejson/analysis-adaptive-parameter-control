# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import sanic.config as module_0
import inspect as module_1
import sanic.http.http1 as module_2
import sanic.constants as module_3


def test_case_0():
    config_0 = module_0.Config()


def test_case_1():
    config_0 = module_0.Config()
    none_type_0 = config_0.update()
    config_1 = module_0.Config(converters=config_0)
    config_1.update_config(none_type_0)


def test_case_2():
    bytes_0 = b"BB\xa4\n\x1f\x95\xb7P\x14l\x13\x9b\xf1'\xcf\xbe"
    str_0 = "sanic.*"
    config_0 = module_0.Config(env_prefix=str_0, keep_alive=str_0)
    config_0.load_environment_vars(bytes_0)


def test_case_3():
    list_0 = []
    str_0 = "%~\te)z"
    str_1 = "nn#"
    dict_0 = {str_0: str_0, str_0: list_0, str_1: str_0, str_1: list_0}
    none_type_0 = None
    config_0 = module_0.Config(env_prefix=none_type_0, keep_alive=none_type_0)
    none_type_1 = config_0.update(*list_0, **dict_0)
    bool_0 = True
    config_1 = module_0.Config()
    var_0 = module_1.isdatadescriptor(bool_0)
    var_0.__getattr__(var_0)


def test_case_4():
    config_0 = module_0.Config()
    bool_0 = True
    config_1 = module_0.Config(config_0, keep_alive=bool_0)
    var_0 = config_0.update_config(config_0)
    config_2 = module_0.Config(converters=config_0)
    config_3 = module_0.Config(config_2, keep_alive=var_0)
    module_2.Http(config_0)


def test_case_5():
    config_0 = module_0.Config()
    config_0.init_for_request()


def test_case_6():
    config_0 = module_0.Config()
    config_1 = module_0.Config(converters=config_0)
    var_0 = config_0.update_config(config_0)
    module_0.Config(env_prefix=config_1)


def test_case_7():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)


def test_case_8():
    config_0 = module_0.Config()
    var_0 = module_1.getmembers(config_0)
    config_0.update_config(var_0)


def test_case_9():
    config_0 = module_0.Config()
    config_1 = module_0.Config()
    config_1.__getattr__(config_1)


def test_case_10():
    none_type_0 = None
    config_0 = module_0.Config(none_type_0)
    var_0 = module_1.getmembers(config_0, none_type_0)
    config_0.__setitem__(none_type_0, var_0)


def test_case_11():
    config_0 = module_0.Config()
    var_0 = module_3.LocalCertCreator.TRUSTME
    config_0.update_config(var_0)


def test_case_12():
    config_0 = module_0.Config()
    bool_0 = True
    config_1 = module_0.Config(config_0, keep_alive=bool_0)
    str_0 = "r0wy#d6/2Qw%\r\tn"
    str_1 = "s9)DR7BDn,~\r7?Ete&P\x0c"
    dict_0 = {str_0: bool_0, str_0: str_0, str_0: bool_0, str_1: bool_0}
    module_0.Config(dict_0, keep_alive=bool_0, converters=str_1)


def test_case_13():
    config_0 = module_0.Config()
    str_0 = "xhA9uf3=(&X1?:c!Rk"
    none_type_0 = None
    none_type_1 = config_0.__setitem__(str_0, none_type_0)
    var_0 = module_1.getmembers(config_0)
    str_1 = "LOCAL_CERT_CREATOR"
    config_0.__setattr__(str_1, str_1)
