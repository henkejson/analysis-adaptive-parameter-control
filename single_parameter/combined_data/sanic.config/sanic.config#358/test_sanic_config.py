# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import sanic.config as module_0
import inspect as module_1
import sanic.utils as module_2


def test_case_0():
    config_0 = module_0.Config()


def test_case_1():
    config_0 = module_0.Config()
    str_0 = "5c7T\tARs"
    config_1 = module_0.Config(env_prefix=str_0, converters=config_0)
    config_0.update_config(str_0)


def test_case_2():
    bool_0 = False
    config_0 = module_0.Config(keep_alive=bool_0)
    config_1 = module_0.Config()
    str_0 = "5c7T\tARs"
    none_type_0 = None
    config_2 = module_0.Config(env_prefix=str_0, converters=none_type_0)
    none_type_1 = config_1.register_type(config_1)
    var_0 = config_1.update_config(config_1)
    var_1 = module_1.isclass(none_type_1)
    config_1.respond(var_0)


def test_case_3():
    config_0 = module_0.Config()
    str_0 = "5c7T\tARs"
    none_type_0 = None
    config_1 = module_0.Config(env_prefix=str_0, converters=none_type_0)
    none_type_1 = config_0.register_type(config_0)
    none_type_1.init_for_request()


def test_case_4():
    config_0 = module_0.Config()
    str_0 = "5c7T\tARs"
    config_1 = module_0.Config(env_prefix=str_0, converters=str_0)
    none_type_0 = config_0.register_type(config_0)
    var_0 = module_1.isclass(config_1)
    module_0.Config(str_0)


def test_case_5():
    bool_0 = True
    config_0 = module_0.Config()
    config_0.__getattr__(bool_0)


def test_case_6():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)


def test_case_7():
    config_0 = module_0.Config()
    none_type_0 = config_0.register_type(config_0)


def test_case_8():
    none_type_0 = None
    config_0 = module_0.Config(env_prefix=none_type_0, converters=none_type_0)
    str_0 = "W_"
    str_1 = "_FALLBACK_ERROR_FORMAT"
    module_2.load_module_from_file_location(str_0, str_1, **str_1)


def test_case_9():
    config_0 = module_0.Config()
    none_type_0 = None
    config_0.update_config(none_type_0)


def test_case_10():
    config_0 = module_0.Config()
    str_0 = "5c7T\tARs"
    config_1 = module_0.Config(env_prefix=str_0, converters=str_0)
    config_0.__getattr__(config_1)


def test_case_11():
    config_0 = module_0.Config()
    str_0 = "5c7T\tARs"
    none_type_0 = config_0.__setitem__(str_0, config_0)
    config_1 = module_0.Config()
    none_type_1 = config_1.register_type(config_1)
    var_0 = config_1.update_config(config_0)
    config_1.update_config(none_type_1)


def test_case_12():
    config_0 = module_0.Config()
    str_0 = "USE_UVLOOP"
    module_0.Config(env_prefix=str_0, converters=str_0)


def test_case_13():
    config_0 = module_0.Config()
    str_0 = "5c7T\tARs"
    var_0 = config_0.update_config(config_0)
    config_1 = module_0.Config(env_prefix=str_0, keep_alive=var_0, converters=var_0)
    str_1 = "LOCAL_CERT_CREATOR"
    config_0.__setattr__(str_1, var_0)
