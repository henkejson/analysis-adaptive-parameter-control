# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import sanic.config as module_0
import inspect as module_1


def test_case_0():
    config_0 = module_0.Config()


def test_case_1():
    none_type_0 = None
    config_0 = module_0.Config(converters=none_type_0)
    str_0 = "Delete the Blueprint object at the specified index.\n\n        Abstract method implemented to turn the `BlueprintGroup` class\n        into a list like object to support all the existing behavior.\n\n        This method is used to delete an item from the list of blueprint\n        groups like it can be done on a regular list with index.\n\n        Args:\n            index (int): Index to use for removing a new Blueprint item\n\n        Returns:\n            None\n\n        Raises:\n            IndexError: If the index is out of range.\n        "
    dict_0 = {str_0: str_0, str_0: str_0, str_0: str_0}
    config_1 = module_0.Config(
        env_prefix=none_type_0, keep_alive=none_type_0, converters=none_type_0
    )
    var_0 = config_1.update_config(dict_0)


def test_case_2():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)
    none_type_0 = None
    none_type_1 = config_0.register_type(none_type_0)
    config_0.__getattr__(none_type_0)


def test_case_3():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)


def test_case_4():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)
    none_type_0 = None
    none_type_1 = config_0.register_type(none_type_0)


def test_case_5():
    none_type_0 = None
    config_0 = module_0.Config(converters=none_type_0)
    str_0 = "Delete the Blueprint object at the specified index.\n\n        Abstract method implemented to turn the `BlueprintGroup` class\n        into a list like object to support all the existing behavior.\n\n        This method is used to delete an item from the list of blueprint\n        groups like it can be done on a regular list with index.\n\n        Args:\n            index (int): Index to use for removing a new Blueprint item\n\n        Returns:\n            None\n\n        Raises:\n            IndexError: If the index is out of range.\n        "
    dict_0 = {str_0: str_0, str_0: str_0, str_0: str_0}
    config_0.__setitem__(dict_0, config_0)


def test_case_6():
    config_0 = module_0.Config()
    config_1 = module_0.Config(config_0)
    var_0 = config_0.update_config(config_1)
    none_type_0 = config_1.register_type(config_1)
    config_0.__setattr__(config_1, config_0)


def test_case_7():
    config_0 = module_0.Config()
    config_0.__getattr__(config_0)


def test_case_8():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)
    config_0.update_config(var_0)


def test_case_9():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)
    none_type_0 = config_0.register_type(var_0)
    none_type_1 = None
    config_0.register_type(none_type_1)


def test_case_10():
    config_0 = module_0.Config()
    str_0 = "$["
    none_type_0 = None
    none_type_1 = config_0.register_type(none_type_0)
    module_0.Config(env_prefix=config_0, keep_alive=none_type_0, converters=str_0)


def test_case_11():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)
    str_0 = "$["
    config_0.update_config(str_0)


def test_case_12():
    str_0 = "f/9d\r.j\tr"
    none_type_0 = None
    str_1 = "FORWARDED_SECRET"
    dict_0 = {str_1: str_1, str_1: str_1}
    config_0 = module_0.Config(dict_0, keep_alive=str_1)
    none_type_1 = config_0.__setitem__(str_0, none_type_0)
    var_0 = module_1.isclass(str_0)
    config_0.update_config(var_0)


def test_case_13():
    none_type_0 = None
    config_0 = module_0.Config(keep_alive=none_type_0, converters=none_type_0)
    str_0 = "LOCAL_CERT_CREATOR"
    config_0.__setitem__(str_0, config_0)
