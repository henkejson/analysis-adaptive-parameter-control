# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import py_backwards.transformers.return_from_generator as module_0
import typed_ast._ast3 as module_1
import typed_ast.ast3 as module_2


def test_case_0():
    none_type_0 = None
    return_from_generator_transformer_0 = module_0.ReturnFromGeneratorTransformer(
        none_type_0
    )


def test_case_1():
    aug_load_0 = module_1.AugLoad()
    var_0 = module_2.dump(aug_load_0, aug_load_0)
    list_0 = [aug_load_0, var_0, var_0, aug_load_0, var_0]
    function_def_0 = module_1.FunctionDef(*list_0)
    return_from_generator_transformer_0 = module_0.ReturnFromGeneratorTransformer(var_0)
    function_def_1 = return_from_generator_transformer_0.visit_FunctionDef(
        function_def_0
    )


def test_case_2():
    aug_load_0 = module_1.AugLoad()
    var_0 = module_2.iter_fields(aug_load_0)
    list_0 = [var_0, aug_load_0, var_0, var_0, var_0]
    function_def_0 = module_1.FunctionDef(*list_0)
    return_from_generator_transformer_0 = module_0.ReturnFromGeneratorTransformer(var_0)
    function_def_1 = return_from_generator_transformer_0.visit_FunctionDef(
        function_def_0
    )


def test_case_3():
    str_0 = "d;=ZP:G4m7"
    str_1 = "<nL\n[e>Q\x0cy0-^)].}vA"
    str_2 = "urllib"
    dict_0 = {str_0: str_0, str_0: str_0, str_1: str_0, str_2: str_1}
    return_from_generator_transformer_0 = module_0.ReturnFromGeneratorTransformer(str_2)
    var_0 = module_2.parse(str_2)
    function_def_0 = return_from_generator_transformer_0.visit_FunctionDef(var_0)
    function_def_1 = return_from_generator_transformer_0.visit_FunctionDef(
        function_def_0
    )
    return_from_generator_transformer_1 = module_0.ReturnFromGeneratorTransformer(
        dict_0
    )
    return_from_generator_transformer_1.generic_visit(str_2)
