# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import py_backwards.transformers.yield_from as module_0
import typed_ast._ast3 as module_1
import typed_ast.ast3 as module_2


def test_case_0():
    str_0 = "\x0b'Mj<7(2Tr`B(i;d!Zzt"
    yield_from_transformer_0 = module_0.YieldFromTransformer(str_0)
    yield_from_transformer_0.visit(str_0)


def test_case_1():
    generator_exp_0 = module_1.GeneratorExp()
    yield_from_transformer_0 = module_0.YieldFromTransformer(generator_exp_0)


def test_case_2():
    str_0 = "tkinter_scrolledtext"
    yield_from_transformer_0 = module_0.YieldFromTransformer(str_0)
    var_0 = module_2.parse(str_0)
    a_s_t_0 = yield_from_transformer_0.visit(var_0)


def test_case_3():
    str_0 = "\x0b2Mj<7K2T)`Bgi;d!Zzt"
    yield_from_transformer_0 = module_0.YieldFromTransformer(str_0)
    list_0 = [yield_from_transformer_0, yield_from_transformer_0]
    module_0 = module_1.Module(*list_0)
    a_s_t_0 = yield_from_transformer_0.visit(module_0)
