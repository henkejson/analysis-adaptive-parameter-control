# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import codetiming._timer as module_0


def test_case_0():
    timer_error_0 = module_0.TimerError()


def test_case_1():
    timer_0 = module_0.Timer()
    timer_1 = timer_0.__enter__()


def test_case_2():
    none_type_0 = None
    timer_0 = module_0.Timer(initial_text=none_type_0)
    timer_1 = module_0.Timer()
    timer_1.__exit__()


def test_case_3():
    timer_0 = module_0.Timer()
    none_type_0 = timer_0.start()
    none_type_1 = timer_0.__exit__()


def test_case_4():
    float_arg_0 = module_0.FloatArg()
    none_type_0 = None
    timer_0 = module_0.Timer(initial_text=none_type_0, logger=none_type_0)
    none_type_1 = timer_0.start()
    none_type_2 = timer_0.__exit__()
    timer_0.stop()


def test_case_5():
    timer_0 = module_0.Timer()
    var_0 = timer_0.__eq__(timer_0)
    none_type_0 = timer_0.start()
    timer_0.__enter__()


def test_case_6():
    float_arg_0 = module_0.FloatArg()
    timer_0 = module_0.Timer(float_arg_0, initial_text=float_arg_0)
    none_type_0 = timer_0.start()


def test_case_7():
    float_arg_0 = module_0.FloatArg()
    none_type_0 = None
    timer_0 = module_0.Timer(none_type_0, initial_text=float_arg_0)
    timer_1 = timer_0.__enter__()


def test_case_8():
    float_arg_0 = module_0.FloatArg()
    timer_0 = module_0.Timer(float_arg_0, initial_text=float_arg_0)
    none_type_0 = timer_0.start()
    list_0 = [none_type_0, none_type_0, none_type_0, none_type_0]
    none_type_1 = timer_0.__exit__(*list_0)


def test_case_9():
    bool_0 = True
    float_arg_0 = module_0.FloatArg()
    str_0 = 'qx0xt9r!3G"pkp6'
    timer_error_0 = module_0.TimerError()
    timer_0 = module_0.Timer(text=bool_0, initial_text=str_0, logger=str_0)
    timer_0.start()


def test_case_10():
    float_arg_0 = module_0.FloatArg()
    none_type_0 = None
    timer_0 = module_0.Timer(initial_text=float_arg_0)
    timer_1 = module_0.Timer(none_type_0, timer_0)
    none_type_1 = timer_1.start()
    float_0 = -578.18
    bytes_0 = b""
    tuple_0 = (float_0, bytes_0, timer_1)
    list_0 = [tuple_0, timer_0, float_arg_0]
    none_type_2 = timer_1.__exit__(*list_0)
    timer_2 = module_0.Timer(logger=none_type_0)
