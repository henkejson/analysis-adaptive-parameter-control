# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import codetiming._timer as module_0
import dataclasses as module_1


def test_case_0():
    timer_0 = module_0.Timer()


def test_case_1():
    int_0 = 954
    timer_0 = module_0.Timer(int_0, int_0, int_0)
    timer_1 = timer_0.__enter__()
    var_0 = timer_1.__repr__()
    timer_0.__enter__()


def test_case_2():
    none_type_0 = None
    timer_0 = module_0.Timer(none_type_0)
    timer_1 = timer_0.__enter__()


def test_case_3():
    str_0 = "T"
    timer_error_0 = module_0.TimerError()
    timer_0 = module_0.Timer(str_0)
    timer_error_1 = module_0.TimerError(*str_0)
    timer_0.__exit__()


def test_case_4():
    int_0 = 954
    timer_0 = module_0.Timer(text=int_0, initial_text=int_0)
    timer_1 = timer_0.__enter__()
    timer_0.__exit__()


def test_case_5():
    none_type_0 = None
    str_0 = "0\x0c`qz`D!\r_PWo\rGr"
    timer_0 = module_0.Timer(none_type_0, str_0, str_0)
    timer_1 = timer_0.__enter__()
    timer_1.__enter__()


def test_case_6():
    none_type_0 = None
    timer_0 = module_0.Timer(none_type_0)
    timer_1 = timer_0.__enter__()
    float_0 = timer_0.stop()


def test_case_7():
    none_type_0 = None
    timer_0 = module_0.Timer(logger=none_type_0)
    none_type_1 = timer_0.start()
    timer_1 = module_0.Timer(none_type_0, timer_0, timer_0)
    timer_2 = timer_1.__enter__()
    var_0 = timer_2.__eq__(none_type_0)
    var_0.__call__(var_0)


def test_case_8():
    str_0 = "+D(D\r.P\nkVO5Z\x0c"
    timer_0 = module_0.Timer(str_0, str_0, str_0)
    timer_1 = timer_0.__enter__()
    var_0 = module_1.field()
    var_1 = timer_0.__repr__()
    float_0 = timer_0.stop()
    var_1.setdefault(str_0, var_1)


def test_case_9():
    tuple_0 = ()
    none_type_0 = None
    str_0 = '73WQc8i.gIXPR"lXB'
    none_type_1 = None
    timer_0 = module_0.Timer(none_type_0, none_type_1, tuple_0, tuple_0)
    list_0 = [timer_0, tuple_0, str_0]
    timer_error_0 = module_0.TimerError(*list_0)
    timer_1 = timer_0.__enter__()
    var_0 = timer_0.__repr__()
    var_1 = module_1.field()
    var_2 = timer_0.__eq__(none_type_0)
    var_3 = timer_0.__repr__()
    float_0 = timer_0.stop()
    timer_1.stop()


def test_case_10():
    int_0 = 954
    timer_0 = module_0.Timer(text=int_0, initial_text=int_0)
    timer_1 = timer_0.__enter__()
    timer_2 = module_0.Timer(text=timer_0)
    timer_3 = timer_2.__enter__()
    none_type_0 = timer_3.__exit__()
