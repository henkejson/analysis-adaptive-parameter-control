# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import codetiming._timer as module_0


def test_case_0():
    timer_error_0 = module_0.TimerError()


def test_case_1():
    timer_0 = module_0.Timer()
    none_type_0 = timer_0.start()


def test_case_2():
    timer_0 = module_0.Timer()
    timer_0.stop()


def test_case_3():
    timer_0 = module_0.Timer()
    none_type_0 = timer_0.start()
    none_type_1 = timer_0.__exit__()


def test_case_4():
    timer_0 = module_0.Timer()
    none_type_0 = timer_0.start()
    timer_0.__enter__()


def test_case_5():
    str_0 = "Timer"
    timer_0 = module_0.Timer(str_0, initial_text=str_0)
    timer_1 = module_0.Timer(str_0, timer_0, str_0)
    none_type_0 = timer_1.start()
    timer_2 = timer_0.__enter__()


def test_case_6():
    timer_0 = module_0.Timer()
    var_0 = timer_0.__eq__(timer_0)
    timer_1 = timer_0.__enter__()
    var_1 = timer_1.__call__(timer_0)
    str_0 = "Timer"
    timer_2 = module_0.Timer(str_0, initial_text=str_0)
    timer_3 = timer_2.__enter__()
    none_type_0 = timer_0.__exit__()
    none_type_1 = timer_2.__exit__()
    var_1.start()


def test_case_7():
    timer_0 = module_0.Timer()
    var_0 = timer_0.__eq__(timer_0)
    timer_1 = timer_0.__enter__()
    float_0 = timer_0.stop()
    var_1 = timer_1.__call__(timer_0)
    list_0 = [timer_1, timer_1, timer_1]
    timer_2 = module_0.Timer(var_0, initial_text=var_0)
    timer_3 = module_0.Timer(var_0, timer_0, var_0)
    timer_error_0 = module_0.TimerError(*list_0)
    timer_4 = timer_2.__enter__()
    var_0.start()


def test_case_8():
    timer_0 = module_0.Timer()
    var_0 = timer_0.__eq__(timer_0)
    timer_1 = timer_0.__enter__()
    float_0 = timer_0.stop()
    var_1 = timer_1.__call__(timer_0)
    timer_2 = module_0.Timer()
    timer_3 = module_0.Timer(initial_text=var_0)
    timer_error_0 = module_0.TimerError()
    timer_4 = timer_3.__enter__()
    timer_0.__exit__()


def test_case_9():
    timer_0 = module_0.Timer()
    var_0 = timer_0.__eq__(timer_0)
    timer_1 = timer_0.__enter__()
    var_1 = timer_1.__call__(timer_0)
    timer_2 = module_0.Timer(var_1, initial_text=var_1)
    timer_3 = module_0.Timer(var_1, timer_0, var_1)
    timer_4 = module_0.Timer(text=timer_2)
    dict_0 = {}
    timer_error_0 = module_0.TimerError(**dict_0)
    timer_5 = timer_3.__enter__()
    none_type_0 = timer_3.__exit__()
    var_0.start()


def test_case_10():
    none_type_0 = None
    timer_0 = module_0.Timer(logger=none_type_0)
    bool_0 = False
    var_0 = timer_0.__call__(timer_0)
    timer_1 = module_0.Timer(initial_text=bool_0)
    timer_2 = module_0.Timer(bool_0)
    var_1 = timer_0.__repr__()
    none_type_1 = timer_0.start()
    none_type_2 = timer_2.start()
    list_0 = []
    float_arg_0 = module_0.FloatArg(*list_0)
    var_2 = timer_2.__eq__(none_type_2)
    timer_1.__len__()


def test_case_11():
    none_type_0 = None
    timer_0 = module_0.Timer(logger=none_type_0)
    bool_0 = False
    var_0 = timer_0.__call__(timer_0)
    timer_1 = module_0.Timer(initial_text=bool_0)
    timer_2 = module_0.Timer(bool_0)
    var_1 = timer_0.__repr__()
    none_type_1 = timer_0.start()
    none_type_2 = timer_2.start()
    list_0 = []
    float_0 = timer_0.stop()
    float_arg_0 = module_0.FloatArg(*list_0)
    var_2 = timer_2.__eq__(none_type_2)
    timer_1.__len__()
