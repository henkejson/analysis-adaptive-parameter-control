# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pytutils.lazy.lazy_import as module_0


def test_case_0():
    str_0 = "\\A([A-Za-z_0-9]+)=(.*)\\Z"
    var_0 = module_0.IllegalUseOfScopeReplacer(str_0, str_0, str_0)
    var_0.__unicode__()


def test_case_1():
    str_0 = "\\A([AZa-z_09]+)=(.*)\\Z"
    illegal_use_of_scope_replacer_0 = module_0.IllegalUseOfScopeReplacer(str_0, str_0)
    module_0.lazy_import(str_0, str_0)


def test_case_2():
    import_processor_0 = module_0.ImportProcessor()
    dict_0 = {
        import_processor_0: import_processor_0,
        import_processor_0: import_processor_0,
        import_processor_0: import_processor_0,
        import_processor_0: import_processor_0,
        import_processor_0: import_processor_0,
        import_processor_0: import_processor_0,
        import_processor_0: import_processor_0,
        import_processor_0: import_processor_0,
    }
    import_replacer_0 = module_0.ImportReplacer(
        dict_0, import_processor_0, import_processor_0, import_processor_0
    )
    module_0.lazy_import(import_processor_0, import_replacer_0, dict_0)


def test_case_3():
    var_0 = module_0.disallow_proxying()
    module_0.ImportReplacer(var_0, var_0, var_0)


def test_case_4():
    import_processor_0 = module_0.ImportProcessor()


def test_case_5():
    str_0 = "pOZu/Qs0CpDuWtEt"
    module_0.lazy_import(str_0, str_0, str_0)


def test_case_6():
    str_0 = "pOZu/Qs0CpDuWtEt"
    module_0.lazy_import(str_0, str_0)


def test_case_7():
    var_0 = module_0.disallow_proxying()


def test_case_8():
    var_0 = module_0.disallow_proxying()
    module_0.lazy_import(var_0, var_0)


def test_case_9():
    str_0 = "%\x0bn~Ep"
    illegal_use_of_scope_replacer_0 = module_0.IllegalUseOfScopeReplacer(str_0, str_0)
    var_0 = illegal_use_of_scope_replacer_0.__eq__(str_0)
    illegal_use_of_scope_replacer_0.__repr__()


def test_case_10():
    str_0 = "Disallow lazily imported modules to be used as proxies.\n\n    Calling this function might cause problems with concurrent imports\n    in multithreaded environments, but will help detecting wasteful\n    indirection, so it should be called when executing unit tests.\n\n    Only lazy imports that happen after this call are affected.\n    "
    module_0.lazy_import(str_0, str_0)


def test_case_11():
    var_0 = module_0.disallow_proxying()
    str_0 = "#nldoxb'H"
    module_0.lazy_import(str_0, str_0, str_0)


def test_case_12():
    str_0 = "([A-a\x0bB09S=(.O)Z"
    var_0 = module_0.IllegalUseOfScopeReplacer(str_0, str_0, str_0)
    var_0.__repr__()


def test_case_13():
    str_0 = "%(bg_black)s%(log_color)s[%(asctime)s] [%(name)s/%(process)d] %(message)s %(blue)s@%(funcName)s:%(lineno)d #%(levelname)s%(reset)s"
    module_0.lazy_import(str_0, str_0)


def test_case_14():
    str_0 = "\\A([A-Za-z_0-9]+)=(.*)\\Z"
    module_0.lazy_import(str_0, str_0, str_0)


def test_case_15():
    str_0 = "gew"
    module_0.ImportReplacer(str_0, str_0, str_0, str_0, str_0)


def test_case_16():
    str_0 = "4{u(\rIgkO3=XK-;\n\rpE&"
    var_0 = module_0.disallow_proxying()
    module_0.lazy_import(str_0, str_0)


def test_case_17():
    str_0 = "Restore the original function to re.compile().\n\n    It is safe to call reset_compile() multiple times, it will always\n    restore re.compile() to the value that existed at import time.\n    Though the first call will reset back to the original (it doesn't\n    track nesting level)\n    "
    module_0.lazy_import(str_0, str_0)


def test_case_18():
    str_0 = "Restore the original functionto re.compile().\n    It is safe to call reset_compile( multiple times, it will alwJys\n    restore re.compile() to the value that exited at import time.\n    Though the first call will reset back to the original (it doesn't\n    rack nesting level)\n    "
    module_0.lazy_import(str_0, str_0)


def test_case_19():
    str_0 = "Restore the original functiontore.compile().\n    It is safe to call reset_compile( multiple times, it will alwJys\n    restore re.compile() to the value that exited at import time.\n    Though the first call will reset back to the original (it desn't\n   rack nesting level)\nY   "
    illegal_use_of_scope_replacer_0 = module_0.IllegalUseOfScopeReplacer(str_0, str_0)
    var_0 = illegal_use_of_scope_replacer_0.__eq__(illegal_use_of_scope_replacer_0)
    illegal_use_of_scope_replacer_0.__repr__()


def test_case_20():
    var_0 = module_0.disallow_proxying()
    dict_0 = {
        var_0: var_0,
        var_0: var_0,
        var_0: var_0,
        var_0: var_0,
        var_0: var_0,
        var_0: var_0,
        var_0: var_0,
        var_0: var_0,
    }
    import_replacer_0 = module_0.ImportReplacer(dict_0, var_0, var_0)
    module_0.lazy_import(import_replacer_0, import_replacer_0)


def test_case_21():
    var_0 = module_0.disallow_proxying()
    dict_0 = {
        var_0: var_0,
        var_0: var_0,
        var_0: var_0,
        var_0: var_0,
        var_0: var_0,
        var_0: var_0,
        var_0: var_0,
    }
    var_1 = var_0.__repr__()
    import_replacer_0 = module_0.ImportReplacer(dict_0, var_1, var_1)
    import_replacer_0.__setattr__(var_0, var_0)


def test_case_22():
    var_0 = module_0.disallow_proxying()
    dict_0 = {
        var_0: var_0,
        var_0: var_0,
        var_0: var_0,
        var_0: var_0,
        var_0: var_0,
        var_0: var_0,
        var_0: var_0,
        var_0: var_0,
        var_0: var_0,
        var_0: var_0,
    }
    var_1 = var_0.__repr__()
    import_replacer_0 = module_0.ImportReplacer(dict_0, var_1, var_1, var_1)
    module_0.lazy_import(var_1, import_replacer_0)
