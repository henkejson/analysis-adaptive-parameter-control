# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import typed_ast._ast3 as module_0
import py_backwards.transformers.base as module_1
import typed_ast.ast3 as module_2


def test_case_0():
    dict_0 = module_0.Dict()
    base_node_transformer_0 = module_1.BaseNodeTransformer(dict_0)


def test_case_1():
    pass_0 = module_0.Pass()
    base_import_rewrite_0 = module_1.BaseImportRewrite(pass_0)
    list_0 = [base_import_rewrite_0, pass_0]
    import_from_0 = module_0.ImportFrom(*list_0)
    base_import_rewrite_0.visit_ImportFrom(import_from_0)


def test_case_2():
    pass_0 = module_0.Pass()
    base_import_rewrite_0 = module_2.iter_child_nodes(pass_0)
    base_import_rewrite_1 = module_1.BaseImportRewrite(pass_0)
    list_0 = [base_import_rewrite_1, base_import_rewrite_0, pass_0]
    import_from_0 = module_0.ImportFrom(*list_0)
    var_0 = base_import_rewrite_1.visit_ImportFrom(import_from_0)


def test_case_3():
    pass_0 = module_0.Pass()
    var_0 = module_2.walk(pass_0)
    base_import_rewrite_0 = module_1.BaseImportRewrite(pass_0)
    list_0 = [base_import_rewrite_0, var_0, pass_0]
    import_from_0 = module_0.ImportFrom(*list_0)
    base_import_rewrite_0.visit_ImportFrom(import_from_0)


def test_case_4():
    pass_0 = module_0.Pass()
    none_type_0 = None
    base_import_rewrite_0 = module_1.BaseImportRewrite(none_type_0)
    var_0 = module_2.iter_child_nodes(pass_0)
    base_import_rewrite_1 = module_1.BaseImportRewrite(pass_0)
    list_0 = [none_type_0, pass_0, none_type_0]
    import_from_0 = module_0.ImportFrom(*list_0)
    base_import_rewrite_1.visit_ImportFrom(import_from_0)
