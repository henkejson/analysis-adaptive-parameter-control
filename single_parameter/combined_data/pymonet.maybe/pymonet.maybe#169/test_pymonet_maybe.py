# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pymonet.maybe as module_0
import builtins as module_1


def test_case_0():
    int_0 = 1510
    maybe_0 = module_0.Maybe(int_0, int_0)


def test_case_1():
    int_0 = 0
    maybe_0 = module_0.Maybe(int_0, int_0)


def test_case_2():
    object_0 = module_1.object()
    dict_0 = {object_0: object_0}
    none_type_0 = None
    maybe_0 = module_0.Maybe(object_0, none_type_0)
    maybe_1 = module_0.Maybe(none_type_0, dict_0)
    var_0 = maybe_1.map(dict_0)
    var_1 = maybe_0.to_either()
    var_2 = var_0.to_validation()
    bool_0 = var_0.__eq__(var_0)
    var_3 = maybe_1.filter(var_2)
    maybe_0.filter(var_2)


def test_case_3():
    none_type_0 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    bool_1 = maybe_0.__eq__(none_type_0)
    var_0 = maybe_0.to_lazy()
    var_0.to_lazy()


def test_case_4():
    none_type_0 = None
    str_0 = "&'gyyd)]HR.1]!'6|GN"
    list_0 = [str_0, str_0, str_0, str_0]
    maybe_0 = module_0.Maybe(list_0, str_0)
    var_0 = maybe_0.ap(none_type_0)
    var_1 = var_0.to_try()
    var_2 = var_0.to_try()
    var_3 = maybe_0.map(none_type_0)


def test_case_5():
    bool_0 = True
    bool_1 = False
    maybe_0 = module_0.Maybe(bool_0, bool_1)
    maybe_0.ap(maybe_0)


def test_case_6():
    int_0 = 1295
    int_1 = -465
    maybe_0 = module_0.Maybe(int_0, int_1)
    var_0 = maybe_0.ap(int_0)
    var_1 = maybe_0.filter(var_0)
    var_2 = maybe_0.to_either()
    none_type_0 = None
    var_3 = maybe_0.to_either()
    maybe_1 = module_0.Maybe(int_0, none_type_0)
    var_4 = maybe_0.bind(int_1)
    bool_0 = var_4.__eq__(maybe_0)
    var_5 = var_4.to_lazy()
    bool_1 = maybe_1.__eq__(int_0)
    bool_2 = var_2.__eq__(int_1)
    maybe_2 = module_0.Maybe(int_1, int_0)
    var_6 = maybe_0.to_try()
    var_7 = maybe_1.to_try()
    var_8 = var_1.to_box()


def test_case_7():
    complex_0 = -2203.8 - 240.853j
    bool_0 = False
    maybe_0 = module_0.Maybe(complex_0, bool_0)
    var_0 = maybe_0.to_box()
    maybe_0.bind(var_0)


def test_case_8():
    object_0 = module_1.object()
    dict_0 = {object_0: object_0}
    none_type_0 = None
    maybe_0 = module_0.Maybe(object_0, none_type_0)
    maybe_1 = module_0.Maybe(none_type_0, dict_0)
    var_0 = maybe_1.filter(maybe_1)
    maybe_0.filter(none_type_0)


def test_case_9():
    object_0 = module_1.object()
    dict_0 = {object_0: object_0}
    complex_0 = 1620.5004 - 1444.616497j
    none_type_0 = None
    maybe_0 = module_0.Maybe(object_0, none_type_0)
    maybe_1 = module_0.Maybe(none_type_0, dict_0)
    var_0 = maybe_1.map(dict_0)
    var_1 = var_0.get_or_else(complex_0)
    var_2 = maybe_0.to_either()
    var_3 = var_0.filter(var_1)
    bool_0 = var_0.__eq__(var_0)
    bool_1 = True
    var_4 = var_2.to_lazy()
    var_5 = var_0.ap(bool_1)
    maybe_0.filter(object_0)


def test_case_10():
    none_type_0 = None
    bool_0 = False
    none_type_1 = None
    maybe_0 = module_0.Maybe(bool_0, none_type_1)
    var_0 = maybe_0.get_or_else(none_type_0)
    var_0.to_lazy()


def test_case_11():
    float_0 = -940.70129
    maybe_0 = module_0.Maybe(float_0, float_0)
    var_0 = maybe_0.to_either()


def test_case_12():
    object_0 = module_1.object()
    dict_0 = {object_0: object_0}
    none_type_0 = None
    maybe_0 = module_0.Maybe(object_0, none_type_0)
    maybe_1 = module_0.Maybe(none_type_0, dict_0)
    var_0 = maybe_0.to_either()
    var_1 = var_0.to_validation()
    var_2 = maybe_1.filter(var_1)
    maybe_0.filter(var_1)


def test_case_13():
    bytes_0 = b"\x16u \xefe\x13\x1c\x83W]\xb1\xd8\xe5\x19\xfa"
    list_0 = [bytes_0, bytes_0]
    maybe_0 = module_0.Maybe(list_0, list_0)
    var_0 = maybe_0.to_box()
    var_0.to_box()


def test_case_14():
    int_0 = 0
    maybe_0 = module_0.Maybe(int_0, int_0)
    set_0 = set()
    maybe_1 = module_0.Maybe(set_0, set_0)
    var_0 = maybe_1.to_box()
    var_1 = var_0.to_validation()
    var_1.filter(maybe_1)


def test_case_15():
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.to_lazy()
    int_0 = -469
    maybe_1 = module_0.Maybe(int_0, int_0)
    var_1 = maybe_1.get_or_else(maybe_1)
    var_2 = maybe_1.to_either()
    var_3 = var_1.ap(maybe_0)
    var_4 = maybe_1.to_either()
    var_5 = var_3.map(var_4)
    var_6 = var_1.filter(var_5)
    var_7 = var_1.filter(var_4)
    var_8 = var_1.bind(var_4)
    var_9 = var_1.ap(maybe_0)
    var_10 = var_2.ap(var_6)
    maybe_2 = module_0.Maybe(int_0, maybe_1)
    var_11 = var_1.to_validation()
    var_12 = maybe_1.bind(var_3)
    bool_1 = maybe_1.__eq__(maybe_1)


def test_case_16():
    int_0 = 1499
    bool_0 = False
    maybe_0 = module_0.Maybe(int_0, bool_0)
    var_0 = maybe_0.to_try()
    var_0.to_try()


def test_case_17():
    int_0 = 1203
    maybe_0 = module_0.Maybe(int_0, int_0)
    var_0 = maybe_0.to_validation()


def test_case_18():
    set_0 = set()
    bool_0 = False
    maybe_0 = module_0.Maybe(set_0, bool_0)
    var_0 = maybe_0.to_validation()


def test_case_19():
    int_0 = -465
    maybe_0 = module_0.Maybe(int_0, int_0)
    var_0 = maybe_0.ap(int_0)
    var_1 = maybe_0.filter(var_0)
    var_2 = maybe_0.to_either()
    none_type_0 = None
    maybe_1 = module_0.Maybe(int_0, none_type_0)
    var_3 = maybe_0.bind(int_0)
    bool_0 = var_3.__eq__(maybe_0)
    var_4 = var_3.to_lazy()
    bool_1 = maybe_1.__eq__(bool_0)
    var_5 = maybe_1.to_lazy()
    var_6 = var_3.bind(var_5)
    var_7 = var_0.to_validation()
    maybe_2 = module_0.Maybe(bool_1, var_5)
    var_8 = var_3.to_lazy()
    var_9 = var_4.to_box()
    maybe_1.filter(var_2)


def test_case_20():
    float_0 = -1042.31
    none_type_0 = None
    maybe_0 = module_0.Maybe(float_0, none_type_0)
    int_0 = 0
    bool_0 = True
    bool_1 = False
    maybe_1 = module_0.Maybe(bool_0, bool_1)
    var_0 = maybe_1.to_lazy()
    var_1 = var_0.to_box()
    var_2 = var_1.to_try()
    var_3 = var_2.get_or_else(int_0)
    var_3.to_try()


def test_case_21():
    object_0 = module_1.object()
    dict_0 = {object_0: object_0}
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    complex_0 = 1620.5004 - 1444.616497j
    none_type_1 = None
    maybe_1 = module_0.Maybe(maybe_0, complex_0)
    maybe_2 = module_0.Maybe(object_0, none_type_1)
    bool_0 = maybe_2.__eq__(maybe_2)
    maybe_3 = module_0.Maybe(none_type_1, dict_0)
    var_0 = maybe_3.filter(maybe_2)
    var_1 = var_0.get_or_else(complex_0)
    var_2 = maybe_2.to_either()
    var_3 = var_0.filter(var_1)
    var_4 = var_0.to_validation()
    bool_1 = var_0.__eq__(var_0)
    var_5 = maybe_3.filter(var_4)
    maybe_4 = module_0.Maybe(var_3, none_type_1)
    var_6 = var_3.filter(maybe_3)
    var_7 = maybe_3.map(var_2)
    var_8 = var_0.ap(var_3)
    var_9 = maybe_4.to_box()
    maybe_2.filter(var_1)
