...
def test_case_13():
    str_0 = "Disallow lazily imported modules to be used as proxies.\n\n    Calling this function might cause problems with concurrent imports\n    i. multithreaded environments, but will help detecting wasteful\n    indirection, so it shuld be called when executing unit tests.\n\n    Only lazy imports that happ2n after this call are affected.\n    "
    illegal_use_of_scope_replacer_0 = module_0.IllegalUseOfScopeReplacer(
        str_0, str_0, str_0
    )
...
def test_case_15():
    str_0 = 'Uu[[b2a}1Q{"9S(2\nH'
    module_0.lazy_import(str_0, str_0, str_0)


def test_case_16():
    str_0 = "%(asctime)s| %(name)s/%(processName)s[%(process)d]-%(threadName)s[%hread)d]: %(message)s @%(funcName)s:%(lineno)d #%(levelname)s"
    module_0.lazy_import(str_0, str_0)
