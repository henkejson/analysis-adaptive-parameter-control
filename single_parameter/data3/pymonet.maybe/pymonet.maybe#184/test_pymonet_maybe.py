# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pymonet.maybe as module_0
import builtins as module_1


def test_case_0():
    int_0 = -2471
    maybe_0 = module_0.Maybe(int_0, int_0)


def test_case_1():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)


def test_case_2():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.map(maybe_0)
    bool_1 = maybe_0.__eq__(maybe_0)


def test_case_3():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    maybe_0.ap(maybe_0)


def test_case_4():
    bool_0 = False
    bool_1 = False
    maybe_0 = module_0.Maybe(bool_0, bool_1)
    var_0 = maybe_0.to_lazy()
    maybe_0.bind(var_0)


def test_case_5():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.map(maybe_0)
    none_type_0 = None
    maybe_1 = module_0.Maybe(bool_0, bool_0)
    var_1 = maybe_1.ap(bool_0)
    var_2 = var_1.to_either()
    var_3 = var_2.to_lazy()
    var_4 = var_0.ap(none_type_0)


def test_case_6():
    complex_0 = -2160.5774 + 1498j
    bool_0 = False
    maybe_0 = module_0.Maybe(complex_0, bool_0)
    maybe_1 = module_0.Maybe(complex_0, complex_0)
    var_0 = maybe_1.filter(maybe_1)
    maybe_0.filter(bool_0)


def test_case_7():
    complex_0 = -3419.5 + 2377.2293j
    maybe_0 = module_0.Maybe(complex_0, complex_0)
    set_0 = {complex_0}
    maybe_1 = module_0.Maybe(set_0, set_0)
    float_0 = -4525.533
    var_0 = maybe_1.get_or_else(complex_0)
    var_1 = maybe_1.to_validation()
    maybe_2 = module_0.Maybe(float_0, float_0)
    var_1.bind(var_0)


def test_case_8():
    complex_0 = -2160.5774 + 1498j
    bool_0 = False
    maybe_0 = module_0.Maybe(complex_0, bool_0)
    maybe_1 = module_0.Maybe(complex_0, complex_0)
    var_0 = maybe_0.get_or_else(bool_0)
    var_1 = maybe_1.filter(maybe_1)
    maybe_0.filter(bool_0)


def test_case_9():
    complex_0 = -2159.535538643912 + 1498j
    bool_0 = False
    maybe_0 = module_0.Maybe(complex_0, bool_0)
    maybe_1 = module_0.Maybe(complex_0, complex_0)
    var_0 = maybe_1.filter(maybe_1)
    var_1 = var_0.to_either()
    maybe_0.filter(bool_0)


def test_case_10():
    dict_0 = {}
    maybe_0 = module_0.Maybe(dict_0, dict_0)
    var_0 = maybe_0.to_either()


def test_case_11():
    bytes_0 = b"xf\xcc\xcf"
    str_0 = "]f}h"
    list_0 = [str_0, str_0]
    maybe_0 = module_0.Maybe(list_0, list_0)
    var_0 = maybe_0.to_box()
    var_0.ap(bytes_0)


def test_case_12():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    var_0 = maybe_0.to_box()


def test_case_13():
    int_0 = 924
    bool_0 = True
    maybe_0 = module_0.Maybe(int_0, bool_0)
    var_0 = maybe_0.to_lazy()
    var_0.to_lazy()


def test_case_14():
    str_0 = "hx_#2pkq"
    list_0 = []
    maybe_0 = module_0.Maybe(list_0, list_0)
    var_0 = maybe_0.to_lazy()
    var_1 = var_0.bind(str_0)
    var_1.to_either()


def test_case_15():
    complex_0 = -2160.5774 + 1498j
    bool_0 = False
    maybe_0 = module_0.Maybe(complex_0, bool_0)
    maybe_1 = module_0.Maybe(complex_0, complex_0)
    var_0 = maybe_1.filter(maybe_1)
    var_1 = var_0.to_try()
    maybe_0.filter(bool_0)


def test_case_16():
    set_0 = set()
    maybe_0 = module_0.Maybe(set_0, set_0)
    var_0 = maybe_0.to_try()


def test_case_17():
    none_type_0 = None
    bool_0 = False
    bool_1 = False
    maybe_0 = module_0.Maybe(bool_0, bool_1)
    maybe_1 = module_0.Maybe(none_type_0, none_type_0)
    var_0 = maybe_0.to_either()
    var_1 = maybe_0.to_validation()
    var_1.get_or_else(bool_0)


def test_case_18():
    tuple_0 = ()
    maybe_0 = module_0.Maybe(tuple_0, tuple_0)
    bool_0 = maybe_0.__eq__(maybe_0)
    var_0 = maybe_0.to_box()
    int_0 = 2963
    maybe_1 = module_0.Maybe(int_0, int_0)
    var_1 = maybe_1.get_or_else(tuple_0)
    module_1.object(**var_0)


def test_case_19():
    str_0 = "m1&+"
    none_type_0 = None
    maybe_0 = module_0.Maybe(str_0, none_type_0)
    var_0 = maybe_0.to_lazy()
    bool_0 = maybe_0.__eq__(var_0)
    maybe_0.filter(var_0)


def test_case_20():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.map(maybe_0)
    none_type_0 = None
    maybe_1 = var_0.bind(none_type_0)
    var_1 = maybe_1.ap(bool_0)
    var_2 = maybe_1.get_or_else(maybe_0)
    var_3 = maybe_0.to_box()
    var_4 = var_1.bind(var_3)
    var_3.bind(var_1)


def test_case_21():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.map(maybe_0)
    none_type_0 = None
    maybe_1 = module_0.Maybe(none_type_0, bool_0)
    var_1 = maybe_0.to_lazy()
    var_2 = maybe_1.filter(var_1)
    bool_1 = var_2.__eq__(var_0)
    var_3 = var_2.get_or_else(var_1)
    var_4 = maybe_0.filter(var_1)
    var_5 = var_1.map(bool_0)
    var_6 = var_1.map(var_3)
    var_7 = maybe_0.to_try()
    var_8 = maybe_1.ap(var_3)
    bool_2 = var_6.__eq__(bool_0)
    var_5.to_box()


def test_case_22():
    complex_0 = -2160.5774 + 1497.61j
    bool_0 = False
    maybe_0 = module_0.Maybe(complex_0, bool_0)
    maybe_1 = module_0.Maybe(complex_0, complex_0)
    var_0 = maybe_1.filter(complex_0)
    var_1 = maybe_1.to_validation()
    var_2 = maybe_1.filter(maybe_1)
    bool_1 = False
    maybe_2 = module_0.Maybe(var_1, bool_1)
    maybe_3 = module_0.Maybe(var_0, var_1)
    bool_2 = maybe_1.__eq__(var_0)
    maybe_0.filter(bool_0)
