# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pymonet.maybe as module_0


def test_case_0():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)


def test_case_1():
    dict_0 = {}
    maybe_0 = module_0.Maybe(dict_0, dict_0)


def test_case_2():
    none_type_0 = None
    none_type_1 = None
    bool_0 = False
    maybe_0 = module_0.Maybe(none_type_1, bool_0)
    maybe_1 = module_0.Maybe(none_type_1, none_type_1)
    bool_1 = maybe_1.__eq__(none_type_0)


def test_case_3():
    float_0 = 369.944
    bool_0 = False
    bool_1 = False
    bool_2 = True
    maybe_0 = module_0.Maybe(bool_1, bool_2)
    var_0 = maybe_0.map(bool_0)
    bool_3 = var_0.__eq__(float_0)


def test_case_4():
    bytes_0 = b"X\xcd\x0fy'\xfc\x9b8Ha\x8d"
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    maybe_0.map(bytes_0)


def test_case_5():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.map(bool_0)
    var_1 = var_0.to_lazy()
    maybe_1 = var_0.map(var_0)
    var_2 = var_0.bind(var_1)
    var_3 = maybe_0.filter(var_0)
    var_4 = var_3.bind(var_1)
    var_5 = maybe_0.to_lazy()
    var_6 = var_1.ap(var_5)
    bool_1 = var_0.__eq__(var_1)
    maybe_2 = module_0.Maybe(var_1, var_6)
    var_6.to_validation()


def test_case_6():
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.to_box()
    maybe_0.bind(var_0)


def test_case_7():
    bool_0 = False
    bool_1 = True
    maybe_0 = module_0.Maybe(bool_1, bool_1)
    var_0 = maybe_0.ap(bool_0)


def test_case_8():
    bool_0 = False
    none_type_0 = None
    maybe_0 = module_0.Maybe(bool_0, none_type_0)
    var_0 = maybe_0.to_lazy()
    var_1 = maybe_0.to_lazy()
    maybe_0.ap(maybe_0)


def test_case_9():
    bytes_0 = b"X\xcd\x0fy'\xfc\x9b8Ha\x8d"
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.filter(bytes_0)
    bool_1 = False
    maybe_1 = module_0.Maybe(bytes_0, bool_1)
    maybe_1.filter(maybe_1)


def test_case_10():
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    none_type_0 = None
    maybe_0.filter(none_type_0)


def test_case_11():
    float_0 = -287.126
    bytes_0 = b"\xe3\xdb\x8d\xab\xd1\x8d'k"
    tuple_0 = (float_0, bytes_0)
    maybe_0 = module_0.Maybe(tuple_0, bytes_0)
    var_0 = maybe_0.to_try()
    var_1 = maybe_0.get_or_else(var_0)
    list_0 = [maybe_0, bytes_0, bytes_0, var_1]
    bool_0 = False
    maybe_1 = module_0.Maybe(list_0, bool_0)
    var_2 = maybe_1.to_box()


def test_case_12():
    none_type_0 = None
    bool_0 = False
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    maybe_1 = module_0.Maybe(none_type_0, none_type_0)
    bool_1 = maybe_1.__eq__(none_type_0)
    var_0 = maybe_1.get_or_else(bool_0)
    var_0.to_lazy()


def test_case_13():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.map(bool_0)
    var_1 = var_0.to_lazy()
    maybe_1 = module_0.Maybe(var_0, bool_0)
    var_2 = var_0.to_validation()
    var_3 = maybe_0.filter(var_0)
    var_4 = var_1.to_either()
    var_5 = var_3.to_box()
    var_6 = maybe_0.to_lazy()
    var_7 = var_3.to_either()
    bool_1 = var_0.__eq__(var_1)
    maybe_2 = module_0.Maybe(maybe_1, var_5)
    var_5.to_box()


def test_case_14():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    var_0 = maybe_0.to_either()
    var_1 = maybe_0.to_lazy()
    var_2 = maybe_0.get_or_else(maybe_0)
    maybe_0.map(var_2)


def test_case_15():
    str_0 = "One[value={}]"
    bytes_0 = b"\x16\xab\xea\x0ep\xed\x03~\xb1d\xc4\xf5"
    bool_0 = True
    maybe_0 = module_0.Maybe(bytes_0, bool_0)
    var_0 = maybe_0.ap(str_0)
    var_1 = var_0.to_box()
    var_1.to_box()


def test_case_16():
    bytes_0 = b"X\xcd\x0fy'\xfc\x9b8Ha\x8d"
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.to_try()
    var_1 = maybe_0.filter(bytes_0)
    var_2 = var_1.to_lazy()
    bool_1 = False
    maybe_1 = module_0.Maybe(bytes_0, bool_1)
    maybe_1.filter(maybe_1)


def test_case_17():
    tuple_0 = ()
    str_0 = "-r\\Q"
    bool_0 = False
    maybe_0 = module_0.Maybe(str_0, bool_0)
    var_0 = maybe_0.to_lazy()
    var_1 = var_0.to_validation()
    var_1.ap(tuple_0)


def test_case_18():
    bool_0 = False
    bool_1 = True
    maybe_0 = module_0.Maybe(bool_1, bool_1)
    var_0 = maybe_0.ap(bool_0)
    var_1 = var_0.to_try()


def test_case_19():
    none_type_0 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    none_type_1 = None
    bool_1 = False
    maybe_1 = module_0.Maybe(none_type_1, bool_1)
    var_0 = maybe_1.to_try()
    maybe_1.filter(none_type_0)


def test_case_20():
    bool_0 = False
    none_type_0 = None
    bool_1 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_1)
    var_0 = maybe_0.ap(bool_0)
    var_1 = var_0.to_validation()
    var_2 = var_1.to_either()
    var_3 = var_2.to_validation()


def test_case_21():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    var_0 = maybe_0.to_validation()
    int_0 = -664
    bool_0 = False
    maybe_1 = module_0.Maybe(int_0, bool_0)
    maybe_1.filter(var_0)


def test_case_22():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.ap(bool_0)
    var_1 = maybe_0.filter(var_0)
    var_2 = var_0.to_box()
    var_3 = var_0.get_or_else(bool_0)
    var_4 = maybe_0.bind(var_3)
    var_5 = var_2.to_lazy()
    bool_1 = maybe_0.__eq__(var_4)
    bool_2 = var_3.__eq__(bool_0)
    var_6 = maybe_0.get_or_else(maybe_0)
    var_7 = maybe_0.ap(var_6)
    var_8 = maybe_0.filter(var_6)
    bool_3 = var_6.__eq__(maybe_0)
    var_9 = var_7.filter(var_6)
    var_10 = maybe_0.to_lazy()


def test_case_23():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.map(maybe_0)
    bool_1 = False
    var_1 = var_0.get_or_else(bool_1)
    bool_2 = False
    maybe_1 = module_0.Maybe(var_1, bool_2)
    var_2 = maybe_0.bind(maybe_1)
    var_3 = var_2.filter(bool_1)
    var_4 = maybe_0.to_box()
    bool_3 = var_0.__eq__(bool_2)
    maybe_2 = module_0.Maybe(var_1, maybe_1)
    var_5 = var_3.ap(maybe_1)
    bool_4 = maybe_1.__eq__(maybe_0)
    maybe_3 = module_0.Maybe(bool_4, maybe_0)
    var_6 = maybe_2.to_box()
    maybe_4 = module_0.Maybe(bool_4, var_1)
    var_7 = maybe_1.to_validation()
    var_8 = maybe_2.to_lazy()
    var_9 = var_2.filter(var_4)


def test_case_24():
    none_type_0 = None
    dict_0 = {none_type_0: none_type_0, none_type_0: none_type_0}
    maybe_0 = module_0.Maybe(dict_0, none_type_0)
    maybe_1 = module_0.Maybe(dict_0, none_type_0)
    maybe_2 = module_0.Maybe(none_type_0, none_type_0)
    bool_0 = maybe_1.__eq__(maybe_1)
    var_0 = maybe_2.to_either()
    bool_1 = True
    maybe_1.bind(bool_1)
