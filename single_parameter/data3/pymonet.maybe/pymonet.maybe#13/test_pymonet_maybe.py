# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pymonet.maybe as module_0


def test_case_0():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)


def test_case_1():
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bool_0)


def test_case_2():
    bytes_0 = b"\xf8\xce\xe3\xdf\x8e3\x8f"
    none_type_0 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bytes_0)
    var_0 = maybe_0.filter(none_type_0)
    var_1 = var_0.to_lazy()
    var_2 = var_1.bind(none_type_0)
    bool_1 = var_2.__eq__(bool_0)
    maybe_1 = module_0.Maybe(none_type_0, bool_0)
    var_3 = maybe_1.get_or_else(maybe_1)
    var_4 = maybe_1.ap(var_3)
    var_5 = maybe_1.to_either()
    var_6 = var_5.to_try()
    var_7 = maybe_1.ap(var_5)
    bool_2 = var_3.__eq__(var_3)
    var_8 = var_5.to_validation()
    var_9 = var_7.to_box()
    var_8.ap(none_type_0)


def test_case_3():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    bool_0 = maybe_0.__eq__(none_type_0)
    var_0 = module_0.Maybe(none_type_0, maybe_0)
    var_1 = var_0.map(var_0)
    maybe_0.bind(var_0)


def test_case_4():
    none_type_0 = None
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, none_type_0)
    maybe_0.map(bool_0)


def test_case_5():
    bytes_0 = b"\xf8\xce\x98\xdf\x8e3\x8f"
    none_type_0 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bytes_0)
    var_0 = maybe_0.filter(none_type_0)
    var_1 = var_0.to_lazy()
    var_2 = var_1.bind(none_type_0)
    bool_1 = var_2.__eq__(bool_0)
    maybe_1 = module_0.Maybe(none_type_0, bool_0)
    var_3 = maybe_1.get_or_else(maybe_1)
    var_4 = maybe_1.ap(var_3)
    var_5 = var_0.get_or_else(maybe_1)
    var_6 = var_5.get_or_else(maybe_1)
    var_7 = maybe_1.to_either()
    bool_2 = False
    maybe_2 = module_0.Maybe(var_3, bool_2)
    var_8 = maybe_1.map(var_3)
    var_9 = maybe_1.bind(bool_1)
    var_10 = maybe_1.filter(var_3)


def test_case_6():
    bytes_0 = b"\xa4\xd5f\xd2\xba`\x0e\xf9"
    none_type_0 = None
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bytes_0)
    var_0 = maybe_0.filter(none_type_0)
    var_1 = maybe_0.ap(var_0)
    maybe_1 = module_0.Maybe(none_type_0, bool_0)
    var_2 = maybe_1.get_or_else(maybe_1)
    maybe_1.filter(var_2)


def test_case_7():
    bytes_0 = b"0\xcb\xb0\xe0\xc5:\xc4}\xbf\xb1{_~`\xac\x9a"
    bool_0 = False
    maybe_0 = module_0.Maybe(bytes_0, bool_0)
    var_0 = maybe_0.get_or_else(bytes_0)
    dict_0 = {bytes_0: bytes_0, bytes_0: bytes_0}
    bool_1 = False
    var_1 = maybe_0.to_either()
    bool_2 = var_0.__eq__(dict_0)
    maybe_1 = module_0.Maybe(dict_0, bool_1)
    maybe_1.ap(bytes_0)


def test_case_8():
    bytes_0 = b"\xa4\xd5f\xd2\xba`\x0e\xf9"
    none_type_0 = None
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bytes_0)
    var_0 = maybe_0.filter(none_type_0)
    var_1 = var_0.to_lazy()
    maybe_1 = module_0.Maybe(none_type_0, bool_0)
    maybe_1.filter(maybe_0)


def test_case_9():
    int_0 = 0
    set_0 = {int_0}
    bool_0 = False
    maybe_0 = module_0.Maybe(set_0, bool_0)
    maybe_0.filter(set_0)


def test_case_10():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    var_0 = maybe_0.get_or_else(none_type_0)
    maybe_0.filter(none_type_0)


def test_case_11():
    tuple_0 = ()
    bool_0 = True
    maybe_0 = module_0.Maybe(tuple_0, bool_0)
    var_0 = maybe_0.to_either()
    var_1 = var_0.to_lazy()
    var_1.to_lazy()


def test_case_12():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    var_0 = maybe_0.to_either()
    var_1 = var_0.to_lazy()


def test_case_13():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.to_box()
    var_0.to_box()


def test_case_14():
    none_type_0 = None
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.get_or_else(none_type_0)
    var_1 = maybe_0.to_box()
    maybe_0.filter(maybe_0)


def test_case_15():
    list_0 = []
    none_type_0 = None
    maybe_0 = module_0.Maybe(list_0, none_type_0)
    var_0 = maybe_0.to_lazy()
    var_1 = var_0.to_try()
    var_1.to_box()


def test_case_16():
    bytes_0 = b"\xa4\xd5f\xd2\xba`\x0e\xf9"
    none_type_0 = None
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bytes_0)
    var_0 = maybe_0.filter(none_type_0)
    var_1 = var_0.to_try()
    bool_1 = maybe_0.__eq__(none_type_0)
    maybe_1 = module_0.Maybe(none_type_0, bool_0)
    var_2 = maybe_1.get_or_else(maybe_1)
    maybe_1.filter(var_2)


def test_case_17():
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.to_try()
    float_0 = -1396.806147
    maybe_0.map(float_0)


def test_case_18():
    none_type_0 = None
    bytes_0 = b"@\xd3-]\x80\xa4"
    maybe_0 = module_0.Maybe(bytes_0, bytes_0)
    var_0 = maybe_0.to_validation()
    var_0.get_or_else(none_type_0)


def test_case_19():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    var_0 = maybe_0.to_validation()
    var_0.filter(var_0)


def test_case_20():
    none_type_0 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_0.to_try()
    none_type_1 = None
    bool_1 = False
    maybe_1 = module_0.Maybe(bool_1, var_0)
    var_1 = maybe_1.filter(none_type_1)
    var_2 = var_1.to_lazy()
    var_3 = var_1.to_try()
    bool_2 = maybe_1.__eq__(none_type_1)
    maybe_2 = module_0.Maybe(none_type_1, bool_1)
    var_4 = var_2.map(bool_1)
    var_5 = var_2.to_validation()
    var_6 = maybe_2.get_or_else(maybe_2)
    maybe_2.filter(var_6)


def test_case_21():
    complex_0 = -1583.779484 + 797.148j
    bool_0 = False
    bool_1 = False
    maybe_0 = module_0.Maybe(bool_0, bool_1)
    bool_2 = maybe_0.__eq__(maybe_0)
    maybe_0.bind(complex_0)


def test_case_22():
    int_0 = 1
    none_type_0 = None
    bool_0 = False
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    maybe_1 = module_0.Maybe(none_type_0, none_type_0)
    bool_1 = maybe_1.__eq__(maybe_0)
    var_0 = maybe_1.to_lazy()
    var_1 = var_0.map(int_0)
    var_1.to_lazy()
