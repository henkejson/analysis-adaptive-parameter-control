# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import pymonet.maybe as module_0
import builtins as module_1


def test_case_0():
    bytes_0 = b"[\x15\x15"
    maybe_0 = module_0.Maybe(bytes_0, bytes_0)


def test_case_1():
    set_0 = set()
    maybe_0 = module_0.Maybe(set_0, set_0)


def test_case_2():
    bytes_0 = b"\xc0\x84G\xe4^O\xc6=\xdeg\xb8\xd0#"
    maybe_0 = module_0.Maybe(bytes_0, bytes_0)
    var_0 = maybe_0.to_validation()
    bool_0 = True
    maybe_1 = module_0.Maybe(bytes_0, bool_0)
    bool_1 = maybe_1.__eq__(bytes_0)
    none_type_0 = None
    bool_2 = True
    var_1 = maybe_1.filter(none_type_0)
    bool_3 = maybe_1.__eq__(var_1)
    maybe_2 = module_0.Maybe(none_type_0, bool_2)
    var_2 = maybe_2.to_try()
    bool_4 = maybe_1.__eq__(var_0)
    var_3 = maybe_2.bind(var_0)
    var_4 = var_3.filter(var_1)
    var_2.ap(maybe_2)


def test_case_3():
    bool_0 = False
    bool_1 = False
    none_type_0 = None
    bool_2 = False
    maybe_0 = module_0.Maybe(bool_1, none_type_0)
    bool_3 = maybe_0.__eq__(bool_0)
    var_0 = maybe_0.to_box()
    maybe_1 = module_0.Maybe(none_type_0, bool_2)
    maybe_1.bind(bool_0)


def test_case_4():
    float_0 = -645.029
    none_type_0 = None
    int_0 = -647
    str_0 = "78"
    maybe_0 = module_0.Maybe(str_0, str_0)
    var_0 = maybe_0.map(int_0)
    var_1 = var_0.ap(none_type_0)
    var_2 = var_1.to_lazy()
    var_3 = var_1.get_or_else(var_1)
    var_4 = var_3.to_validation()
    maybe_1 = module_0.Maybe(var_4, var_4)
    var_5 = maybe_1.filter(float_0)


def test_case_5():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    var_0 = maybe_0.to_validation()
    var_1 = maybe_0.to_box()
    maybe_0.map(var_0)


def test_case_6():
    none_type_0 = None
    bool_0 = False
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    maybe_0.bind(maybe_0)


def test_case_7():
    none_type_0 = None
    bool_0 = True
    bool_1 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_1)
    maybe_1 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_1.to_either()
    var_1 = maybe_0.ap(var_0)
    var_2 = maybe_0.to_lazy()
    var_3 = maybe_1.to_either()
    var_4 = var_0.to_validation()
    var_5 = maybe_1.to_try()
    var_5.to_lazy()


def test_case_8():
    none_type_0 = None
    bool_0 = False
    bool_1 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_1)
    maybe_1 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_1.to_either()
    var_1 = maybe_0.ap(var_0)
    var_2 = maybe_0.bind(var_0)
    var_3 = var_2.to_validation()
    var_4 = var_0.ap(var_1)
    var_5 = var_2.get_or_else(var_4)
    var_6 = var_4.to_validation()
    var_7 = var_5.to_validation()
    var_8 = maybe_0.to_lazy()
    var_9 = maybe_0.map(maybe_0)
    var_10 = maybe_1.ap(var_1)
    var_11 = var_5.to_lazy()
    var_12 = var_1.filter(var_4)
    var_6.filter(var_3)


def test_case_9():
    bool_0 = False
    none_type_0 = None
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.to_either()
    maybe_1 = module_0.Maybe(bool_0, var_0)
    var_1 = maybe_1.filter(none_type_0)
    maybe_0.filter(maybe_0)


def test_case_10():
    bool_0 = False
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    var_0 = maybe_0.to_try()
    var_1 = maybe_0.get_or_else(maybe_0)
    bool_1 = var_1.__eq__(bool_0)
    var_1.ap(var_1)


def test_case_11():
    bool_0 = True
    maybe_0 = module_0.Maybe(bool_0, bool_0)
    bool_1 = maybe_0.__eq__(maybe_0)
    none_type_0 = None
    bool_2 = True
    var_0 = maybe_0.filter(none_type_0)
    bool_3 = maybe_0.__eq__(var_0)
    maybe_1 = module_0.Maybe(none_type_0, bool_2)
    var_1 = maybe_1.to_try()
    var_2 = maybe_1.get_or_else(maybe_1)
    var_3 = var_2.to_box()
    var_1.to_validation()


def test_case_12():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    var_0 = maybe_0.to_box()
    var_0.ap(var_0)


def test_case_13():
    float_0 = -2751.8006
    bytes_0 = b"#\x19\x83X"
    maybe_0 = module_0.Maybe(float_0, bytes_0)
    var_0 = maybe_0.to_lazy()
    var_0.to_lazy()


def test_case_14():
    float_0 = 1670.865
    bool_0 = False
    maybe_0 = module_0.Maybe(float_0, bool_0)
    var_0 = maybe_0.to_lazy()
    none_type_0 = None
    var_1 = maybe_0.to_try()
    maybe_0.ap(none_type_0)


def test_case_15():
    bytes_0 = b"0\xa3\x17\x19"
    list_0 = [bytes_0, bytes_0]
    bool_0 = True
    maybe_0 = module_0.Maybe(list_0, bool_0)
    var_0 = maybe_0.to_try()


def test_case_16():
    none_type_0 = None
    bool_0 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    var_0 = maybe_0.to_validation()


def test_case_17():
    none_type_0 = None
    bool_0 = True
    bytes_0 = b"`\xea\xc3\xc6I\xab\x8d\\\xb7"
    dict_0 = {}
    list_0 = [bool_0, bytes_0, dict_0, bool_0]
    maybe_0 = module_0.Maybe(list_0, dict_0)
    var_0 = maybe_0.to_validation()
    var_0.get_or_else(none_type_0)


def test_case_18():
    none_type_0 = None
    maybe_0 = module_0.Maybe(none_type_0, none_type_0)
    object_0 = module_1.object()
    none_type_1 = None
    bool_0 = True
    maybe_1 = module_0.Maybe(none_type_1, bool_0)
    var_0 = maybe_1.to_lazy()
    var_1 = var_0.to_either()
    var_2 = maybe_1.to_try()
    var_3 = var_2.bind(var_0)
    var_4 = var_1.to_lazy()
    maybe_2 = module_0.Maybe(var_4, var_1)
    var_5 = var_4.map(none_type_1)
    var_6 = maybe_2.get_or_else(object_0)
    var_6.get_or_else(none_type_0)


def test_case_19():
    none_type_0 = None
    bool_0 = False
    bool_1 = True
    maybe_0 = module_0.Maybe(none_type_0, bool_0)
    maybe_1 = module_0.Maybe(bool_0, bool_1)
    var_0 = maybe_1.to_validation()
    var_1 = maybe_1.ap(bool_0)
    bool_2 = maybe_0.__eq__(maybe_0)
    bool_3 = var_1.__eq__(bool_1)
    var_2 = var_1.to_validation()
    var_2.to_validation()


def test_case_20():
    complex_0 = 2571.1 - 851.605j
    int_0 = 1045
    float_0 = 505.5
    bytes_0 = b"\x0eZ\xfe\xe9FP\xf7\xa2\x97\x00"
    tuple_0 = (int_0, float_0, bytes_0, bytes_0)
    bool_0 = False
    maybe_0 = module_0.Maybe(tuple_0, bool_0)
    var_0 = maybe_0.to_lazy()
    var_1 = var_0.to_validation()
    var_2 = var_0.ap(complex_0)
    var_2.to_lazy()


def test_case_21():
    int_0 = 1889
    maybe_0 = module_0.Maybe(int_0, int_0)
    none_type_0 = None
    var_0 = maybe_0.map(none_type_0)
    var_1 = maybe_0.ap(int_0)
    var_2 = maybe_0.to_either()
    var_3 = maybe_0.to_try()
    bool_0 = var_0.__eq__(maybe_0)
    var_4 = maybe_0.to_try()
    var_4.to_box()
