# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import py_backwards.utils.helpers as module_0


def test_case_0():
    none_type_0 = None
    none_type_1 = module_0.debug(none_type_0)


def test_case_1():
    variables_generator_0 = module_0.VariablesGenerator()


def test_case_2():
    none_type_0 = None
    callable_0 = module_0.eager(none_type_0)
    variables_generator_0 = module_0.VariablesGenerator()
    none_type_1 = module_0.debug(callable_0)


def test_case_3():
    variables_generator_0 = module_0.VariablesGenerator()
    module_0.get_source(variables_generator_0)


def test_case_4():
    int_0 = -3162
    none_type_0 = module_0.debug(int_0)
    none_type_1 = module_0.warn(int_0)


def test_case_5():
    none_type_0 = None
    callable_0 = module_0.eager(none_type_0)
    variables_generator_0 = module_0.VariablesGenerator()
    none_type_1 = module_0.warn(none_type_0)
    list_0 = []
    none_type_2 = module_0.warn(list_0)
    list_1 = []
    str_0 = "c>$US4tx*jwMFw||*Mt"
    none_type_3 = module_0.warn(str_0)
    variables_generator_1 = module_0.VariablesGenerator(*list_1)
    none_type_4 = module_0.debug(none_type_0)
    callable_0.__call__(none_type_3, callable_0, module=none_type_0, type=callable_0)
