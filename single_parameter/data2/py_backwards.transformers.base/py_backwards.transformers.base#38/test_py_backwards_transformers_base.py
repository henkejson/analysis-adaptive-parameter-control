# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import typed_ast._ast3 as module_0
import py_backwards.transformers.base as module_1
import typed_ast.ast3 as module_2


def test_case_0():
    u_sub_0 = module_0.USub()
    base_import_rewrite_0 = module_1.BaseImportRewrite(u_sub_0)


def test_case_1():
    or_0 = module_0.Or()
    base_import_rewrite_0 = module_1.BaseImportRewrite(or_0)
    base_import_rewrite_1 = module_1.BaseImportRewrite(or_0)
    none_type_0 = None
    list_0 = [none_type_0]
    str_0 = "Jv%<EF\t1\\"
    dict_0 = {str_0: or_0, str_0: str_0, str_0: none_type_0}
    import_from_0 = module_0.ImportFrom(*list_0, **dict_0)
    base_import_rewrite_1.visit_ImportFrom(import_from_0)


def test_case_2():
    or_0 = module_0.Or()
    base_import_rewrite_0 = module_1.BaseImportRewrite(or_0)
    list_0 = [or_0, or_0, base_import_rewrite_0]
    dict_0 = {}
    import_from_0 = module_0.ImportFrom(*list_0, **dict_0)
    base_import_rewrite_0.visit_ImportFrom(import_from_0)


def test_case_3():
    or_0 = module_0.Or()
    var_0 = module_2.walk(or_0)
    base_import_rewrite_0 = module_1.BaseImportRewrite(or_0)
    list_0 = [var_0, var_0]
    dict_0 = {}
    import_from_0 = module_0.ImportFrom(*list_0, **dict_0)
    base_import_rewrite_0.visit_ImportFrom(import_from_0)


def test_case_4():
    or_0 = module_0.Or()
    var_0 = module_2.iter_child_nodes(or_0)
    base_import_rewrite_0 = module_1.BaseImportRewrite(var_0)
    list_0 = [var_0, var_0, base_import_rewrite_0]
    dict_0 = {}
    import_from_0 = module_0.ImportFrom(*list_0, **dict_0)
    var_1 = base_import_rewrite_0.visit_ImportFrom(import_from_0)
