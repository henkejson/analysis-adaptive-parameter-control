# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import sanic.config as module_0
import inspect as module_1
import sanic.constants as module_2


def test_case_0():
    config_0 = module_0.Config()


def test_case_1():
    bytes_0 = b"\x93M"
    config_0 = module_0.Config(converters=bytes_0)
    bool_0 = True
    none_type_0 = config_0.register_type(bool_0)
    none_type_1 = None
    var_0 = module_1.isclass(none_type_1)


def test_case_2():
    config_0 = module_0.Config()
    var_0 = module_2.LocalCertCreator.MKCERT
    config_1 = module_0.Config(keep_alive=config_0)
    none_type_0 = config_0.update(**config_0)
    config_2 = module_0.Config(keep_alive=none_type_0, converters=var_0)
    config_1.__getattr__(var_0)


def test_case_3():
    str_0 = 'GtYH#(?\ri1G"Q["'
    config_0 = module_0.Config(env_prefix=str_0)


def test_case_4():
    bool_0 = True
    module_0.Config(bool_0)


def test_case_5():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)
    none_type_0 = None
    none_type_1 = config_0.update()
    var_1 = module_1.getmembers(config_0, none_type_0)
    var_1.respond(config_0)


def test_case_6():
    str_0 = "Get a named task.\n\n        This method is used to get a task by its name. Optionally, you can\n        control whether an exception should be raised if the task is not found.\n\n        Args:\n            name (str): The name of the task to be retrieved.\n            raise_exception (bool): If `True`, an exception will be raised if\n                the task is not found. Defaults to `True`.\n\n        Returns:\n            Optional[Task]: The task, if found.\n        "
    none_type_0 = None
    config_0 = module_0.Config(none_type_0, none_type_0)
    dict_0 = {str_0: str_0, str_0: none_type_0}
    config_1 = module_0.Config(dict_0, none_type_0)
    config_1.__getattr__(dict_0)


def test_case_7():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)


def test_case_8():
    str_0 = 'n{\x0cV"\r(qLT}i'
    str_1 = "(&wM<>\n)bz["
    str_2 = ""
    bool_0 = False
    dict_0 = {str_1: str_0, str_2: bool_0}
    config_0 = module_0.Config(dict_0, str_2, dict_0)
    none_type_0 = None
    config_1 = module_0.Config(none_type_0, none_type_0)
    config_1.update_config(str_0)


def test_case_9():
    bytes_0 = b"\x93M"
    config_0 = module_0.Config(converters=bytes_0)
    bool_0 = True
    config_0.update_config(bool_0)


def test_case_10():
    config_0 = module_0.Config()
    var_0 = config_0.load_environment_vars()
    var_1 = config_0.update_config(config_0)
    config_1 = module_0.Config(converters=config_0)
    config_1.__setitem__(var_1, config_1)


def test_case_11():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)
    none_type_0 = config_0.register_type(var_0)
    config_1 = module_0.Config(converters=config_0)
    config_2 = module_0.Config(keep_alive=config_0)
    none_type_1 = config_0.update(**config_0)
    config_0.register_type(var_0)


def test_case_12():
    config_0 = module_0.Config()
    config_1 = module_0.Config(converters=config_0)
    var_0 = module_1.getmembers(config_1)
    list_0 = [var_0]
    none_type_0 = config_0.update(*list_0)
    config_2 = module_0.Config(config_1)
    config_2.load_environment_vars(list_0)


def test_case_13():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)
    config_1 = module_0.Config(converters=config_0)
    var_1 = module_1.getmembers(config_1)
    list_0 = [var_1]
    none_type_0 = config_0.update(*list_0)
    var_2 = module_1.getmembers(config_0)
    var_2.update_config(var_0)


def test_case_14():
    config_0 = module_0.Config()
    str_0 = "LOCAL_CERT_CREATOR"
    config_1 = module_0.Config(env_prefix=str_0, keep_alive=str_0)
    config_0.__setitem__(str_0, config_1)
