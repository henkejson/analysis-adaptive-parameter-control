# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import sanic.config as module_0
import inspect as module_1


def test_case_0():
    config_0 = module_0.Config()


def test_case_1():
    dict_0 = {}
    config_0 = module_0.Config(env_prefix=dict_0)


def test_case_2():
    int_0 = 255
    str_0 = "_;/NA"
    module_0.Config(int_0, str_0, str_0)


def test_case_3():
    config_0 = module_0.Config()
    config_0.__getattr__(config_0)


def test_case_4():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)


def test_case_5():
    str_0 = ")aJ8\x0bc"
    config_0 = module_0.Config(env_prefix=str_0)


def test_case_6():
    str_0 = ")aJ8\x0bc"
    dict_0 = {}
    config_0 = module_0.Config(env_prefix=dict_0)
    config_1 = module_0.Config(dict_0, str_0)
    config_2 = module_0.Config(config_0, keep_alive=config_0, converters=dict_0)
    var_0 = config_0.update_config(config_2)


def test_case_7():
    str_0 = ")aJ8\x0bc"
    config_0 = module_0.Config()
    config_1 = module_0.Config(env_prefix=str_0, converters=str_0)
    var_0 = config_1.update_config(config_0)


def test_case_8():
    config_0 = module_0.Config()
    none_type_0 = None
    config_1 = module_0.Config(
        env_prefix=none_type_0, keep_alive=none_type_0, converters=none_type_0
    )
    str_0 = "Ff-{JLHp;Kz\\_}}(@,\\i"
    config_0.update_config(str_0)


def test_case_9():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)
    none_type_0 = config_0.register_type(config_0)
    config_0.__getattr__(var_0)


def test_case_10():
    config_0 = module_0.Config()
    none_type_0 = None
    config_0.update_config(none_type_0)


def test_case_11():
    str_0 = ")aJ8\x0bc("
    dict_0 = {str_0: str_0, str_0: str_0, str_0: str_0}
    none_type_0 = None
    config_0 = module_0.Config()
    none_type_1 = config_0.__setitem__(str_0, none_type_0)
    config_1 = module_0.Config()
    var_0 = module_1.isclass(dict_0)


def test_case_12():
    str_0 = "Wkm~<!Kzzb\r]SA"
    str_1 = ")aJ8\x0bc("
    dict_0 = {}
    config_0 = module_0.Config(keep_alive=str_1)
    module_0.Config(keep_alive=dict_0, converters=str_0)


def test_case_13():
    config_0 = module_0.Config()
    str_0 = "LOCAL_CERT_CREATOR"
    config_0.__setitem__(str_0, config_0)
