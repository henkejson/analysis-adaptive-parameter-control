# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import sanic.config as module_0
import inspect as module_1
import sanic.constants as module_2


def test_case_0():
    config_0 = module_0.Config()


def test_case_1():
    str_0 = "2o:p72Cj\nxu6J%7"
    set_0 = {str_0, str_0, str_0, str_0}
    config_0 = module_0.Config(keep_alive=set_0)
    bool_0 = True
    module_0.DescriptorMeta(bool_0)


def test_case_2():
    none_type_0 = None
    config_0 = module_0.Config(env_prefix=none_type_0)
    config_0.update_config(none_type_0)


def test_case_3():
    none_type_0 = None
    config_0 = module_0.Config(keep_alive=none_type_0)
    var_0 = module_1.getmembers(config_0)
    var_1 = config_0.update_config(config_0)
    var_1.__subclasscheck__(none_type_0, config_0)


def test_case_4():
    none_type_0 = None
    list_0 = [none_type_0]
    config_0 = module_0.Config(converters=list_0)


def test_case_5():
    none_type_0 = None
    config_0 = module_0.Config(keep_alive=none_type_0)
    list_0 = [none_type_0]
    config_0.__getattr__(list_0)


def test_case_6():
    none_type_0 = None
    list_0 = [none_type_0]
    config_0 = module_0.Config(converters=list_0)
    none_type_1 = config_0.update(**config_0)


def test_case_7():
    none_type_0 = None
    config_0 = module_0.Config(keep_alive=none_type_0)
    config_0.__setitem__(config_0, config_0)


def test_case_8():
    str_0 = "2o:p72Cj\nxu6J%7"
    set_0 = {str_0, str_0, str_0, str_0}
    none_type_0 = None
    config_0 = module_0.Config(keep_alive=none_type_0, converters=none_type_0)
    var_0 = module_1.isclass(set_0)
    module_0.Config(set_0, keep_alive=none_type_0, converters=config_0)


def test_case_9():
    bool_0 = False
    str_0 = "SanicTestClient"
    dict_0 = {str_0: bool_0, str_0: str_0}
    module_0.Config(dict_0, converters=str_0)


def test_case_10():
    str_0 = "PROXIES_COUNT"
    config_0 = module_0.Config(env_prefix=str_0)
    list_0 = [config_0, config_0, config_0, config_0]
    none_type_0 = config_0.update(*list_0)


def test_case_11():
    config_0 = module_0.Config()
    none_type_0 = module_1.isdatadescriptor(config_0)
    none_type_1 = config_0.register_type(none_type_0)
    var_0 = module_2.LocalCertCreator.AUTO
    config_0.update_config(var_0)


def test_case_12():
    str_0 = "TLS_CERT_PASSWORD"
    bool_0 = False
    str_1 = "I"
    dict_0 = {str_0: bool_0, str_1: str_1}
    config_0 = module_0.Config(dict_0, converters=str_1)
    config_1 = module_0.Config()
    var_0 = config_1.update_config(config_1)
    var_1 = var_0.__bool__()
    str_2 = "LOCAL_CERT_CREATOR"
    config_0.__setattr__(str_2, config_1)


def test_case_13():
    config_0 = module_0.Config()
    str_0 = "_FALLBACK_ERROR_FORMAT"
    str_1 = "ACCESS_LOG"
    none_type_0 = config_0.__setattr__(str_0, str_1)
    var_0 = module_1.getmembers(config_0)
    none_type_1 = config_0.register_type(var_0)
    var_1 = config_0.update_config(config_0)
    config_0.update_config(var_1)
