# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import sanic.config as module_0
import inspect as module_1
import sanic.http.stream as module_2


def test_case_0():
    config_0 = module_0.Config()


def test_case_1():
    config_0 = module_0.Config()
    module_0.Config(env_prefix=config_0)


def test_case_2():
    bytes_0 = b"\xfdt[\xd4,"
    tuple_0 = ()
    config_0 = module_0.Config(tuple_0, converters=tuple_0)
    config_0.__getattr__(bytes_0)


def test_case_3():
    str_0 = "S"
    config_0 = module_0.Config(env_prefix=str_0)


def test_case_4():
    none_type_0 = None
    config_0 = module_0.Config(env_prefix=none_type_0)
    var_0 = config_0.update_config(config_0)
    none_type_1 = None
    var_1 = module_1.getmembers(none_type_1)
    config_1 = module_0.Config(converters=none_type_0)


def test_case_5():
    str_0 = "Starting worker [%s]"
    config_0 = module_0.Config()
    config_0.update_config(str_0)


def test_case_6():
    str_0 = "P;4o@8%M\n]dv6mFX"
    config_0 = module_0.Config(env_prefix=str_0)
    none_type_0 = config_0.register_type(config_0)
    config_0.__getattr__(config_0)


def test_case_7():
    str_0 = "S"
    config_0 = module_0.Config(env_prefix=str_0)
    none_type_0 = config_0.register_type(config_0)


def test_case_8():
    none_type_0 = None
    bytes_0 = b"\xfdt[\xd4,"
    tuple_0 = ()
    config_0 = module_0.Config(tuple_0, keep_alive=tuple_0, converters=none_type_0)
    config_0.__getattr__(bytes_0)


def test_case_9():
    none_type_0 = None
    tuple_0 = ()
    config_0 = module_0.Config(tuple_0, converters=tuple_0)
    var_0 = none_type_0.__bool__()
    config_1 = module_0.Config(config_0, keep_alive=none_type_0)


def test_case_10():
    str_0 = "P;4o@8%M\n]dv6mFX"
    config_0 = module_0.Config(env_prefix=str_0, converters=str_0)
    none_type_0 = None
    config_0.load_environment_vars(none_type_0)


def test_case_11():
    config_0 = module_0.Config()
    none_type_0 = None
    config_0.update_config(none_type_0)


def test_case_12():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)


def test_case_13():
    str_0 = "P;4o@8%M\n]dv6mFX"
    config_0 = module_0.Config(env_prefix=str_0)
    none_type_0 = config_0.register_type(config_0)
    config_0.register_type(config_0)


def test_case_14():
    config_0 = module_0.Config()
    str_0 = "{l\tICd03GP\\$Tv"
    none_type_0 = config_0.__setattr__(str_0, str_0)
    none_type_1 = config_0.register_type(config_0)
    str_1 = "S"
    str_2 = ".&~p}O"
    none_type_2 = config_0.__setattr__(str_2, str_0)
    var_0 = config_0.load_environment_vars()
    config_1 = module_0.Config(env_prefix=str_1)
    config_0.load_environment_vars(str_1)


def test_case_15():
    config_0 = module_0.Config()
    str_0 = "LOCAL_CERT_CREATOR"
    config_0.__setattr__(str_0, str_0)


def test_case_16():
    config_0 = module_0.Config()
    str_0 = "{l\tICd03GP\\$Tv"
    none_type_0 = config_0.__setattr__(str_0, str_0)
    none_type_1 = None
    str_1 = "_"
    stream_0 = module_2.Stream()
    config_1 = module_0.Config(env_prefix=str_1)
    var_0 = config_1.update_config(config_0)
    none_type_2 = config_1.register_type(config_1)
    config_1.update_config(none_type_1)
