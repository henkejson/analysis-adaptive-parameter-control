# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import sanic.config as module_0
import inspect as module_1
import sanic.helpers as module_2


def test_case_0():
    config_0 = module_0.Config()


def test_case_1():
    str_0 = ""
    config_0 = module_0.Config(keep_alive=str_0)
    var_0 = config_0.update_config(config_0)
    config_1 = module_0.Config(converters=str_0)
    config_2 = module_0.Config()


def test_case_2():
    bool_0 = False
    config_0 = module_0.Config(env_prefix=bool_0)


def test_case_3():
    bool_0 = True
    module_0.Config(env_prefix=bool_0)


def test_case_4():
    config_0 = module_0.Config()
    var_0 = module_1.getmembers(config_0)
    config_0.update_config(var_0)


def test_case_5():
    str_0 = " kV=?$j{-8HrUT8"
    config_0 = module_0.Config(keep_alive=str_0)
    config_0.update_config(str_0)


def test_case_6():
    str_0 = "@"
    none_type_0 = None
    config_0 = module_0.Config(keep_alive=none_type_0)
    none_type_1 = config_0.__setitem__(str_0, str_0)
    default_0 = module_2.Default()
    var_0 = config_0.update_config(default_0)
    var_0.load_environment_vars()


def test_case_7():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)


def test_case_8():
    bool_0 = True
    module_0.Config(converters=bool_0)


def test_case_9():
    str_0 = "@"
    bytes_0 = b"\xf6\xa6x\xbb}:h\xd3\xdd#\\H"
    config_0 = module_0.Config(keep_alive=str_0)
    config_1 = module_0.Config(env_prefix=str_0, converters=config_0)
    var_0 = config_1.update_config(config_1)
    config_2 = module_0.Config(config_0, keep_alive=var_0)
    var_1 = module_1.isdatadescriptor(str_0)
    var_0.__new__(str_0, var_0, var_1, bytes_0)


def test_case_10():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)
    config_0.update_config(var_0)


def test_case_11():
    str_0 = "@"
    config_0 = module_0.Config(keep_alive=str_0)
    config_1 = module_0.Config(keep_alive=config_0)
    str_1 = "No server information found for "
    module_0.Config(converters=str_1)


def test_case_12():
    str_0 = "@"
    config_0 = module_0.Config(keep_alive=str_0)
    config_1 = module_0.Config(env_prefix=str_0, converters=config_0)
    var_0 = config_1.update_config(config_1)
    var_1 = module_1.isdatadescriptor(str_0)
    var_0.__new__(str_0, var_0, var_1, var_0)


def test_case_13():
    str_0 = "@"
    config_0 = module_0.Config(keep_alive=str_0)
    config_1 = module_0.Config(env_prefix=str_0, converters=config_0)
    var_0 = config_0.update_config(config_0)
    config_0.__getattr__(config_0)


def test_case_14():
    str_0 = "_FALLBACK_ERROR_FORMAT"
    str_1 = "BbzfbN=ng"
    str_2 = "9!*cN_~"
    dict_0 = {str_0: str_1, str_2: str_2}
    none_type_0 = None
    module_0.Config(dict_0, none_type_0, none_type_0)


def test_case_15():
    str_0 = ""
    config_0 = module_0.Config(keep_alive=str_0)
    config_0.load_environment_vars(str_0)
