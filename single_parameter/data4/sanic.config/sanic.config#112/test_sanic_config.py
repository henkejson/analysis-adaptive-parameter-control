# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import sanic.config as module_0
import inspect as module_1
import sanic.constants as module_2


def test_case_0():
    config_0 = module_0.Config()


def test_case_1():
    bool_0 = True
    module_0.Config(converters=bool_0)


def test_case_2():
    str_0 = "5\x0b9&H0ZD?Mf"
    config_0 = module_0.Config()
    var_0 = module_1.getmembers(config_0)
    none_type_0 = None
    config_1 = module_0.Config(env_prefix=none_type_0, keep_alive=var_0)
    var_1 = config_0.update_config(config_1)
    config_2 = module_0.Config(env_prefix=var_1)
    config_3 = module_0.Config(config_1, keep_alive=str_0, converters=var_0)
    config_0.update_config(var_1)


def test_case_3():
    none_type_0 = None
    config_0 = module_0.Config(env_prefix=none_type_0)
    var_0 = module_1.getmembers(config_0)
    config_0.update_config(var_0)


def test_case_4():
    none_type_0 = None
    config_0 = module_0.Config(env_prefix=none_type_0)
    config_0.update_config(none_type_0)


def test_case_5():
    int_0 = 96
    config_0 = module_0.Config(keep_alive=int_0)
    var_0 = config_0.update_config(config_0)


def test_case_6():
    config_0 = module_0.Config()
    config_0.__setitem__(config_0, config_0)


def test_case_7():
    bool_0 = False
    config_0 = module_0.Config(keep_alive=bool_0, converters=bool_0)


def test_case_8():
    none_type_0 = None
    config_0 = module_0.Config(env_prefix=none_type_0)


def test_case_9():
    str_0 = "?vD-on+p+XmZ"
    config_0 = module_0.Config(env_prefix=str_0)


def test_case_10():
    local_cert_creator_0 = module_2.LocalCertCreator.MKCERT
    config_0 = module_0.Config(env_prefix=local_cert_creator_0)
    config_0.update_config(local_cert_creator_0)


def test_case_11():
    str_0 = "b[tQ)S\nHdV,:%LIG3B`"
    config_0 = module_0.Config(converters=str_0)


def test_case_12():
    str_0 = "5\x0b9&H0ZD?Mf"
    none_type_0 = None
    config_0 = module_0.Config(none_type_0, converters=str_0)
    var_0 = config_0.update_config(config_0)
    var_1 = module_1.getmembers(none_type_0)
    none_type_1 = None
    config_1 = module_0.Config(
        none_type_0, none_type_0, none_type_1, converters=none_type_1
    )
    config_1.__getattr__(config_0)


def test_case_13():
    bool_0 = True
    config_0 = module_0.Config(keep_alive=bool_0)
    none_type_0 = None
    var_0 = module_1.getmembers(none_type_0, none_type_0)
    none_type_1 = None
    list_0 = [bool_0, none_type_0, none_type_1]
    module_0.Config(converters=list_0)


def test_case_14():
    config_0 = module_0.Config()
    var_0 = module_1.getmembers(config_0)
    none_type_0 = None
    config_1 = module_0.Config(env_prefix=none_type_0, keep_alive=var_0)
    var_1 = config_0.update_config(config_1)
    config_2 = module_0.Config(env_prefix=var_1)
    str_0 = "LOCAL_CERT_CREATOR"
    config_2.__setitem__(str_0, none_type_0)
