# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import sanic.config as module_0
import sanic.constants as module_1
import inspect as module_2
import sanic.errorpages as module_3
import sanic.http.http1 as module_4


def test_case_0():
    config_0 = module_0.Config()


def test_case_1():
    local_cert_creator_0 = module_1.LocalCertCreator.MKCERT
    var_0 = module_2.getmembers(local_cert_creator_0)
    module_0.Config(env_prefix=var_0, keep_alive=var_0, converters=var_0)


def test_case_2():
    str_0 = "alt-svc"
    str_1 = "\r+Y#HXWQtnnbbC,"
    str_2 = "4AZdt#blrS{?]Tv\n"
    dict_0 = {str_0: str_0, str_0: str_1, str_2: str_1}
    config_0 = module_0.Config(dict_0)
    var_0 = module_2.getmembers(config_0)
    bytes_0 = b":status"
    var_1 = module_2.isdatadescriptor(bytes_0)
    module_3.check_error_format(var_1)


def test_case_3():
    config_0 = module_0.Config()
    module_4.Http(config_0)


def test_case_4():
    config_0 = module_0.Config()
    bool_0 = False
    none_type_0 = config_0.register_type(bool_0)


def test_case_5():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)


def test_case_6():
    config_0 = module_0.Config()
    none_type_0 = config_0.register_type(config_0)
    none_type_1 = config_0.update()
    var_0 = config_0.load_environment_vars()
    none_type_2 = config_0.update()
    config_0.__setitem__(var_0, none_type_0)


def test_case_7():
    none_type_0 = None
    config_0 = module_0.Config(none_type_0, none_type_0, converters=none_type_0)
    config_1 = module_0.Config()
    bool_0 = False
    none_type_1 = config_1.register_type(bool_0)


def test_case_8():
    config_0 = module_0.Config()
    config_0.__getattr__(config_0)


def test_case_9():
    config_0 = module_0.Config()
    bool_0 = False
    config_0.update_config(bool_0)


def test_case_10():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)
    http_0 = module_2.isdatadescriptor(config_0)
    none_type_0 = config_0.register_type(var_0)
    var_1 = module_2.isdatadescriptor(none_type_0)
    config_0.register_type(var_0)


def test_case_11():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)
    str_0 = var_0.__str__()
    config_0.update_config(str_0)


def test_case_12():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)
    str_0 = "LOCAL_CERT_CREATOR"
    config_0.__setitem__(str_0, var_0)
