# Test cases automatically generated by Pynguin (https://www.pynguin.eu).
# Please check them before you use them.
import sanic.config as module_0
import sanic.utils as module_1
import inspect as module_2


def test_case_0():
    config_0 = module_0.Config()


def test_case_1():
    list_0 = []
    none_type_0 = None
    config_0 = module_0.Config(none_type_0, converters=none_type_0)
    none_type_1 = config_0.update(**config_0)
    config_1 = module_0.Config(converters=config_0)
    config_0.update_config(list_0)


def test_case_2():
    bool_0 = False
    none_type_0 = None
    config_0 = module_0.Config(none_type_0, none_type_0, bool_0)
    config_1 = module_0.Config()
    config_0.update_config(none_type_0)


def test_case_3():
    str_0 = "P"
    config_0 = module_0.Config(env_prefix=str_0)


def test_case_4():
    list_0 = []
    none_type_0 = None
    config_0 = module_0.Config(none_type_0, none_type_0)
    var_0 = config_0.load_environment_vars()
    config_1 = module_0.Config(config_0, var_0, config_0)
    config_1.update_config(list_0)


def test_case_5():
    none_type_0 = None
    config_0 = module_0.Config(converters=none_type_0)
    module_1.load_module_from_file_location(config_0)


def test_case_6():
    none_type_0 = None
    config_0 = module_0.Config()
    config_0.update_config(none_type_0)


def test_case_7():
    config_0 = module_0.Config()
    var_0 = config_0.update_config(config_0)


def test_case_8():
    bytes_0 = b"\xdf\xd7\xf1\xc7\xb3\xf8\x85\x00\x18d6"
    var_0 = module_2.getmembers(bytes_0)
    none_type_0 = None
    var_1 = var_0.__repr__()
    module_0.Config(none_type_0, converters=var_1)


def test_case_9():
    none_type_0 = None
    config_0 = module_0.Config(env_prefix=none_type_0)
    config_0.__getattr__(config_0)


def test_case_10():
    str_0 = "u4VJj;b.MmxIjqG_\t6"
    str_1 = "P"
    config_0 = module_0.Config(env_prefix=str_1)
    var_0 = config_0.update_config(config_0)
    config_0.update_config(str_0)


def test_case_11():
    bool_0 = False
    none_type_0 = None
    config_0 = module_0.Config(env_prefix=bool_0)
    none_type_1 = config_0.update()
    str_0 = "LOCAL_CERT_CREATOR"
    config_0.__setattr__(str_0, none_type_0)


def test_case_12():
    str_0 = "u4VJj;b.MmxIjqG_\t6"
    str_1 = "P"
    config_0 = module_0.Config(env_prefix=str_0)
    module_0.Config(config_0, str_1, converters=str_1)


def test_case_13():
    str_0 = ""
    str_1 = "_"
    config_0 = module_0.Config(env_prefix=str_1)
    config_1 = module_0.Config(env_prefix=str_1)
    config_0.update_config(str_0)
